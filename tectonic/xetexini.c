...
prefixed_command(void)
{
    ...
    case ASSIGN_TOKS:
        ...
        if (mem[def_ref].b32.s1 == MIN_HALFWORD) {
            if (e) {
                if (a >= 4)
                    gsa_def(p, MIN_HALFWORD);
                else
...
load_fmt_file(void)
{
    ...
    eqtb[UNDEFINED_CONTROL_SEQUENCE].b32.s1 = MIN_HALFWORD;
...
initialize_more_variables(void)
{
    ...
    prim_eqtb[0].b32.s1 = MIN_HALFWORD;
    ...
    sa_root[MARK_VAL] = MIN_HALFWORD;
    sa_null.b32.s0 = MIN_HALFWORD;
    sa_null.b32.s1 = MIN_HALFWORD;
...
initialize_more_initex_variables(void)
{
    ...
    eqtb[BOX_BASE].b32.s1 = MIN_HALFWORD;
...
initialize_primitives(void)
{
    ...
    primitive(maketexstring("lineskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__line_skip);
    primitive(maketexstring("baselineskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__baseline_skip);
    primitive(maketexstring("parskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__par_skip);
    primitive(maketexstring("abovedisplayskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__above_display_skip);
    primitive(maketexstring("belowdisplayskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__below_display_skip);
    primitive(maketexstring("abovedisplayshortskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__above_display_short_skip);
    primitive(maketexstring("belowdisplayshortskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__below_display_short_skip);
    primitive(maketexstring("leftskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__left_skip);
    primitive(maketexstring("rightskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__right_skip);
    primitive(maketexstring("topskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__top_skip);
    primitive(maketexstring("splittopskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__split_top_skip);
    primitive(maketexstring("tabskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__tab_skip);
    primitive(maketexstring("spaceskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__space_skip);
    primitive(maketexstring("xspaceskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__xspace_skip);
    primitive(maketexstring("parfillskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__par_fill_skip);
    primitive(maketexstring("XeTeXlinebreakskip"), ASSIGN_GLUE, GLUE_BASE + GLUE_PAR__xetex_linebreak_skip);

    primitive(maketexstring("thinmuskip"), ASSIGN_MU_GLUE, GLUE_BASE + GLUE_PAR__thin_mu_skip);
    primitive(maketexstring("medmuskip"), ASSIGN_MU_GLUE, GLUE_BASE + GLUE_PAR__med_mu_skip);
    primitive(maketexstring("thickmuskip"), ASSIGN_MU_GLUE, GLUE_BASE + GLUE_PAR__thick_mu_skip);

    primitive(maketexstring("output"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__output_routine);
    primitive(maketexstring("everypar"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_par);
    primitive(maketexstring("everymath"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_math);
    primitive(maketexstring("everydisplay"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_display);
    primitive(maketexstring("everyhbox"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_hbox);
    primitive(maketexstring("everyvbox"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_vbox);
    primitive(maketexstring("everyjob"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_job);
    primitive(maketexstring("everycr"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_cr);
    primitive(maketexstring("errhelp"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__err_help);
    primitive(maketexstring("everyeof"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__every_eof);
    primitive(maketexstring("XeTeXinterchartoks"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__xetex_inter_char);
    primitive(maketexstring("TectonicCodaTokens"), ASSIGN_TOKS, LOCAL_BASE + LOCAL__TectonicCodaTokens);

    primitive(maketexstring("pretolerance"), ASSIGN_INT, INT_BASE + 0);
    primitive(maketexstring("tolerance"), ASSIGN_INT, INT_BASE + 1);
    primitive(maketexstring("linepenalty"), ASSIGN_INT, INT_BASE + 2);
    primitive(maketexstring("hyphenpenalty"), ASSIGN_INT, INT_BASE + 3);
    primitive(maketexstring("exhyphenpenalty"), ASSIGN_INT, INT_BASE + 4);
    primitive(maketexstring("clubpenalty"), ASSIGN_INT, INT_BASE + 5);
    primitive(maketexstring("widowpenalty"), ASSIGN_INT, INT_BASE + 6);
    primitive(maketexstring("displaywidowpenalty"), ASSIGN_INT, INT_BASE + 7);
    primitive(maketexstring("brokenpenalty"), ASSIGN_INT, INT_BASE + 8);
    primitive(maketexstring("binoppenalty"), ASSIGN_INT, INT_BASE + 9);
    primitive(maketexstring("relpenalty"), ASSIGN_INT, INT_BASE + 10);
    primitive(maketexstring("predisplaypenalty"), ASSIGN_INT, INT_BASE + 11);
    primitive(maketexstring("postdisplaypenalty"), ASSIGN_INT, INT_BASE + 12);
    primitive(maketexstring("interlinepenalty"), ASSIGN_INT, INT_BASE + 13);
    primitive(maketexstring("doublehyphendemerits"), ASSIGN_INT, INT_BASE + 14);
    primitive(maketexstring("finalhyphendemerits"), ASSIGN_INT, INT_BASE + 15);
    primitive(maketexstring("adjdemerits"), ASSIGN_INT, INT_BASE + 16);
    primitive(maketexstring("mag"), ASSIGN_INT, INT_BASE + 17);
    primitive(maketexstring("delimiterfactor"), ASSIGN_INT, INT_BASE + 18);
    primitive(maketexstring("looseness"), ASSIGN_INT, INT_BASE + 19);
    primitive(maketexstring("time"), ASSIGN_INT, INT_BASE + 20);
    primitive(maketexstring("day"), ASSIGN_INT, INT_BASE + 21);
    primitive(maketexstring("month"), ASSIGN_INT, INT_BASE + 22);
    primitive(maketexstring("year"), ASSIGN_INT, INT_BASE + 23);
    primitive(maketexstring("showboxbreadth"), ASSIGN_INT, INT_BASE + 24);
    primitive(maketexstring("showboxdepth"), ASSIGN_INT, INT_BASE + 25);
    primitive(maketexstring("hbadness"), ASSIGN_INT, INT_BASE + 26);
    primitive(maketexstring("vbadness"), ASSIGN_INT, INT_BASE + 27);
    primitive(maketexstring("pausing"), ASSIGN_INT, INT_BASE + 28);
    primitive(maketexstring("tracingonline"), ASSIGN_INT, INT_BASE + 29);
    primitive(maketexstring("tracingmacros"), ASSIGN_INT, INT_BASE + 30);
    primitive(maketexstring("tracingstats"), ASSIGN_INT, INT_BASE + 31);
    primitive(maketexstring("tracingparagraphs"), ASSIGN_INT, INT_BASE + 32);
    primitive(maketexstring("tracingpages"), ASSIGN_INT, INT_BASE + 33);
    primitive(maketexstring("tracingoutput"), ASSIGN_INT, INT_BASE + 34);
    primitive(maketexstring("tracinglostchars"), ASSIGN_INT, INT_BASE + 35);
    primitive(maketexstring("tracingcommands"), ASSIGN_INT, INT_BASE + 36);
    primitive(maketexstring("tracingrestores"), ASSIGN_INT, INT_BASE + 37);
    primitive(maketexstring("uchyph"), ASSIGN_INT, INT_BASE + 38);
    primitive(maketexstring("outputpenalty"), ASSIGN_INT, INT_BASE + 39);
    primitive(maketexstring("maxdeadcycles"), ASSIGN_INT, INT_BASE + 40);
    primitive(maketexstring("hangafter"), ASSIGN_INT, INT_BASE + 41);
    primitive(maketexstring("floatingpenalty"), ASSIGN_INT, INT_BASE + 42);
    primitive(maketexstring("globaldefs"), ASSIGN_INT, INT_BASE + 43);
    primitive(maketexstring("fam"), ASSIGN_INT, INT_BASE + 44);
    primitive(maketexstring("escapechar"), ASSIGN_INT, INT_BASE + 45);
    primitive(maketexstring("defaulthyphenchar"), ASSIGN_INT, INT_BASE + 46);
    primitive(maketexstring("defaultskewchar"), ASSIGN_INT, INT_BASE + 47);
    primitive(maketexstring("endlinechar"), ASSIGN_INT, INT_BASE + 48);
    primitive(maketexstring("newlinechar"), ASSIGN_INT, INT_BASE + 49);
    primitive(maketexstring("language"), ASSIGN_INT, INT_BASE + 50);
    primitive(maketexstring("lefthyphenmin"), ASSIGN_INT, INT_BASE + 51);
    primitive(maketexstring("righthyphenmin"), ASSIGN_INT, INT_BASE + 52);
    primitive(maketexstring("holdinginserts"), ASSIGN_INT, INT_BASE + 53);
    primitive(maketexstring("errorcontextlines"), ASSIGN_INT, INT_BASE + 54);

    primitive(maketexstring("XeTeXlinebreakpenalty"), ASSIGN_INT, INT_BASE + 69);
    primitive(maketexstring("XeTeXprotrudechars"), ASSIGN_INT, INT_BASE + 70);

    primitive(maketexstring("parindent"), ASSIGN_DIMEN, DIMEN_BASE + 0);
    primitive(maketexstring("mathsurround"), ASSIGN_DIMEN, DIMEN_BASE + 1);
    primitive(maketexstring("lineskiplimit"), ASSIGN_DIMEN, DIMEN_BASE + 2);
    primitive(maketexstring("hsize"), ASSIGN_DIMEN, DIMEN_BASE + 3);
    primitive(maketexstring("vsize"), ASSIGN_DIMEN, DIMEN_BASE + 4);
    primitive(maketexstring("maxdepth"), ASSIGN_DIMEN, DIMEN_BASE + 5);
    primitive(maketexstring("splitmaxdepth"), ASSIGN_DIMEN, DIMEN_BASE + 6);
    primitive(maketexstring("boxmaxdepth"), ASSIGN_DIMEN, DIMEN_BASE + 7);
    primitive(maketexstring("hfuzz"), ASSIGN_DIMEN, DIMEN_BASE + 8);
    primitive(maketexstring("vfuzz"), ASSIGN_DIMEN, DIMEN_BASE + 9);
    primitive(maketexstring("delimitershortfall"), ASSIGN_DIMEN, DIMEN_BASE + 10);
    primitive(maketexstring("nulldelimiterspace"), ASSIGN_DIMEN, DIMEN_BASE + 11);
    primitive(maketexstring("scriptspace"), ASSIGN_DIMEN, DIMEN_BASE + 12);
    primitive(maketexstring("predisplaysize"), ASSIGN_DIMEN, DIMEN_BASE + 13);
    primitive(maketexstring("displaywidth"), ASSIGN_DIMEN, DIMEN_BASE + 14);
    primitive(maketexstring("displayindent"), ASSIGN_DIMEN, DIMEN_BASE + 15);
    primitive(maketexstring("overfullrule"), ASSIGN_DIMEN, DIMEN_BASE + 16);
    primitive(maketexstring("hangindent"), ASSIGN_DIMEN, DIMEN_BASE + 17);
    primitive(maketexstring("hoffset"), ASSIGN_DIMEN, DIMEN_BASE + 18);
    primitive(maketexstring("voffset"), ASSIGN_DIMEN, DIMEN_BASE + 19);
    primitive(maketexstring("emergencystretch"), ASSIGN_DIMEN, DIMEN_BASE + 20);
    primitive(maketexstring("pdfpagewidth"), ASSIGN_DIMEN, DIMEN_BASE + 21);
    primitive(maketexstring("pdfpageheight"), ASSIGN_DIMEN, DIMEN_BASE + 22);

    primitive(' ', EX_SPACE, 0);
    primitive('/', ITAL_CORR, 0);
    primitive(maketexstring("accent"), ACCENT, 0);
    primitive(maketexstring("advance"), ADVANCE, 0);
    primitive(maketexstring("afterassignment"), AFTER_ASSIGNMENT, 0);
    primitive(maketexstring("aftergroup"), AFTER_GROUP, 0);
    primitive(maketexstring("begingroup"), BEGIN_GROUP, 0);
    primitive(maketexstring("char"), CHAR_NUM, 0);
    primitive(maketexstring("csname"), CS_NAME, 0);
    primitive(maketexstring("delimiter"), DELIM_NUM, 0);
    primitive(maketexstring("XeTeXdelimiter"), DELIM_NUM, 1);
    primitive(maketexstring("Udelimiter"), DELIM_NUM, 1);
    primitive(maketexstring("divide"), DIVIDE, 0);
    primitive(maketexstring("endcsname"), END_CS_NAME, 0);
    primitive(maketexstring("endgroup"), END_GROUP, 0);
    hash[FROZEN_END_GROUP].s1 = maketexstring("endgroup");
    eqtb[FROZEN_END_GROUP] = eqtb[cur_val];
    primitive(maketexstring("expandafter"), EXPAND_AFTER, 0);
    primitive(maketexstring("font"), DEF_FONT, 0);
    primitive(maketexstring("fontdimen"), ASSIGN_FONT_DIMEN, 0);
    primitive(maketexstring("halign"), HALIGN, 0);
    primitive(maketexstring("hrule"), HRULE, 0);
    primitive(maketexstring("ignorespaces"), IGNORE_SPACES, 0);
    primitive(maketexstring("insert"), INSERT, 0);
    primitive(maketexstring("mark"), MARK, 0);
    primitive(maketexstring("mathaccent"), MATH_ACCENT, 0);
    primitive(maketexstring("XeTeXmathaccent"), MATH_ACCENT, 1);
    primitive(maketexstring("Umathaccent"), MATH_ACCENT, 1);
    primitive(maketexstring("mathchar"), MATH_CHAR_NUM, 0);
    primitive(maketexstring("XeTeXmathcharnum"), MATH_CHAR_NUM, 1);
    primitive(maketexstring("Umathcharnum"), MATH_CHAR_NUM, 1);
    primitive(maketexstring("XeTeXmathchar"), MATH_CHAR_NUM, 2);
    primitive(maketexstring("Umathchar"), MATH_CHAR_NUM, 2);
    primitive(maketexstring("mathchoice"), MATH_CHOICE, 0);
    primitive(maketexstring("multiply"), MULTIPLY, 0);
    primitive(maketexstring("noalign"), NO_ALIGN, 0);
    primitive(maketexstring("noboundary"), NO_BOUNDARY, 0);
    primitive(maketexstring("noexpand"), NO_EXPAND, 0);
    primitive(maketexstring("primitive"), NO_EXPAND, 1);
    primitive(maketexstring("nonscript"), NON_SCRIPT, 0);
    primitive(maketexstring("omit"), OMIT, 0);
    primitive(maketexstring("parshape"), SET_SHAPE, LOCAL_BASE + LOCAL__par_shape);
    primitive(maketexstring("penalty"), BREAK_PENALTY, 0);
    primitive(maketexstring("prevgraf"), SET_PREV_GRAF, 0);
    primitive(maketexstring("radical"), RADICAL, 0);
    primitive(maketexstring("XeTeXradical"), RADICAL, 1);
    primitive(maketexstring("Uradical"), RADICAL, 1);
    primitive(maketexstring("read"), READ_TO_CS, 0);
    primitive(maketexstring("relax"), RELAX, TOO_BIG_USV);
    hash[FROZEN_RELAX].s1 = maketexstring("relax");
    eqtb[FROZEN_RELAX] = eqtb[cur_val];
    primitive(maketexstring("setbox"), SET_BOX, 0);
    primitive(maketexstring("the"), THE, 0);
    primitive(maketexstring("toks"), TOKS_REGISTER, 0);
    primitive(maketexstring("vadjust"), VADJUST, 0);
    primitive(maketexstring("valign"), VALIGN, 0);
    primitive(maketexstring("vcenter"), VCENTER, 0);
    primitive(maketexstring("vrule"), VRULE, 0);
    primitive(maketexstring("par"), PAR_END, TOO_BIG_USV);
    par_loc = cur_val;
    par_token = CS_TOKEN_FLAG + par_loc;

    primitive(maketexstring("input"), INPUT, 0);
    primitive(maketexstring("endinput"), INPUT, 1);

    primitive(maketexstring("topmark"), TOP_BOT_MARK, TOP_MARK_CODE);
    primitive(maketexstring("firstmark"), TOP_BOT_MARK, FIRST_MARK_CODE);
    primitive(maketexstring("botmark"), TOP_BOT_MARK, BOT_MARK_CODE);
    primitive(maketexstring("splitfirstmark"), TOP_BOT_MARK, SPLIT_FIRST_MARK_CODE);
    primitive(maketexstring("splitbotmark"), TOP_BOT_MARK, SPLIT_BOT_MARK_CODE);

    primitive(maketexstring("count"), REGISTER, 0);
    primitive(maketexstring("dimen"), REGISTER, 1);
    primitive(maketexstring("skip"), REGISTER, 2);
    primitive(maketexstring("muskip"), REGISTER, 3);

    primitive(maketexstring("spacefactor"), SET_AUX, HMODE);
    primitive(maketexstring("prevdepth"), SET_AUX, VMODE);

    primitive(maketexstring("deadcycles"), SET_PAGE_INT, 0);
    primitive(maketexstring("insertpenalties"), SET_PAGE_INT, 1);

    primitive(maketexstring("wd"), SET_BOX_DIMEN, WIDTH_OFFSET);
    primitive(maketexstring("ht"), SET_BOX_DIMEN, HEIGHT_OFFSET);
    primitive(maketexstring("dp"), SET_BOX_DIMEN, DEPTH_OFFSET);

    primitive(maketexstring("lastpenalty"), LAST_ITEM, INT_VAL);
    primitive(maketexstring("lastkern"), LAST_ITEM, DIMEN_VAL);
    primitive(maketexstring("lastskip"), LAST_ITEM, GLUE_VAL);
    primitive(maketexstring("inputlineno"), LAST_ITEM, INPUT_LINE_NO_CODE);
    primitive(maketexstring("badness"), LAST_ITEM, BADNESS_CODE);

    primitive(maketexstring("number"), CONVERT, NUMBER_CODE);
    primitive(maketexstring("romannumeral"), CONVERT, ROMAN_NUMERAL_CODE);
    primitive(maketexstring("string"), CONVERT, STRING_CODE);
    primitive(maketexstring("meaning"), CONVERT, MEANING_CODE);
    primitive(maketexstring("fontname"), CONVERT, FONT_NAME_CODE);
    primitive(maketexstring("jobname"), CONVERT, JOB_NAME_CODE);
    primitive(maketexstring("leftmarginkern"), CONVERT, LEFT_MARGIN_KERN_CODE);
    primitive(maketexstring("rightmarginkern"), CONVERT, RIGHT_MARGIN_KERN_CODE);
    primitive(maketexstring("Uchar"), CONVERT, XETEX_UCHAR_CODE);
    primitive(maketexstring("Ucharcat"), CONVERT, XETEX_UCHARCAT_CODE);

    primitive(maketexstring("if"), IF_TEST, IF_CHAR_CODE);
    primitive(maketexstring("ifcat"), IF_TEST, IF_CAT_CODE);
    primitive(maketexstring("ifnum"), IF_TEST, IF_INT_CODE);
    primitive(maketexstring("ifdim"), IF_TEST, IF_DIM_CODE);
    primitive(maketexstring("ifodd"), IF_TEST, IF_ODD_CODE);
    primitive(maketexstring("ifvmode"), IF_TEST, IF_VMODE_CODE);
    primitive(maketexstring("ifhmode"), IF_TEST, IF_HMODE_CODE);
    primitive(maketexstring("ifmmode"), IF_TEST, IF_MMODE_CODE);
    primitive(maketexstring("ifinner"), IF_TEST, IF_INNER_CODE);
    primitive(maketexstring("ifvoid"), IF_TEST, IF_VOID_CODE);
    primitive(maketexstring("ifhbox"), IF_TEST, IF_HBOX_CODE);
    primitive(maketexstring("ifvbox"), IF_TEST, IF_VBOX_CODE);
    primitive(maketexstring("ifx"), IF_TEST, IFX_CODE);
    primitive(maketexstring("ifeof"), IF_TEST, IF_EOF_CODE);
    primitive(maketexstring("iftrue"), IF_TEST, IF_TRUE_CODE);
    primitive(maketexstring("iffalse"), IF_TEST, IF_FALSE_CODE);
    primitive(maketexstring("ifcase"), IF_TEST, IF_CASE_CODE);
    primitive(maketexstring("ifprimitive"), IF_TEST, IF_PRIMITIVE_CODE);

    primitive(maketexstring("fi"), FI_OR_ELSE, FI_CODE);
    hash[FROZEN_FI].s1 = maketexstring("fi");
    eqtb[FROZEN_FI] = eqtb[cur_val];
    primitive(maketexstring("or"), FI_OR_ELSE, OR_CODE);
    primitive(maketexstring("else"), FI_OR_ELSE, ELSE_CODE);

    primitive(maketexstring("nullfont"), SET_FONT, FONT_BASE);
    hash[FROZEN_NULL_FONT].s1 = maketexstring("nullfont");
    eqtb[FROZEN_NULL_FONT] = eqtb[cur_val];

    primitive(maketexstring("span"), TAB_MARK, SPAN_CODE);
    primitive(maketexstring("cr"), CAR_RET, CR_CODE);
    hash[FROZEN_CR].s1 = maketexstring("cr");
    eqtb[FROZEN_CR] = eqtb[cur_val];
    primitive(maketexstring("crcr"), CAR_RET, CR_CR_CODE);
    ...
    primitive(maketexstring("pagegoal"), SET_PAGE_DIMEN, 0);
    primitive(maketexstring("pagetotal"), SET_PAGE_DIMEN, 1);
    primitive(maketexstring("pagestretch"), SET_PAGE_DIMEN, 2);
    primitive(maketexstring("pagefilstretch"), SET_PAGE_DIMEN, 3);
    primitive(maketexstring("pagefillstretch"), SET_PAGE_DIMEN, 4);
    primitive(maketexstring("pagefilllstretch"), SET_PAGE_DIMEN, 5);
    primitive(maketexstring("pageshrink"), SET_PAGE_DIMEN, 6);
    primitive(maketexstring("pagedepth"), SET_PAGE_DIMEN, 7);

    primitive(maketexstring("end"), STOP, 0);
    primitive(maketexstring("dump"), STOP, 1);

    primitive(maketexstring("hskip"), HSKIP, SKIP_CODE);
    primitive(maketexstring("hfil"), HSKIP, FIL_CODE);
    primitive(maketexstring("hfill"), HSKIP, FILL_CODE);
    primitive(maketexstring("hss"), HSKIP, SS_CODE);
    primitive(maketexstring("hfilneg"), HSKIP, FIL_NEG_CODE);
    primitive(maketexstring("vskip"), VSKIP, SKIP_CODE);
    primitive(maketexstring("vfil"), VSKIP, FIL_CODE);
    primitive(maketexstring("vfill"), VSKIP, FILL_CODE);
    primitive(maketexstring("vss"), VSKIP, SS_CODE);
    primitive(maketexstring("vfilneg"), VSKIP, FIL_NEG_CODE);
    primitive(maketexstring("mskip"), MSKIP, MSKIP_CODE);

    primitive(maketexstring("kern"), KERN, EXPLICIT);
    primitive(maketexstring("mkern"), MKERN, MU_GLUE);
    primitive(maketexstring("moveleft"), HMOVE, 1);
    primitive(maketexstring("moveright"), HMOVE, 0);
    primitive(maketexstring("raise"), VMOVE, 1);
    primitive(maketexstring("lower"), VMOVE, 0);

    primitive(maketexstring("box"), MAKE_BOX, BOX_CODE);
    primitive(maketexstring("copy"), MAKE_BOX, COPY_CODE);
    primitive(maketexstring("lastbox"), MAKE_BOX, LAST_BOX_CODE);
    primitive(maketexstring("vsplit"), MAKE_BOX, VSPLIT_CODE);
    primitive(maketexstring("vtop"), MAKE_BOX, VTOP_CODE);
    primitive(maketexstring("vbox"), MAKE_BOX, VTOP_CODE + 1);
    primitive(maketexstring("hbox"), MAKE_BOX, VTOP_CODE + 104);

    primitive(maketexstring("shipout"), LEADER_SHIP, A_LEADERS - 1);
    primitive(maketexstring("leaders"), LEADER_SHIP, A_LEADERS);
    primitive(maketexstring("cleaders"), LEADER_SHIP, C_LEADERS);
    primitive(maketexstring("xleaders"), LEADER_SHIP, X_LEADERS);

    primitive(maketexstring("indent"), START_PAR, 1);
    primitive(maketexstring("noindent"), START_PAR, 0);
    primitive(maketexstring("unpenalty"), REMOVE_ITEM, PENALTY_NODE);
    primitive(maketexstring("unkern"), REMOVE_ITEM, KERN_NODE);
    primitive(maketexstring("unskip"), REMOVE_ITEM, GLUE_NODE);
    primitive(maketexstring("unhbox"), UN_HBOX, BOX_CODE);
    primitive(maketexstring("unhcopy"), UN_HBOX, COPY_CODE);
    primitive(maketexstring("unvbox"), UN_VBOX, BOX_CODE);
    primitive(maketexstring("unvcopy"), UN_VBOX, COPY_CODE);

    primitive('-', DISCRETIONARY, 1);
    primitive(maketexstring("discretionary"), DISCRETIONARY, 0);

    primitive(maketexstring("eqno"), EQ_NO, 0);
    primitive(maketexstring("leqno"), EQ_NO, 1);

    primitive(maketexstring("mathord"), MATH_COMP, ORD_NOAD);
    primitive(maketexstring("mathop"), MATH_COMP, OP_NOAD);
    primitive(maketexstring("mathbin"), MATH_COMP, BIN_NOAD);
    primitive(maketexstring("mathrel"), MATH_COMP, REL_NOAD);
    primitive(maketexstring("mathopen"), MATH_COMP, OPEN_NOAD);
    primitive(maketexstring("mathclose"), MATH_COMP, CLOSE_NOAD);
    primitive(maketexstring("mathpunct"), MATH_COMP, PUNCT_NOAD);
    primitive(maketexstring("mathinner"), MATH_COMP, INNER_NOAD);
    primitive(maketexstring("underline"), MATH_COMP, UNDER_NOAD);
    primitive(maketexstring("overline"), MATH_COMP, OVER_NOAD);

    primitive(maketexstring("displaylimits"), LIMIT_SWITCH, NORMAL);
    primitive(maketexstring("limits"), LIMIT_SWITCH, LIMITS);
    primitive(maketexstring("nolimits"), LIMIT_SWITCH, NO_LIMITS);

    primitive(maketexstring("displaystyle"), MATH_STYLE, DISPLAY_STYLE);
    primitive(maketexstring("textstyle"), MATH_STYLE, TEXT_STYLE);
    primitive(maketexstring("scriptstyle"), MATH_STYLE, SCRIPT_STYLE);
    primitive(maketexstring("scriptscriptstyle"), MATH_STYLE, SCRIPT_SCRIPT_STYLE);

    primitive(maketexstring("above"), ABOVE, ABOVE_CODE);
    primitive(maketexstring("over"), ABOVE, OVER_CODE);
    primitive(maketexstring("atop"), ABOVE, ATOP_CODE);
    primitive(maketexstring("abovewithdelims"), ABOVE, DELIMITED_CODE + 0);
    primitive(maketexstring("overwithdelims"), ABOVE, DELIMITED_CODE + 1);
    primitive(maketexstring("atopwithdelims"), ABOVE, DELIMITED_CODE + 2);

    primitive(maketexstring("left"), LEFT_RIGHT, LEFT_NOAD);
    primitive(maketexstring("right"), LEFT_RIGHT, RIGHT_NOAD);
    hash[FROZEN_RIGHT].s1 = maketexstring("right");
    eqtb[FROZEN_RIGHT] = eqtb[cur_val];

    primitive(maketexstring("long"), PREFIX, 1);
    primitive(maketexstring("outer"), PREFIX, 2);
    primitive(maketexstring("global"), PREFIX, 4);
    primitive(maketexstring("def"), DEF, 0);
    primitive(maketexstring("gdef"), DEF, 1);
    primitive(maketexstring("edef"), DEF, 2);
    primitive(maketexstring("xdef"), DEF, 3);
    primitive(maketexstring("let"), LET, NORMAL);
    primitive(maketexstring("futurelet"), LET, NORMAL + 1);

    primitive(maketexstring("chardef"), SHORTHAND_DEF, CHAR_DEF_CODE);
    primitive(maketexstring("mathchardef"), SHORTHAND_DEF, MATH_CHAR_DEF_CODE);
    primitive(maketexstring("XeTeXmathcharnumdef"), SHORTHAND_DEF, XETEX_MATH_CHAR_NUM_DEF_CODE);
    primitive(maketexstring("Umathcharnumdef"), SHORTHAND_DEF, XETEX_MATH_CHAR_NUM_DEF_CODE);
    primitive(maketexstring("XeTeXmathchardef"), SHORTHAND_DEF, XETEX_MATH_CHAR_DEF_CODE);
    primitive(maketexstring("Umathchardef"), SHORTHAND_DEF, XETEX_MATH_CHAR_DEF_CODE);
    primitive(maketexstring("countdef"), SHORTHAND_DEF, COUNT_DEF_CODE);
    primitive(maketexstring("dimendef"), SHORTHAND_DEF, DIMEN_DEF_CODE);
    primitive(maketexstring("skipdef"), SHORTHAND_DEF, SKIP_DEF_CODE);
    primitive(maketexstring("muskipdef"), SHORTHAND_DEF, MU_SKIP_DEF_CODE);
    primitive(maketexstring("toksdef"), SHORTHAND_DEF, TOKS_DEF_CODE);

    primitive(maketexstring("catcode"), DEF_CODE, CAT_CODE_BASE);
    primitive(maketexstring("mathcode"), DEF_CODE, MATH_CODE_BASE);
    primitive(maketexstring("XeTeXmathcodenum"), XETEX_DEF_CODE, MATH_CODE_BASE);
    primitive(maketexstring("Umathcodenum"), XETEX_DEF_CODE, MATH_CODE_BASE);
    primitive(maketexstring("XeTeXmathcode"), XETEX_DEF_CODE, MATH_CODE_BASE + 1);
    primitive(maketexstring("Umathcode"), XETEX_DEF_CODE, MATH_CODE_BASE + 1);
    primitive(maketexstring("lccode"), DEF_CODE, LC_CODE_BASE);
    primitive(maketexstring("uccode"), DEF_CODE, UC_CODE_BASE);
    primitive(maketexstring("sfcode"), DEF_CODE, SF_CODE_BASE);
    primitive(maketexstring("XeTeXcharclass"), XETEX_DEF_CODE, SF_CODE_BASE);
    primitive(maketexstring("delcode"), DEF_CODE, DEL_CODE_BASE);
    primitive(maketexstring("XeTeXdelcodenum"), XETEX_DEF_CODE, DEL_CODE_BASE);
    primitive(maketexstring("Udelcodenum"), XETEX_DEF_CODE, DEL_CODE_BASE);
    primitive(maketexstring("XeTeXdelcode"), XETEX_DEF_CODE, DEL_CODE_BASE + 1);
    primitive(maketexstring("Udelcode"), XETEX_DEF_CODE, DEL_CODE_BASE + 1);

    primitive(maketexstring("textfont"), DEF_FAMILY, MATH_FONT_BASE + TEXT_SIZE);
    primitive(maketexstring("scriptfont"), DEF_FAMILY, MATH_FONT_BASE + SCRIPT_SIZE);
    primitive(maketexstring("scriptscriptfont"), DEF_FAMILY, MATH_FONT_BASE + SCRIPT_SCRIPT_SIZE);

    primitive(maketexstring("hyphenation"), HYPH_DATA, 0);
    primitive(maketexstring("patterns"), HYPH_DATA, 1);

    primitive(maketexstring("hyphenchar"), ASSIGN_FONT_INT, 0);
    primitive(maketexstring("skewchar"), ASSIGN_FONT_INT, 1);
    primitive(maketexstring("lpcode"), ASSIGN_FONT_INT, 2);
    primitive(maketexstring("rpcode"), ASSIGN_FONT_INT, 3);

    primitive(maketexstring("batchmode"), SET_INTERACTION, BATCH_MODE);
    primitive(maketexstring("nonstopmode"), SET_INTERACTION, NONSTOP_MODE);
    primitive(maketexstring("scrollmode"), SET_INTERACTION, SCROLL_MODE);
    primitive(maketexstring("errorstopmode"), SET_INTERACTION, ERROR_STOP_MODE);

    primitive(maketexstring("openin"), IN_STREAM, 1);
    primitive(maketexstring("closein"), IN_STREAM, 0);
    primitive(maketexstring("message"), MESSAGE, 0);
    primitive(maketexstring("errmessage"), MESSAGE, 1);
    primitive(maketexstring("lowercase"), CASE_SHIFT, LC_CODE_BASE);
    primitive(maketexstring("uppercase"), CASE_SHIFT, UC_CODE_BASE);

    primitive(maketexstring("show"), XRAY, SHOW_CODE);
    primitive(maketexstring("showbox"), XRAY, SHOW_BOX_CODE);
    primitive(maketexstring("showthe"), XRAY, SHOW_THE_CODE);
    primitive(maketexstring("showlists"), XRAY, SHOW_LISTS);

    primitive(maketexstring("openout"), EXTENSION, OPEN_NODE);
    primitive(maketexstring("write"), EXTENSION, WRITE_NODE);
    write_loc = cur_val;
    primitive(maketexstring("closeout"), EXTENSION, CLOSE_NODE);
    primitive(maketexstring("special"), EXTENSION, SPECIAL_NODE);
    hash[FROZEN_SPECIAL].s1 = maketexstring("special");
    eqtb[FROZEN_SPECIAL] = eqtb[cur_val];
    primitive(maketexstring("immediate"), EXTENSION, IMMEDIATE_CODE);
    primitive(maketexstring("setlanguage"), EXTENSION, SET_LANGUAGE_CODE);

    primitive(maketexstring("synctex"), ASSIGN_INT, INT_BASE + INT_PAR__synctex);
...
tt_run_engine(char *dump_name, char *input_file_name)
{
    ...
    if (in_initex_mode) {
        ...
        primitive(maketexstring("XeTeXpicfile"), EXTENSION, PIC_FILE_CODE);
        primitive(maketexstring("XeTeXpdffile"), EXTENSION, PDF_FILE_CODE);
        primitive(maketexstring("XeTeXglyph"), EXTENSION, GLYPH_CODE);
        primitive(maketexstring("XeTeXlinebreaklocale"), EXTENSION, XETEX_LINEBREAK_LOCALE_EXTENSION_CODE);
        primitive(maketexstring("pdfsavepos"), EXTENSION, PDFTEX_FIRST_EXTENSION_CODE + 0);

        primitive(maketexstring("lastnodetype"), LAST_ITEM, LAST_NODE_TYPE_CODE);
        primitive(maketexstring("eTeXversion"), LAST_ITEM, ETEX_VERSION_CODE);

        primitive(maketexstring("eTeXrevision"), CONVERT, ETEX_REVISION_CODE);

        primitive(maketexstring("XeTeXversion"), LAST_ITEM, XETEX_VERSION_CODE);

        primitive(maketexstring("XeTeXrevision"), CONVERT, XETEX_REVISION_CODE);

        primitive(maketexstring("XeTeXcountglyphs"), LAST_ITEM, XETEX_COUNT_GLYPHS_CODE);
        primitive(maketexstring("XeTeXcountvariations"), LAST_ITEM, XETEX_COUNT_VARIATIONS_CODE);
        primitive(maketexstring("XeTeXvariation"), LAST_ITEM, XETEX_VARIATION_CODE);
        primitive(maketexstring("XeTeXfindvariationbyname"), LAST_ITEM, XETEX_FIND_VARIATION_BY_NAME_CODE);
        primitive(maketexstring("XeTeXvariationmin"), LAST_ITEM, XETEX_VARIATION_MIN_CODE);
        primitive(maketexstring("XeTeXvariationmax"), LAST_ITEM, XETEX_VARIATION_MAX_CODE);
        primitive(maketexstring("XeTeXvariationdefault"), LAST_ITEM, XETEX_VARIATION_DEFAULT_CODE);
        primitive(maketexstring("XeTeXcountfeatures"), LAST_ITEM, XETEX_COUNT_FEATURES_CODE);
        primitive(maketexstring("XeTeXfeaturecode"), LAST_ITEM, XETEX_FEATURE_CODE_CODE);
        primitive(maketexstring("XeTeXfindfeaturebyname"), LAST_ITEM, XETEX_FIND_FEATURE_BY_NAME_CODE);
        primitive(maketexstring("XeTeXisexclusivefeature"), LAST_ITEM, XETEX_IS_EXCLUSIVE_FEATURE_CODE);
        primitive(maketexstring("XeTeXcountselectors"), LAST_ITEM, XETEX_COUNT_SELECTORS_CODE);
        primitive(maketexstring("XeTeXselectorcode"), LAST_ITEM, XETEX_SELECTOR_CODE_CODE);
        primitive(maketexstring("XeTeXfindselectorbyname"), LAST_ITEM, XETEX_FIND_SELECTOR_BY_NAME_CODE);
        primitive(maketexstring("XeTeXisdefaultselector"), LAST_ITEM, XETEX_IS_DEFAULT_SELECTOR_CODE);

        primitive(maketexstring("XeTeXvariationname"), CONVERT, XETEX_VARIATION_NAME_CODE);
        primitive(maketexstring("XeTeXfeaturename"), CONVERT, XeTeX_feature_name);
        primitive(maketexstring("XeTeXselectorname"), CONVERT, XeTeX_selector_name);

        primitive(maketexstring("XeTeXOTcountscripts"), LAST_ITEM, XETEX_OT_COUNT_SCRIPTS_CODE);
        primitive(maketexstring("XeTeXOTcountlanguages"), LAST_ITEM, XETEX_OT_COUNT_LANGUAGES_CODE);
        primitive(maketexstring("XeTeXOTcountfeatures"), LAST_ITEM, XETEX_OT_COUNT_FEATURES_CODE);
        primitive(maketexstring("XeTeXOTscripttag"), LAST_ITEM, XETEX_OT_SCRIPT_CODE);
        primitive(maketexstring("XeTeXOTlanguagetag"), LAST_ITEM, XETEX_OT_LANGUAGE_CODE);
        primitive(maketexstring("XeTeXOTfeaturetag"), LAST_ITEM, XETEX_OT_FEATURE_CODE);
        primitive(maketexstring("XeTeXcharglyph"), LAST_ITEM, XETEX_MAP_CHAR_TO_GLYPH_CODE);
        primitive(maketexstring("XeTeXglyphindex"), LAST_ITEM, XETEX_GLYPH_INDEX_CODE);
        primitive(maketexstring("XeTeXglyphbounds"), LAST_ITEM, XETEX_GLYPH_BOUNDS_CODE);

        primitive(maketexstring("XeTeXglyphname"), CONVERT, XETEX_GLYPH_NAME_CODE);

        primitive(maketexstring("XeTeXfonttype"), LAST_ITEM, XETEX_FONT_TYPE_CODE);
        primitive(maketexstring("XeTeXfirstfontchar"), LAST_ITEM, XETEX_FIRST_CHAR_CODE);
        primitive(maketexstring("XeTeXlastfontchar"), LAST_ITEM, XETEX_LAST_CHAR_CODE);
        primitive(maketexstring("pdflastxpos"), LAST_ITEM, PDF_LAST_X_POS_CODE);
        primitive(maketexstring("pdflastypos"), LAST_ITEM, PDF_LAST_Y_POS_CODE);

        primitive(maketexstring("strcmp"), CONVERT, PDF_STRCMP_CODE);
        primitive(maketexstring("mdfivesum"), CONVERT, PDF_MDFIVE_SUM_CODE);
        primitive(maketexstring("pdfmdfivesum"), CONVERT, PDF_MDFIVE_SUM_CODE);

        primitive(maketexstring("shellescape"), LAST_ITEM, PDF_SHELL_ESCAPE_CODE);
        primitive(maketexstring("XeTeXpdfpagecount"), LAST_ITEM, XETEX_PDF_PAGE_COUNT_CODE);


        primitive(maketexstring("tracingassigns"), ASSIGN_INT, INT_BASE + INT_PAR__tracing_assigns);
        primitive(maketexstring("tracinggroups"), ASSIGN_INT, INT_BASE + INT_PAR__tracing_groups);
        primitive(maketexstring("tracingifs"), ASSIGN_INT, INT_BASE + INT_PAR__tracing_ifs);
        primitive(maketexstring("tracingscantokens"), ASSIGN_INT, INT_BASE + INT_PAR__tracing_scan_tokens);
        primitive(maketexstring("tracingnesting"), ASSIGN_INT, INT_BASE + INT_PAR__tracing_nesting);
        primitive(maketexstring("predisplaydirection"), ASSIGN_INT, INT_BASE + INT_PAR__pre_display_correction);
        primitive(maketexstring("lastlinefit"), ASSIGN_INT, INT_BASE + INT_PAR__last_line_fit);
        primitive(maketexstring("savingvdiscards"), ASSIGN_INT, INT_BASE + INT_PAR__saving_vdiscards);
        primitive(maketexstring("savinghyphcodes"), ASSIGN_INT, INT_BASE + INT_PAR__saving_hyphs);

        primitive(maketexstring("currentgrouplevel"), LAST_ITEM, CURRENT_GROUP_LEVEL_CODE);
        primitive(maketexstring("currentgrouptype"), LAST_ITEM, CURRENT_GROUP_TYPE_CODE);
        primitive(maketexstring("currentiflevel"), LAST_ITEM, CURRENT_IF_LEVEL_CODE);
        primitive(maketexstring("currentiftype"), LAST_ITEM, CURRENT_IF_TYPE_CODE);
        primitive(maketexstring("currentifbranch"), LAST_ITEM, CURRENT_IF_BRANCH_CODE);
        primitive(maketexstring("fontcharwd"), LAST_ITEM, FONT_CHAR_WD_CODE);
        primitive(maketexstring("fontcharht"), LAST_ITEM, FONT_CHAR_HT_CODE);
        primitive(maketexstring("fontchardp"), LAST_ITEM, FONT_CHAR_DP_CODE);
        primitive(maketexstring("fontcharic"), LAST_ITEM, FONT_CHAR_IC_CODE);
        primitive(maketexstring("parshapelength"), LAST_ITEM, PAR_SHAPE_LENGTH_CODE);
        primitive(maketexstring("parshapeindent"), LAST_ITEM, PAR_SHAPE_INDENT_CODE);
        primitive(maketexstring("parshapedimen"), LAST_ITEM, PAR_SHAPE_DIMEN_CODE);

        primitive(maketexstring("showgroups"), XRAY, SHOW_GROUPS);
        primitive(maketexstring("showtokens"), XRAY, SHOW_TOKENS);

        primitive(maketexstring("unexpanded"), THE, 1);
        primitive(maketexstring("detokenize"), THE, SHOW_TOKENS);

        primitive(maketexstring("showifs"), XRAY, SHOW_IFS);

        primitive(maketexstring("interactionmode"), SET_PAGE_INT, 2);

        primitive(maketexstring("middle"), LEFT_RIGHT, 1);

        primitive(maketexstring("suppressfontnotfounderror"), ASSIGN_INT, INT_BASE + INT_PAR__suppress_fontnotfound_error);

        primitive(maketexstring("TeXXeTstate"), ASSIGN_INT, INT_BASE + INT_PAR__texxet);
        primitive(maketexstring("XeTeXupwardsmode"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_upwards);
        primitive(maketexstring("XeTeXuseglyphmetrics"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_use_glyph_metrics);
        primitive(maketexstring("XeTeXinterchartokenstate"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_inter_char_tokens);
        primitive(maketexstring("XeTeXdashbreakstate"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_dash_break);
        primitive(maketexstring("XeTeXinputnormalization"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_input_normalization);
        primitive(maketexstring("XeTeXtracingfonts"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_tracing_fonts);
        primitive(maketexstring("XeTeXinterwordspaceshaping"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_interword_space_shaping);
        primitive(maketexstring("XeTeXgenerateactualtext"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_generate_actual_text);
        primitive(maketexstring("XeTeXhyphenatablelength"), ASSIGN_INT, INT_BASE + INT_PAR__xetex_hyphenatable_length);
        primitive(maketexstring("pdfoutput"), ASSIGN_INT, INT_BASE + INT_PAR__pdfoutput);

        primitive(maketexstring("XeTeXinputencoding"), EXTENSION, XETEX_INPUT_ENCODING_EXTENSION_CODE);
        primitive(maketexstring("XeTeXdefaultencoding"), EXTENSION, XETEX_DEFAULT_ENCODING_EXTENSION_CODE);

        primitive(maketexstring("beginL"), VALIGN, BEGIN_L_CODE);
        primitive(maketexstring("endL"), VALIGN, END_L_CODE);
        primitive(maketexstring("beginR"), VALIGN, BEGIN_R_CODE);
        primitive(maketexstring("endR"), VALIGN, END_R_CODE);

        primitive(maketexstring("scantokens"), INPUT, 2);
        primitive(maketexstring("readline"), READ_TO_CS, 1);
        primitive(maketexstring("unless"), EXPAND_AFTER, 1);

        primitive(maketexstring("ifdefined"), IF_TEST, IF_DEF_CODE);
        primitive(maketexstring("ifcsname"), IF_TEST, IF_CS_CODE);
        primitive(maketexstring("iffontchar"), IF_TEST, IF_FONT_CHAR_CODE);
        primitive(maketexstring("ifincsname"), IF_TEST, IF_IN_CSNAME_CODE);

        primitive(maketexstring("protected"), PREFIX, 8);

        primitive(maketexstring("numexpr"), LAST_ITEM, ETEX_EXPR + 0);
        primitive(maketexstring("dimexpr"), LAST_ITEM, ETEX_EXPR + 1);
        primitive(maketexstring("glueexpr"), LAST_ITEM, ETEX_EXPR + 2);
        primitive(maketexstring("muexpr"), LAST_ITEM, ETEX_EXPR + 3);
        primitive(maketexstring("gluestretchorder"), LAST_ITEM, GLUE_STRETCH_ORDER_CODE);
        primitive(maketexstring("glueshrinkorder"), LAST_ITEM, GLUE_SHRINK_ORDER_CODE);
        primitive(maketexstring("gluestretch"), LAST_ITEM, GLUE_STRETCH_CODE);
        primitive(maketexstring("glueshrink"), LAST_ITEM, GLUE_SHRINK_CODE);
        primitive(maketexstring("mutoglue"), LAST_ITEM, MU_TO_GLUE_CODE);
        primitive(maketexstring("gluetomu"), LAST_ITEM, GLUE_TO_MU_CODE);

        primitive(maketexstring("marks"), MARK, 5);
        primitive(maketexstring("topmarks"), TOP_BOT_MARK, TOP_MARK_CODE + 5);
        primitive(maketexstring("firstmarks"), TOP_BOT_MARK, FIRST_MARK_CODE + 5);
        primitive(maketexstring("botmarks"), TOP_BOT_MARK, BOT_MARK_CODE + 5);
        primitive(maketexstring("splitfirstmarks"), TOP_BOT_MARK, SPLIT_FIRST_MARK_CODE + 5);
        primitive(maketexstring("splitbotmarks"), TOP_BOT_MARK, SPLIT_BOT_MARK_CODE + 5);

        primitive(maketexstring("pagediscards"), UN_VBOX, LAST_BOX_CODE);
        primitive(maketexstring("splitdiscards"), UN_VBOX, VSPLIT_CODE);

        primitive(maketexstring("interlinepenalties"), SET_SHAPE, INTER_LINE_PENALTIES_LOC);
        primitive(maketexstring("clubpenalties"), SET_SHAPE, CLUB_PENALTIES_LOC);
        primitive(maketexstring("widowpenalties"), SET_SHAPE, WIDOW_PENALTIES_LOC);
        primitive(maketexstring("displaywidowpenalties"), SET_SHAPE, DISPLAY_WIDOW_PENALTIES_LOC);
