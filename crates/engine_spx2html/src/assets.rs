...
impl Assets {
    ...
    pub fn try_handle_special(&mut self, special: Special) -> bool {
        match special {
                ...
                let (src_tex_path, dest_path) = match spec.split_once(' ') {
                    ...
                    None => {
                        warn!(
                            ...
                            "ignoring malformatted special `{}`", special
                        );
                        return false;
                    }
                };
            ...
            Special::ProvideSpecial(spec) => {
                ...
                match kind {
                    ...
                    _ => {
                        warn!(
                            tectonic_log_source = "spx2html",
                            "ignoring unsupported special `{}`", special
                        );
                        false
                    }
                }
...
fn emit_copied_file(src_tex_path: &str, dest_path: &str, common: &mut Common) -> Result<()> {
    let mut ih = atry!(
        common.hooks.io().input_open_name(src_tex_path).must_exist();
