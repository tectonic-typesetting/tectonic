...
pub struct tfm_font {
    pub level: int32_t,
    pub wlenfile: uint32_t,
    pub wlenheader: uint32_t,
    pub bc: uint32_t,
    pub ec: uint32_t,
    pub nwidths: uint32_t,
    pub nheights: uint32_t,
    pub ndepths: uint32_t,
    pub nitcor: uint32_t,
    pub nlig: uint32_t,
    pub nkern: uint32_t,
    pub nextens: uint32_t,
    pub nfonparm: uint32_t,
    pub fontdir: uint32_t,
    pub nco: uint32_t,
    pub ncw: uint32_t,
    pub npc: uint32_t,
    pub header: *mut fixword,
    pub char_info: *mut uint32_t,
    pub width_index: *mut libc::c_ushort,
    pub height_index: *mut libc::c_uchar,
    pub depth_index: *mut libc::c_uchar,
...
unsafe extern "C" fn tfm_font_init(mut tfm: *mut tfm_font) {
    ...
    (*tfm).level = 0i32;
    (*tfm).fontdir = 0i32 as uint32_t;
    (*tfm).npc = 0i32 as uint32_t;
    (*tfm).ncw = (*tfm).npc;
    (*tfm).nco = (*tfm).ncw;
    (*tfm).char_info = 0 as *mut uint32_t;
    (*tfm).width_index = 0 as *mut libc::c_ushort;
    (*tfm).height_index = 0 as *mut libc::c_uchar;
    (*tfm).depth_index = 0 as *mut libc::c_uchar;
...
unsafe extern "C" fn tfm_font_clear(mut tfm: *mut tfm_font) {
    if !tfm.is_null() {
        (*tfm).header =
            mfree((*tfm).header as *mut libc::c_void) as *mut fixword;
        (*tfm).char_info =
            mfree((*tfm).char_info as *mut libc::c_void) as *mut uint32_t;
        (*tfm).width =
            mfree((*tfm).width as *mut libc::c_void) as *mut fixword;
        (*tfm).height =
            mfree((*tfm).height as *mut libc::c_void) as *mut fixword;
        (*tfm).depth =
            mfree((*tfm).depth as *mut libc::c_void) as *mut fixword;
        (*tfm).width_index =
            mfree((*tfm).width_index as *mut libc::c_void) as
                *mut libc::c_ushort;
        (*tfm).height_index =
            mfree((*tfm).height_index as *mut libc::c_void) as
                *mut libc::c_uchar;
        (*tfm).depth_index =
            mfree((*tfm).depth_index as *mut libc::c_void) as
                *mut libc::c_uchar
    };
}
unsafe extern "C" fn release_char_map(mut map: *mut char_map) {
    (*map).indices =
        mfree((*map).indices as *mut libc::c_void) as *mut libc::c_ushort;
...
unsafe extern "C" fn lookup_char(mut map: *const char_map,
                                 mut charcode: libc::c_int) -> libc::c_int {
    if charcode >= (*map).coverage.first_char &&
           charcode <= (*map).coverage.first_char + (*map).coverage.num_chars
       {
        return *(*map).indices.offset((charcode - (*map).coverage.first_char)
                                          as isize) as libc::c_int
    } else { return -1i32 };
}
unsafe extern "C" fn lookup_range(mut map: *const range_map,
                                  mut charcode: libc::c_int) -> libc::c_int {
    let mut idx: libc::c_int = 0;
    idx = (*map).num_coverages as libc::c_int - 1i32;
    while idx >= 0i32 &&
              charcode >= (*(*map).coverages.offset(idx as isize)).first_char
          {
        if charcode <=
...
unsafe extern "C" fn fm_clear(mut fm: *mut font_metric) {
    if !fm.is_null() {
        ...
        match (*fm).charmap.type_0 {
            1 => { release_char_map((*fm).charmap.data as *mut char_map); }
            2 => { release_range_map((*fm).charmap.data as *mut range_map); }
            _ => { }
        }
    };
}
static mut fms: *mut font_metric =
    0 as *const font_metric as *mut font_metric;
...
unsafe extern "C" fn fms_need(mut n: libc::c_uint) {
    if n > max_fms {
        max_fms =
            if max_fms.wrapping_add(16i32 as libc::c_uint) > n {
                max_fms.wrapping_add(16i32 as libc::c_uint)
            } else { n };
        fms =
            renew(fms as *mut libc::c_void,
                  (max_fms as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<font_metric>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut font_metric
    };
}
#[no_mangle]
...
unsafe extern "C" fn fread_fwords(mut words: *mut fixword,
                                  mut nmemb: uint32_t,
...
unsafe extern "C" fn fread_uquads(mut quads: *mut uint32_t,
                                  mut nmemb: uint32_t,
                                  mut handle: rust_input_handle_t)
 -> libc::c_int {
    let mut i: uint32_t = 0;
 ...
 * TFM and JFM
 */
unsafe extern "C" fn tfm_check_size(mut tfm: *mut tfm_font,
                                    mut tfm_file_size: off_t) {
    let mut expected_size: uint32_t = 6i32 as uint32_t;
    ...
    */
    if tfm_file_size < (*tfm).wlenfile as int64_t * 4i32 as libc::c_long {
        _tt_abort(b"Can\'t proceed...\x00" as *const u8 as
                      *const libc::c_char);
    }
    expected_size =
        (expected_size as
             libc::c_uint).wrapping_add((*tfm).ec.wrapping_sub((*tfm).bc).wrapping_add(1i32
                                                                                           as
                                                                                           libc::c_uint))
            as uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).wlenheader) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nwidths) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nheights) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).ndepths) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nitcor) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nlig) as uint32_t
            as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nkern) as uint32_t
            as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nextens) as
            uint32_t as uint32_t;
    expected_size =
        (expected_size as libc::c_uint).wrapping_add((*tfm).nfonparm) as
            uint32_t as uint32_t;
    if expected_size != (*tfm).wlenfile {
        dpx_warning(b"TFM file size is expected to be %ld bytes but it says it is %ldbytes!\x00"
        ...
        if tfm_file_size > expected_size as int64_t * 4i32 as libc::c_long {
            dpx_warning(b"Proceeding nervously...\x00" as *const u8 as
                            *const libc::c_char);
        } else {
            _tt_abort(b"Can\'t proceed...\x00" as *const u8 as
                          *const libc::c_char);
...
unsafe extern "C" fn tfm_get_sizes(mut tfm_handle: rust_input_handle_t,
                                   mut tfm_file_size: off_t,
                                   mut tfm: *mut tfm_font) {
    (*tfm).wlenfile = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).wlenheader = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).bc = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).ec = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    if (*tfm).ec < (*tfm).bc {
        _tt_abort(b"TFM file error: ec(%u) < bc(%u) ???\x00" as *const u8 as
                      *const libc::c_char, (*tfm).ec, (*tfm).bc);
    }
    (*tfm).nwidths = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nheights = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).ndepths = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nitcor = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nlig = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nkern = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nextens = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    (*tfm).nfonparm = tt_get_unsigned_pair(tfm_handle) as uint32_t;
    tfm_check_size(tfm, tfm_file_size);
}
unsafe extern "C" fn tfm_unpack_arrays(mut fm: *mut font_metric,
                                       mut tfm: *mut tfm_font) {
    let mut charinfo: uint32_t = 0;
    let mut width_index: libc::c_ushort = 0;
    let mut height_index: libc::c_uchar = 0;
    let mut depth_index: libc::c_uchar = 0;
    let mut i: uint32_t = 0;
    (*fm).widths =
        new((256i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<fixword>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut fixword;
    (*fm).heights =
        new((256i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<fixword>()
                                                 as libc::c_ulong) as
    ...
    while i <= (*tfm).ec {
        charinfo =
            *(*tfm).char_info.offset(i.wrapping_sub((*tfm).bc) as isize);
        width_index = (charinfo >> 24i32) as libc::c_ushort;
        height_index =
            (charinfo >> 20i32 & 0xfi32 as libc::c_uint) as libc::c_uchar;
        depth_index =
            (charinfo >> 16i32 & 0xfi32 as libc::c_uint) as libc::c_uchar;
        *(*fm).widths.offset(i as isize) =
            *(*tfm).width.offset(width_index as isize);
        *(*fm).heights.offset(i as isize) =
            *(*tfm).height.offset(height_index as isize);
        *(*fm).depths.offset(i as isize) =
            *(*tfm).depth.offset(depth_index as isize);
        i = i.wrapping_add(1)
    };
}
unsafe extern "C" fn sput_bigendian(mut s: *mut libc::c_char, mut v: int32_t,
...
unsafe extern "C" fn tfm_unpack_header(mut fm: *mut font_metric,
                                       mut tfm: *mut tfm_font) {
    if (*tfm).wlenheader < 12i32 as libc::c_uint {
        ...
        if len < 0i32 || len > 39i32 {
            _tt_abort(b"Invalid TFM header.\x00" as *const u8 as
                          *const libc::c_char);
        }
        if len > 0i32 {
            (*fm).codingscheme =
                new((40i32 as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut libc::c_char;
            p = (*fm).codingscheme;
            p =
                p.offset(sput_bigendian(p, *(*tfm).header.offset(2), 3i32) as
                             isize);
            i = 1i32;
            while i <= len / 4i32 {
                p =
                    p.offset(sput_bigendian(p,
                                            *(*tfm).header.offset((2i32 + i)
                                                                      as
                                                                      isize),
                                            4i32) as isize);
                i += 1
            }
            *(*fm).codingscheme.offset(len as isize) =
                '\u{0}' as i32 as libc::c_char
        } else { (*fm).codingscheme = 0 as *mut libc::c_char }
    }
    (*fm).designsize = *(*tfm).header.offset(1);
}
unsafe extern "C" fn ofm_check_size_one(mut tfm: *mut tfm_font,
                                        mut ofm_file_size: off_t) {
    let mut ofm_size: uint32_t = 14i32 as uint32_t;
    ofm_size =
        (ofm_size as
             libc::c_uint).wrapping_add((2i32 as
                                             libc::c_uint).wrapping_mul((*tfm).ec.wrapping_sub((*tfm).bc).wrapping_add(1i32
                                                                                                                           as
                                                                                                                           libc::c_uint)))
            as uint32_t as uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).wlenheader) as uint32_t
            as uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).nwidths) as uint32_t as
            uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).nheights) as uint32_t
            as uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).ndepths) as uint32_t as
            uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).nitcor) as uint32_t as
            uint32_t;
    ofm_size =
        (ofm_size as
             libc::c_uint).wrapping_add((2i32 as
                                             libc::c_uint).wrapping_mul((*tfm).nlig))
            as uint32_t as uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).nkern) as uint32_t as
            uint32_t;
    ofm_size =
        (ofm_size as
             libc::c_uint).wrapping_add((2i32 as
                                             libc::c_uint).wrapping_mul((*tfm).nextens))
            as uint32_t as uint32_t;
    ofm_size =
        (ofm_size as libc::c_uint).wrapping_add((*tfm).nfonparm) as uint32_t
            as uint32_t;
    if (*tfm).wlenfile as libc::c_long != ofm_file_size / 4i32 as libc::c_long
           || (*tfm).wlenfile != ofm_size {
        _tt_abort(b"OFM file problem.  Table sizes don\'t agree.\x00" as
                      *const u8 as *const libc::c_char);
    };
}
unsafe extern "C" fn ofm_get_sizes(mut ofm_handle: rust_input_handle_t,
                                   mut ofm_file_size: off_t,
                                   mut tfm: *mut tfm_font) {
    (*tfm).level = tt_get_signed_quad(ofm_handle);
    (*tfm).wlenfile =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"wlenfile\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).wlenheader =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"wlenheader\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).bc =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
    ...
    if (*tfm).ec < (*tfm).bc {
        _tt_abort(b"OFM file error: ec(%u) < bc(%u) ???\x00" as *const u8 as
                      *const libc::c_char, (*tfm).ec, (*tfm).bc);
    }
    (*tfm).nwidths =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nwidths\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).nheights =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nheights\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).ndepths =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"ndepths\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).nitcor =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nitcor\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).nlig =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
    ...
    (*tfm).nkern =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nkern\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).nextens =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nextens\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).nfonparm =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"nfonparm\x00" as *const u8 as
                                 *const libc::c_char);
    (*tfm).fontdir =
        tt_get_positive_quad(ofm_handle,
                             b"OFM\x00" as *const u8 as *const libc::c_char,
                             b"fontdir\x00" as *const u8 as
                                 *const libc::c_char);
    if (*tfm).fontdir != 0 {
        dpx_warning(b"I may be interpreting a font direction incorrectly.\x00"
                        as *const u8 as *const libc::c_char);
    }
    if (*tfm).level == 0i32 {
        ...
        (*tfm).nco =
            tt_get_positive_quad(ofm_handle,
                                 b"OFM\x00" as *const u8 as
                                     *const libc::c_char,
                                 b"nco\x00" as *const u8 as
                                     *const libc::c_char);
        (*tfm).ncw =
            tt_get_positive_quad(ofm_handle,
                                 b"OFM\x00" as *const u8 as
        ...
        (*tfm).npc =
            tt_get_positive_quad(ofm_handle,
                                 b"OFM\x00" as *const u8 as
                                     *const libc::c_char,
                                 b"npc\x00" as *const u8 as
                                     *const libc::c_char);
        ttstub_input_seek(ofm_handle,
                          4i32 as libc::c_long *
                              (*tfm).nco.wrapping_sub((*tfm).wlenheader) as
                                  off_t, 0i32);
    } else {
        _tt_abort(b"can\'t handle OFM files with level > 1\x00" as *const u8
                      as *const libc::c_char);
    };
}
unsafe extern "C" fn ofm_do_char_info_zero(mut ofm_handle:
                                               rust_input_handle_t,
                                           mut tfm: *mut tfm_font) {
    let mut num_chars: uint32_t = 0;
    num_chars =
        (*tfm).ec.wrapping_sub((*tfm).bc).wrapping_add(1i32 as libc::c_uint);
    if num_chars != 0i32 as libc::c_uint {
        ...
        while i < num_chars {
            ...
            *(*tfm).height_index.offset(i as isize) =
                tt_get_unsigned_byte(ofm_handle);
...
unsafe extern "C" fn ofm_do_char_info_one(mut ofm_handle: rust_input_handle_t,
                                          mut tfm: *mut tfm_font) {
    let mut num_char_infos: uint32_t = 0;
    let mut num_chars: uint32_t = 0;
    num_char_infos =
        (*tfm).ncw.wrapping_div((3i32 as
                                     libc::c_uint).wrapping_add((*tfm).npc.wrapping_div(2i32
                                                                                            as
                                                                                            libc::c_uint)));
    num_chars =
        (*tfm).ec.wrapping_sub((*tfm).bc).wrapping_add(1i32 as libc::c_uint);
    if num_chars != 0i32 as libc::c_uint {
        let mut i: uint32_t = 0;
        let mut char_infos_read: uint32_t = 0;
        (*tfm).width_index =
            new((num_chars as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ushort>()
                                                     as libc::c_ulong) as
        ...
        (*tfm).depth_index =
            new((num_chars as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        char_infos_read = 0i32 as uint32_t;
        i = 0i32 as uint32_t;
        while i < num_chars && char_infos_read < num_char_infos {
            let mut repeats: uint32_t = 0;
            ...
            *(*tfm).depth_index.offset(i as isize) =
            ...
            /* Ignore next quad */
            tt_skip_bytes(4i32 as libc::c_uint, ofm_handle);
            repeats = tt_get_unsigned_pair(ofm_handle) as uint32_t;
            /* Skip params */
            j = 0i32 as uint32_t;
            while j < (*tfm).npc {
                tt_get_unsigned_pair(ofm_handle);
            ...
            if (*tfm).npc.wrapping_div(2i32 as
                                           libc::c_uint).wrapping_mul(2i32 as
                                                                          libc::c_uint)
                   == (*tfm).npc {
                tt_get_unsigned_pair(ofm_handle);
            }
            ...
            if i.wrapping_add(repeats) > num_chars {
                _tt_abort(b"OFM \"repeats\" causes number of characters to be exceeded.\x00"
                              as *const u8 as *const libc::c_char);
            }
            j = 0i32 as uint32_t;
            while j < repeats {
                *(*tfm).width_index.offset(i.wrapping_add(j).wrapping_add(1i32
                                                                              as
                                                                              libc::c_uint)
                                               as isize) =
                    *(*tfm).width_index.offset(i as isize);
                *(*tfm).height_index.offset(i.wrapping_add(j).wrapping_add(1i32
                                                                               as
                                                                               libc::c_uint)
                                                as isize) =
                    *(*tfm).height_index.offset(i as isize);
                *(*tfm).depth_index.offset(i.wrapping_add(j).wrapping_add(1i32
                                                                              as
                                                                              libc::c_uint)
                                               as isize) =
                    *(*tfm).depth_index.offset(i as isize);
            ...
            i =
                (i as libc::c_uint).wrapping_add(repeats) as uint32_t as
                    uint32_t;
            i = i.wrapping_add(1)
        }
    };
}
unsafe extern "C" fn ofm_unpack_arrays(mut fm: *mut font_metric,
                                       mut tfm: *mut tfm_font,
                                       mut num_chars: uint32_t) {
    let mut i: uint32_t = 0;
    (*fm).widths =
        new(((*tfm).bc.wrapping_add(num_chars) as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<fixword>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut fixword;
    (*fm).heights =
    ...
    i = 0i32 as uint32_t;
    while i < num_chars {
        *(*fm).widths.offset((*tfm).bc.wrapping_add(i) as isize) =
            *(*tfm).width.offset(*(*tfm).width_index.offset(i as isize) as
                                     isize);
        *(*fm).heights.offset((*tfm).bc.wrapping_add(i) as isize) =
            *(*tfm).height.offset(*(*tfm).height_index.offset(i as isize) as
                                      isize);
        *(*fm).depths.offset((*tfm).bc.wrapping_add(i) as isize) =
            *(*tfm).depth.offset(*(*tfm).depth_index.offset(i as isize) as
                                     isize);
        i = i.wrapping_add(1)
    };
}
unsafe extern "C" fn read_ofm(mut fm: *mut font_metric,
                              mut ofm_handle: rust_input_handle_t,
                              mut ofm_file_size: off_t) {
    let mut tfm: tfm_font =
        tfm_font{level: 0,
                 wlenfile: 0,
                 wlenheader: 0,
                 bc: 0,
                 ...
                 char_info: 0 as *mut uint32_t,
                 width_index: 0 as *mut libc::c_ushort,
                 height_index: 0 as *mut libc::c_uchar,
                 depth_index: 0 as *mut libc::c_uchar,
    ...
    if tfm.level < 0i32 || tfm.level > 1i32 {
        _tt_abort(b"OFM level %d not supported.\x00" as *const u8 as
                      *const libc::c_char, tfm.level);
    }
    if tfm.wlenheader > 0i32 as libc::c_uint {
        tfm.header =
            new((tfm.wlenheader as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<fixword>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut fixword;
    ...
    ofm_unpack_arrays(fm, &mut tfm,
                      tfm.ec.wrapping_sub(tfm.bc).wrapping_add(1i32 as
                                                                   libc::c_uint));
...
unsafe extern "C" fn read_tfm(mut fm: *mut font_metric,
    ...
    if tfm.ec.wrapping_sub(tfm.bc).wrapping_add(1i32 as libc::c_uint) >
           0i32 as libc::c_uint {
        tfm.char_info =
            new((tfm.ec.wrapping_sub(tfm.bc).wrapping_add(1i32 as
                                                              libc::c_uint) as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<uint32_t>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut uint32_t;
        fread_uquads(tfm.char_info,
                     tfm.ec.wrapping_sub(tfm.bc).wrapping_add(1i32 as
                                                                  libc::c_uint),
                     tfm_handle);
    }
...
#[no_mangle]
pub unsafe extern "C" fn tfm_open(mut tfm_name: *const libc::c_char,
 ...
 -> libc::c_int {
    ...
    if suffix.is_null() ||
           strcmp(suffix, b".tfm\x00" as *const u8 as *const libc::c_char) !=
               0i32 &&
               strcmp(suffix, b".ofm\x00" as *const u8 as *const libc::c_char)
                   != 0i32 {
        ofm_name =
            new((strlen(tfm_name).wrapping_add(strlen(b".ofm\x00" as *const u8
                                                          as
    ...
    } else { ofm_name = 0 as *mut libc::c_char }
    if !ofm_name.is_null() &&
           {
        ...
        if !tfm_handle.is_null() {
            ...
            if !tfm_handle.is_null() { format = 2i32 }
        }
    }
    free(ofm_name as *mut libc::c_void);
    if tfm_handle.is_null() {
        if must_exist != 0 {
            _tt_abort(b"Unable to find TFM file \"%s\".\x00" as *const u8 as
                          *const libc::c_char, tfm_name);
        }
        return -1i32
    }
    if verbose != 0 {
        if format == 1i32 {
            dpx_message(b"(TFM:%s\x00" as *const u8 as *const libc::c_char,
                        tfm_name);
        } else if format == 2i32 {
            dpx_message(b"(OFM:%s\x00" as *const u8 as *const libc::c_char,
                        tfm_name);
        }
    }
    tfm_file_size = ttstub_input_get_size(tfm_handle) as off_t;
    if tfm_file_size > 0x1ffffffffi64 {
        _tt_abort(b"TFM/OFM file size exceeds 33-bit\x00" as *const u8 as
                      *const libc::c_char);
    }
    if tfm_file_size < 24i32 as libc::c_long {
        _tt_abort(b"TFM/OFM file too small to be a valid file.\x00" as
                      *const u8 as *const libc::c_char);
    }
    fms_need(numfms.wrapping_add(1i32 as libc::c_uint));
    fm_init(fms.offset(numfms as isize));
    if format == 2i32 {
        read_ofm(&mut *fms.offset(numfms as isize), tfm_handle,
                 tfm_file_size);
    } else {
        read_tfm(&mut *fms.offset(numfms as isize), tfm_handle,
                 tfm_file_size);
    }
    ttstub_input_close(tfm_handle);
    let ref mut fresh0 = (*fms.offset(numfms as isize)).tex_name;
    *fresh0 =
        new((strlen(tfm_name).wrapping_add(1i32 as libc::c_ulong) as uint32_t
                 as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_char;
...
#[no_mangle]
pub unsafe extern "C" fn tfm_get_fw_width(mut font_id: libc::c_int,
                                          mut ch: int32_t) -> fixword {
    let mut fm: *mut font_metric = 0 as *mut font_metric;
    let mut idx: libc::c_int = 0i32;
    if font_id < 0i32 || font_id as libc::c_uint >= numfms {
        _tt_abort(b"TFM: Invalid TFM ID: %d\x00" as *const u8 as
                      *const libc::c_char, font_id);
    }
    ...
    if ch >= (*fm).firstchar && ch <= (*fm).lastchar {
        match (*fm).charmap.type_0 {
            ...
            2 => {
                idx =
                    lookup_range((*fm).charmap.data as *const range_map, ch);
                if idx < 0i32 {
                    _tt_abort(b"Invalid char: %d\n\x00" as *const u8 as
                                  *const libc::c_char, ch);
                }
...
#[no_mangle]
pub unsafe extern "C" fn tfm_get_fw_height(mut font_id: libc::c_int,
                                           mut ch: int32_t) -> fixword {
    let mut fm: *mut font_metric = 0 as *mut font_metric;
    let mut idx: libc::c_int = 0i32;
    if font_id < 0i32 || font_id as libc::c_uint >= numfms {
...
#[no_mangle]
pub unsafe extern "C" fn tfm_get_fw_depth(mut font_id: libc::c_int,
                                          mut ch: int32_t) -> fixword {
    let mut fm: *mut font_metric = 0 as *mut font_metric;
    let mut idx: libc::c_int = 0i32;
    if font_id < 0i32 || font_id as libc::c_uint >= numfms {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn tfm_get_width(mut font_id: libc::c_int,
                                       mut ch: int32_t) -> libc::c_double {
    return tfm_get_fw_width(font_id, ch) as libc::c_double /
               (1i32 << 20i32) as libc::c_double;
}
/* tfm_string_xxx() do not work for OFM... */
#[no_mangle]
pub unsafe extern "C" fn tfm_string_width(mut font_id: libc::c_int,
                                          mut s: *const libc::c_uchar,
                                          mut len: libc::c_uint) -> fixword {
...
#[no_mangle]
pub unsafe extern "C" fn tfm_get_design_size(mut font_id: libc::c_int)
 -> libc::c_double {
    if font_id < 0i32 || font_id as libc::c_uint >= numfms {
        _tt_abort(b"TFM: Invalid TFM ID: %d\x00" as *const u8 as
                      *const libc::c_char, font_id);
    ...
    return (*fms.offset(font_id as isize)).designsize as libc::c_double /
               (1i32 << 20i32) as libc::c_double * (72.0f64 / 72.27f64);
}
/* This is dvipdfmx, an eXtended version of dvipdfm by Mark A. Wicks.
...
/* From TFM header */
#[no_mangle]
pub unsafe extern "C" fn tfm_exists(mut tfm_name: *const libc::c_char)
 -> bool {
    let mut handle: *mut rust_input_handle_t = 0 as *mut rust_input_handle_t;
    handle =
        ttstub_input_open(tfm_name, TTIF_OFM, 0i32) as
    ...
    handle =
        ttstub_input_open(tfm_name, TTIF_TFM, 0i32) as
            *mut rust_input_handle_t;
    if !handle.is_null() {
        ttstub_input_close(handle as rust_input_handle_t);
        return 1i32 != 0
