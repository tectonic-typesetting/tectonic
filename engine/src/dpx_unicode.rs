...
pub unsafe extern "C" fn UC_is_valid(mut ucv: int32_t) -> bool {
    return !(ucv < 0i32 || ucv as libc::c_long > 0x10ffffi64 ||
                 ucv as libc::c_long >= 0xd800i64 &&
                     ucv as libc::c_long <= 0xdfffi64);
}
#[no_mangle]
pub unsafe extern "C" fn UC_UTF16BE_is_valid_string(mut p:
                                                        *const libc::c_uchar,
                                                    mut endptr:
                                                        *const libc::c_uchar)
 -> bool {
    if p.offset(1) >= endptr { return 0i32 != 0 }
    while p < endptr {
        let mut ucv: int32_t = UC_UTF16BE_decode_char(&mut p, endptr);
        if !UC_is_valid(ucv) { return 0i32 != 0 }
    }
    return 1i32 != 0;
}
#[no_mangle]
pub unsafe extern "C" fn UC_UTF8_is_valid_string(mut p: *const libc::c_uchar,
                                                 mut endptr:
                                                     *const libc::c_uchar)
 -> bool {
    if p.offset(1) >= endptr { return 0i32 != 0 }
...
#[no_mangle]
pub unsafe extern "C" fn UC_UTF16BE_decode_char(mut pp:
 ...
 -> int32_t {
    ...
    if p.offset(1) >= endptr { return -1i32 }
    first =
        ((*p.offset(0) as libc::c_int) << 8i32 | *p.offset(1) as libc::c_int)
            as uint16_t;
    p = p.offset(2);
    if first as libc::c_uint >= 0xd800u32 &&
           (first as libc::c_uint) < 0xdc00u32 {
        if p.offset(1) >= endptr { return -1i32 }
        second =
            ((*p.offset(0) as libc::c_int) << 8i32 |
                 *p.offset(1) as libc::c_int) as uint16_t;
        p = p.offset(2);
        ucv = (second as libc::c_uint & 0x3ffu32) as int32_t;
        ucv =
            (ucv as libc::c_uint |
                 (first as libc::c_uint & 0x3ffu32) << 10i32) as int32_t;
        ucv += 0x10000i32
    } else if first as libc::c_uint >= 0xdc00u32 &&
                  (first as libc::c_uint) < 0xe000u32 {
        return -1i32
    } else { ucv = first as int32_t }
    *pp = p;
    return ucv;
}
#[no_mangle]
pub unsafe extern "C" fn UC_UTF16BE_encode_char(mut ucv: int32_t,
                                                mut pp:
 ...
 -> size_t {
    ...
    if ucv >= 0i32 && ucv <= 0xffffi32 {
        if p.offset(2) >= endptr { return 0i32 as size_t }
        ...
        *p.offset(1) = (ucv & 0xffi32) as libc::c_uchar;
        ...
        if p.offset(4) >= endptr { return 0i32 as size_t }
        ucv -= 0x10000i32;
        high =
            ((ucv >> 10i32) as libc::c_uint).wrapping_add(0xd800u32) as
                libc::c_ushort;
        low =
            (ucv as libc::c_uint & 0x3ffu32).wrapping_add(0xdc00u32) as
                libc::c_ushort;
        *p.offset(0) =
            (high as libc::c_int >> 8i32 & 0xffi32) as libc::c_uchar;
        *p.offset(1) = (high as libc::c_int & 0xffi32) as libc::c_uchar;
        *p.offset(2) =
            (low as libc::c_int >> 8i32 & 0xffi32) as libc::c_uchar;
...
#[no_mangle]
pub unsafe extern "C" fn UC_UTF8_decode_char(mut pp:
                                                 *mut *const libc::c_uchar,
 ...
 -> int32_t {
    ...
    if p.offset(nbytes as isize) > endptr { return -1i32 }
    loop  {
        let fresh1 = nbytes;
        nbytes = nbytes - 1;
        ...
        p = p.offset(1);
        c = *fresh2;
        if c as libc::c_int & 0xc0i32 != 0x80i32 { return -1i32 }
...
*/
#[no_mangle]
pub unsafe extern "C" fn UC_UTF8_encode_char(mut ucv: int32_t,
 ...
 -> size_t {
    ...
    if !pp.is_null() && !(*pp).is_null() && !endptr.is_null() {
        ...
        __assert_fail(b"pp && *pp && endptr\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-unicode.c\x00" as *const u8 as
                          *const libc::c_char, 197i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 71],
                                                &[libc::c_char; 71]>(b"size_t UC_UTF8_encode_char(int32_t, unsigned char **, unsigned char *)\x00")).as_ptr());
    }
    if !UC_is_valid(ucv) { return 0i32 as size_t }
    if ucv < 0x7fi32 {
        if p >= endptr.offset(-1) { return 0i32 as size_t }
        *p.offset(0) = ucv as libc::c_uchar;
        count = 1i32
    } else if ucv <= 0x7ffi32 {
        if p >= endptr.offset(-2) { return 0i32 as size_t }
        *p.offset(0) = (0xc0i32 | ucv >> 6i32) as libc::c_uchar;
        *p.offset(1) = (0x80i32 | ucv & 0x3fi32) as libc::c_uchar;
        count = 2i32
    } else if ucv <= 0xffffi32 {
        if p >= endptr.offset(-3) { return 0i32 as size_t }
        ...
        *p.offset(2) = (0x80i32 | ucv & 0x3fi32) as libc::c_uchar;
        ...
        if p >= endptr.offset(-4) { return 0i32 as size_t }
        ...
        *p.offset(3) = (0x80i32 | ucv & 0x3fi32) as libc::c_uchar;
        ...
        if p >= endptr.offset(-5) { return 0i32 as size_t }
        ...
        *p.offset(4) = (0x80i32 | ucv & 0x3fi32) as libc::c_uchar;
        ...
        if p >= endptr.offset(-6) { return 0i32 as size_t }
