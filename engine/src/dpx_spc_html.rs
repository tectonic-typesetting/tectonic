...
static mut _html_state: spc_html_ =
    {
        let mut init =
            spc_html_{opts:
                          {
                              let mut init =
                                  C2RustUnnamed_0{extensions: 0i32,};
                              init
                          },
...
unsafe extern "C" fn parse_key_val(mut pp: *mut *const libc::c_char,
 ...
 -> libc::c_int {
    ...
    let mut p: *const libc::c_char =
        0 as *const libc::c_char; /* include trailing NULL here!!! */
    let mut k: *mut libc::c_char =
        0 as *mut libc::c_char; /* we may want to add '/' */
    let mut v: *mut libc::c_char =
        0 as *mut libc::c_char; /* Should be checked somewhere else */
            ...
            v =
                new(((n + 1i32) as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut libc::c_char;
            memcpy(v as *mut libc::c_void, q as *const libc::c_void,
                   n as libc::c_ulong);
...
unsafe extern "C" fn read_html_tag(mut name: *mut libc::c_char,
 ...
 -> libc::c_int {
    ...
    if p >= endptr || *p as libc::c_int != '<' as i32 { return -1i32 }
    *type_0 = 1i32;
    p = p.offset(1);
    while p < endptr &&
              *(*__ctype_b_loc()).offset(*p as libc::c_uchar as libc::c_int as
    ...
    *name.offset(n as isize) = '\u{0}' as i32 as libc::c_char;
    ...
    while p < endptr && error == 0 && *p as libc::c_int != '/' as i32 &&
              *p as libc::c_int != '>' as i32 {
        ...
        if error == 0 {
            if !kp.is_null() {
                ...
                while *_p as libc::c_int != 0i32 {
                    if *_p as libc::c_int >= 'A' as i32 &&
                           *_p as libc::c_int <= 'Z' as i32 {
                        *_p =
                            (*_p as libc::c_int - 'A' as i32 + 'a' as i32) as
                                libc::c_char
                    }
                    _p = _p.offset(1)
                }
            }
            pdf_add_dict(attr, pdf_new_name(kp),
                         pdf_new_string(vp as *const libc::c_void,
                                        strlen(vp).wrapping_add(1i32 as
                                                                    libc::c_ulong)));
    ...
    if !name.is_null() {
        ...
        while *_p_0 as libc::c_int != 0i32 {
            if *_p_0 as libc::c_int >= 'A' as i32 &&
                   *_p_0 as libc::c_int <= 'Z' as i32 {
                *_p_0 =
                    (*_p_0 as libc::c_int - 'A' as i32 + 'a' as i32) as
                        libc::c_char
            }
            _p_0 = _p_0.offset(1)
        }
...
unsafe extern "C" fn spc_handler_html__init(mut dp: *mut libc::c_void)
 -> libc::c_int {
...
unsafe extern "C" fn spc_handler_html__clean(mut spe: *mut spc_env,
 ...
 -> libc::c_int {
    ...
    if (*sd).pending_type >= 0i32 || !(*sd).link_dict.is_null() {
        spc_warn(spe,
                 b"Unclosed html anchor found.\x00" as *const u8 as
                     *const libc::c_char);
    }
...
unsafe extern "C" fn spc_handler_html__bophook(mut spe: *mut spc_env,
                                               mut dp: *mut libc::c_void)
 -> libc::c_int {
    let mut sd: *mut spc_html_ = dp as *mut spc_html_;
    if (*sd).pending_type >= 0i32 {
        spc_warn(spe,
                 b"...html anchor continues from previous page processed...\x00"
                     as *const u8 as *const libc::c_char);
    }
    return 0i32;
}
unsafe extern "C" fn spc_handler_html__eophook(mut spe: *mut spc_env,
                                               mut dp: *mut libc::c_void)
 -> libc::c_int {
    let mut sd: *mut spc_html_ = dp as *mut spc_html_;
    if (*sd).pending_type >= 0i32 {
        spc_warn(spe,
                 b"Unclosed html anchor at end-of-page!\x00" as *const u8 as
                     *const libc::c_char);
    }
    return 0i32;
}
unsafe extern "C" fn fqurl(mut baseurl: *const libc::c_char,
 ...
 -> *mut libc::c_char {
    ...
    if !baseurl.is_null() {
        len =
            (len as
                 libc::c_ulong).wrapping_add(strlen(baseurl).wrapping_add(1i32
                                                                              as
                                                                              libc::c_ulong))
    ...
    *q = '\u{0}' as i32 as libc::c_char;
    if !baseurl.is_null() && *baseurl.offset(0) as libc::c_int != 0 {
        ...
        if *name.offset(0) as libc::c_int != 0 &&
...
unsafe extern "C" fn html_open_link(mut spe: *mut spc_env,
                                    mut name: *const libc::c_char,
                                    mut sd: *mut spc_html_) -> libc::c_int {
    ...
    if !name.is_null() {
        ...
        __assert_fail(b"name\x00" as *const u8 as *const libc::c_char,
                      b"dpx-spc_html.c\x00" as *const u8 as
                          *const libc::c_char, 289i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 71],
                                                &[libc::c_char; 71]>(b"int html_open_link(struct spc_env *, const char *, struct spc_html_ *)\x00")).as_ptr());
    }
    if (*sd).link_dict.is_null() {
    } else {
        __assert_fail(b"sd->link_dict == NULL\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-spc_html.c\x00" as *const u8 as
    ...
    pdf_add_dict((*sd).link_dict,
                 pdf_new_name(b"Type\x00" as *const u8 as
                                  *const libc::c_char),
                 pdf_new_name(b"Annot\x00" as *const u8 as
                                  *const libc::c_char));
    pdf_add_dict((*sd).link_dict,
                 pdf_new_name(b"Subtype\x00" as *const u8 as
                                  *const libc::c_char),
                 pdf_new_name(b"Link\x00" as *const u8 as
                                  *const libc::c_char));
    color = pdf_new_array();
    ...
    if *url.offset(0) as libc::c_int == '#' as i32 {
        ...
        pdf_add_dict((*sd).link_dict,
                     pdf_new_name(b"Dest\x00" as *const u8 as
                                      *const libc::c_char),
                     pdf_new_string(url.offset(1) as *const libc::c_void,
                                    strlen(url.offset(1)))); /* Otherwise must be bug */
    } else {
        let mut action: *mut pdf_obj = pdf_new_dict();
        pdf_add_dict(action,
                     pdf_new_name(b"Type\x00" as *const u8 as
                                      *const libc::c_char),
                     pdf_new_name(b"Action\x00" as *const u8 as
                                      *const libc::c_char));
        pdf_add_dict(action,
                     pdf_new_name(b"S\x00" as *const u8 as
                                      *const libc::c_char),
                     pdf_new_name(b"URI\x00" as *const u8 as
                                      *const libc::c_char));
        pdf_add_dict(action,
                     pdf_new_name(b"URI\x00" as *const u8 as
                                      *const libc::c_char),
...
unsafe extern "C" fn html_open_dest(mut spe: *mut spc_env,
                                    mut name: *const libc::c_char,
                                    mut sd: *mut spc_html_) -> libc::c_int {
    ...
    if !page_ref.is_null() {
        ...
        __assert_fail(b"page_ref\x00" as *const u8 as *const libc::c_char,
                      b"dpx-spc_html.c\x00" as *const u8 as
                          *const libc::c_char, 346i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 71],
                                                &[libc::c_char; 71]>(b"int html_open_dest(struct spc_env *, const char *, struct spc_html_ *)\x00")).as_ptr());
    }
    array = pdf_new_array();
    pdf_add_array(array, page_ref);
    pdf_add_array(array,
                  pdf_new_name(b"XYZ\x00" as *const u8 as
                                   *const libc::c_char));
    ...
    error =
        pdf_doc_add_names(b"Dests\x00" as *const u8 as *const libc::c_char,
                          ...
                          strlen(name) as libc::c_int, array);
    if error != 0 {
        spc_warn(spe,
                 b"Failed to add named destination: %s\x00" as *const u8 as
                     *const libc::c_char, name);
    }
    (*sd).pending_type = 1i32;
    return error;
}
unsafe extern "C" fn spc_html__anchor_open(mut spe: *mut spc_env,
                                           mut attr: *mut pdf_obj,
 ...
 -> libc::c_int {
    ...
    if (*sd).pending_type >= 0i32 || !(*sd).link_dict.is_null() {
        spc_warn(spe,
                 b"Nested html anchors found!\x00" as *const u8 as
                     *const libc::c_char);
        return -1i32
    }
    href =
        pdf_lookup_dict(attr,
    ...
    if !href.is_null() && !name.is_null() {
        ...
        error =
            html_open_link(spe, pdf_string_value(href) as *const libc::c_char,
                           sd)
    } else if !name.is_null() {
        /* name */
        error =
            html_open_dest(spe, pdf_string_value(name) as *const libc::c_char,
                           sd)
    } else {
        spc_warn(spe,
                 b"You should have \"href\" or \"name\" in anchor tag!\x00" as
                     *const u8 as *const libc::c_char);
        error = -1i32
    }
    return error;
}
unsafe extern "C" fn spc_html__anchor_close(mut spe: *mut spc_env,
                                            mut sd: *mut spc_html_)
 -> libc::c_int {
    let mut error: libc::c_int = 0i32;
    match (*sd).pending_type {
        0 => {
            if !(*sd).link_dict.is_null() {
                ...
                spc_warn(spe,
                         b"Closing html anchor (link) without starting!\x00"
                             as *const u8 as *const libc::c_char);
                error = -1i32
            }
        }
        ...
        _ => {
            spc_warn(spe,
                     b"No corresponding opening tag for html anchor.\x00" as
                         *const u8 as *const libc::c_char);
            error = -1i32
        }
    }
...
unsafe extern "C" fn spc_html__base_empty(mut spe: *mut spc_env,
                                          mut attr: *mut pdf_obj,
                                          mut sd: *mut spc_html_)
 -> libc::c_int {
    let mut href: *mut pdf_obj = 0 as *mut pdf_obj;
    ...
    if href.is_null() {
        spc_warn(spe,
                 b"\"href\" not found for \"base\" tag!\x00" as *const u8 as
                     *const libc::c_char);
        return -1i32
    }
    ...
    if !(*sd).baseurl.is_null() {
        spc_warn(spe,
                 b"\"baseurl\" changed: \"%s\" --> \"%s\"\x00" as *const u8 as
                     *const libc::c_char, (*sd).baseurl, vp);
        free((*sd).baseurl as *mut libc::c_void);
    }
    (*sd).baseurl =
        new((strlen(vp).wrapping_add(1i32 as libc::c_ulong) as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_char;
...
unsafe extern "C" fn atopt(mut a: *const libc::c_char) -> libc::c_double {
    ...
    if q.is_null() {
        dpx_warning(b"Invalid length value: %s (%c)\x00" as *const u8 as
                        *const libc::c_char, a, *p as libc::c_int);
        return 0.0f64
    }
    v = atof(q);
    ...
    if !q.is_null() {
        ...
        match k {
            ...
            9 => { u *= 1.0f64 }
            _ => {
                dpx_warning(b"Unknown unit of measure: %s\x00" as *const u8 as
                                *const libc::c_char, q);
            }
        }
        free(q as *mut libc::c_void);
...
unsafe extern "C" fn spc_html__img_empty(mut spe: *mut spc_env,
                                         mut attr: *mut pdf_obj)
 -> libc::c_int {
    ...
    /* ENABLE_HTML_SVG_TRANSFORM */
    spc_warn(spe,
             b"html \"img\" tag found (not completed, plese don\'t use!).\x00"
                 as *const u8 as *const libc::c_char);
    src =
        pdf_lookup_dict(attr, b"src\x00" as *const u8 as *const libc::c_char);
    if src.is_null() {
        spc_warn(spe,
                 b"\"src\" attribute not found for \"img\" tag!\x00" as
                     *const u8 as *const libc::c_char);
        return -1i32
    }
    ...
    obj =
        pdf_lookup_dict(attr,
                        b"svg:transform\x00" as *const u8 as
                            *const libc::c_char);
    if !obj.is_null() {
        let mut p: *const libc::c_char =
            pdf_string_value(obj) as *const libc::c_char;
        let mut N: pdf_tmatrix =
            pdf_tmatrix{a: 0., b: 0., c: 0., d: 0., e: 0., f: 0.,};
    ...
    if error != 0 {
        spc_warn(spe,
                 b"Error in html \"img\" tag attribute.\x00" as *const u8 as
                     *const libc::c_char); /* Not Tps prefix but... */
        return error
    } /* op: */
    id =
        pdf_ximage_findresource(pdf_string_value(src) as *const libc::c_char,
                                options); /* op: */
    if id < 0i32 {
        spc_warn(spe,
                 b"Could not find/load image: %s\x00" as *const u8 as
                     *const libc::c_char,
                 pdf_string_value(src) as *mut libc::c_char); /* op: gs */
        error = -1i32
    } else {
        ...
        if a != 0i32 {
            res_name =
                new((strlen(b"_Tps_a100_\x00" as *const u8 as
            ...
            if check_resourcestatus(b"ExtGState\x00" as *const u8 as
                                        *const libc::c_char, res_name) == 0 {
                dict =
                    create_xgstate(round(0.01f64 * a as libc::c_double /
                                             0.01f64) * 0.01f64, 0i32);
                pdf_doc_add_page_resource(b"ExtGState\x00" as *const u8 as
                                              *const libc::c_char, res_name,
                                          pdf_ref_obj(dict));
                pdf_release_obj(dict);
            }
            pdf_doc_add_page_content(b" /\x00" as *const u8 as
                                         *const libc::c_char,
                                     2i32 as libc::c_uint);
            pdf_doc_add_page_content(res_name,
                                     strlen(res_name) as libc::c_uint);
            pdf_doc_add_page_content(b" gs\x00" as *const u8 as
                                         *const libc::c_char,
                                     3i32 as libc::c_uint);
        ...
        pdf_doc_add_page_content(b" Do\x00" as *const u8 as
                                     *const libc::c_char,
                                 3i32 as libc::c_uint);
        pdf_dev_grestore();
...
unsafe extern "C" fn spc_handler_html_default(mut spe: *mut spc_env,
                                              mut ap: *mut spc_arg)
 -> libc::c_int {
    let mut sd: *mut spc_html_ =
        &mut _html_state; /* treat "open" same as "empty" */
    let mut name: [libc::c_char; 128] =
        [0; 128]; /* treat "open" same as "empty" */
    let mut attr: *mut pdf_obj = 0 as *mut pdf_obj;
    let mut error: libc::c_int = 0i32;
    let mut type_0: libc::c_int = 1i32;
    if (*ap).curptr >= (*ap).endptr { return 0i32 }
    attr = pdf_new_dict();
    error =
        read_html_tag(name.as_mut_ptr(), attr, &mut type_0, &mut (*ap).curptr,
                      (*ap).endptr);
    if error != 0 { pdf_release_obj(attr); return error }
    if streq_ptr(name.as_mut_ptr(),
        ...
        match type_0 {
            ...
            _ => {
                spc_warn(spe,
                         b"Empty html anchor tag???\x00" as *const u8 as
                             *const libc::c_char);
                error = -1i32
            }
        }
    } else if streq_ptr(name.as_mut_ptr(),
                        b"base\x00" as *const u8 as *const libc::c_char) {
        if type_0 == 2i32 {
            spc_warn(spe,
                     b"Close tag for \"base\"???\x00" as *const u8 as
                         *const libc::c_char);
            error = -1i32
        } else { error = spc_html__base_empty(spe, attr, sd) }
    } else if streq_ptr(name.as_mut_ptr(),
                        b"img\x00" as *const u8 as *const libc::c_char) {
        if type_0 == 2i32 {
            spc_warn(spe,
                     b"Close tag for \"img\"???\x00" as *const u8 as
                         *const libc::c_char);
            error = -1i32
        } else { error = spc_html__img_empty(spe, attr) }
    }
    pdf_release_obj(attr);
    while (*ap).curptr < (*ap).endptr &&
              *(*__ctype_b_loc()).offset(*(*ap).curptr.offset(0) as
...
unsafe extern "C" fn cvt_a_to_tmatrix(mut M: *mut pdf_tmatrix,
 ...
 -> libc::c_int {
    ...
    n = 0i32;
    while n < 6i32 && *p as libc::c_int != 0 &&
              *p as libc::c_int != ')' as i32 {
        q = parse_float_decimal(&mut p, p.offset(strlen(p) as isize));
        if q.is_null() { break ; }
        v[n as usize] = atof(q);
...
pub unsafe extern "C" fn spc_html_at_begin_page() -> libc::c_int {
    let mut sd: *mut spc_html_ = &mut _html_state;
    return spc_handler_html__bophook(0 as *mut spc_env,
                                     sd as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn spc_html_at_end_page() -> libc::c_int {
    let mut sd: *mut spc_html_ = &mut _html_state;
    return spc_handler_html__eophook(0 as *mut spc_env,
                                     sd as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn spc_html_at_end_document() -> libc::c_int {
    let mut sd: *mut spc_html_ = &mut _html_state;
    return spc_handler_html__clean(0 as *mut spc_env,
                                   sd as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn spc_html_check_special(mut buffer:
                                                    *const libc::c_char,
                                                mut size: libc::c_int)
 -> bool {
    ...
    if size as libc::c_ulong >=
           ...
           memcmp(p as *const libc::c_void,
                  b"html:\x00" as *const u8 as *const libc::c_char as
                      *const libc::c_void,
                  strlen(b"html:\x00" as *const u8 as *const libc::c_char)) ==
               0 {
        return 1i32 != 0
...
*/
#[no_mangle]
pub unsafe extern "C" fn spc_html_setup_handler(mut sph: *mut spc_handler,
                                                mut spe: *mut spc_env,
                                                mut ap: *mut spc_arg)
 -> libc::c_int {
    ...
    if (*ap).curptr.offset(strlen(b"html:\x00" as *const u8 as
    ...
    (*sph).exec =
        Some(spc_handler_html_default as
                 unsafe extern "C" fn(_: *mut spc_env, _: *mut spc_arg)
                     -> libc::c_int);
    (*ap).curptr =
        (*ap).curptr.offset(strlen(b"html:\x00" as *const u8 as
                                       *const libc::c_char) as isize);
    while (*ap).curptr < (*ap).endptr &&
              *(*__ctype_b_loc()).offset(*(*ap).curptr.offset(0) as
                                             libc::c_uchar as libc::c_int as
