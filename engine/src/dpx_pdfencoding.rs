...
unsafe extern "C" fn pdf_init_encoding_struct(mut encoding:
                                                  *mut pdf_encoding) {
    if !encoding.is_null() {
    } else {
        __assert_fail(b"encoding\x00" as *const u8 as *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 93i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 46],
                                                &[libc::c_char; 46]>(b"void pdf_init_encoding_struct(pdf_encoding *)\x00")).as_ptr());
    }
    (*encoding).ident = 0 as *mut libc::c_char;
    (*encoding).enc_name = 0 as *mut libc::c_char;
    memset((*encoding).glyphs.as_mut_ptr() as *mut libc::c_void, 0i32,
           (256i32 as
                libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut libc::c_char>()
                                                as libc::c_ulong));
    memset((*encoding).is_used.as_mut_ptr() as *mut libc::c_void, 0i32,
 ...
 */
unsafe extern "C" fn create_encoding_resource(mut encoding: *mut pdf_encoding,
 ...
 -> *mut pdf_obj {
    ...
    if !encoding.is_null() {
        ...
        __assert_fail(b"encoding\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 66],
                                                &[libc::c_char; 66]>(b"pdf_obj *create_encoding_resource(pdf_encoding *, pdf_encoding *)\x00")).as_ptr());
    }
    if (*encoding).resource.is_null() {
    } else {
        __assert_fail(b"!encoding->resource\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 120i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 66],
                                                &[libc::c_char; 66]>(b"pdf_obj *create_encoding_resource(pdf_encoding *, pdf_encoding *)\x00")).as_ptr());
    }
    differences =
        make_encoding_differences((*encoding).glyphs.as_mut_ptr(),
                                  if !baseenc.is_null() {
    ...
    if !differences.is_null() {
        ...
        if !baseenc.is_null() {
            pdf_add_dict(resource,
                         pdf_new_name(b"BaseEncoding\x00" as *const u8 as
                                          *const libc::c_char),
                         pdf_link_obj((*baseenc).resource));
        }
        pdf_add_dict(resource,
                     pdf_new_name(b"Differences\x00" as *const u8 as
                                      *const libc::c_char), differences);
        return resource
    } else {
...
unsafe extern "C" fn pdf_flush_encoding(mut encoding: *mut pdf_encoding) {
    if !encoding.is_null() {
        ...
        __assert_fail(b"encoding\x00" as *const u8 as *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 152i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 40],
                                                &[libc::c_char; 40]>(b"void pdf_flush_encoding(pdf_encoding *)\x00")).as_ptr());
    }
    if !(*encoding).resource.is_null() {
...
unsafe extern "C" fn pdf_clean_encoding_struct(mut encoding:
                                                   *mut pdf_encoding) {
    ...
    if !encoding.is_null() {
        ...
        __assert_fail(b"encoding\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 47],
                                                &[libc::c_char; 47]>(b"void pdf_clean_encoding_struct(pdf_encoding *)\x00")).as_ptr());
    }
    if !(*encoding).resource.is_null() {
        _tt_abort(b"Object not flushed.\x00" as *const u8 as
                      *const libc::c_char);
    }
    pdf_release_obj((*encoding).tounicode);
    ...
    while code < 256i32 {
        (*encoding).glyphs[code as usize] =
            mfree((*encoding).glyphs[code as usize] as *mut libc::c_void) as
                *mut libc::c_char;
        code += 1
    }
    (*encoding).ident = 0 as *mut libc::c_char;
...
/* This is dvipdfmx, an eXtended version of dvipdfm by Mark A. Wicks.
   ...
   Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA.
*/
unsafe extern "C" fn is_similar_charset(mut enc_vec: *mut *mut libc::c_char,
                                        mut enc_vec2:
    ...
    while code < 256i32 {
        if !(!(*enc_vec.offset(code as isize)).is_null() &&
                 strcmp(*enc_vec.offset(code as isize),
                        *enc_vec2.offset(code as isize)) != 0) &&
               { same += 1; same >= 64i32 } {
            /* is 64 a good level? */
            return 1i32 != 0
        }
 ...
 */
unsafe extern "C" fn make_encoding_differences(mut enc_vec:
                                                   *mut *mut libc::c_char,
                                               mut baseenc:
                                                   *mut *mut libc::c_char,
                                               mut is_used:
 ...
 -> *mut pdf_obj {
    ...
    if !enc_vec.is_null() {
        ...
        __assert_fail(b"enc_vec\x00" as *const u8 as *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 217i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 67],
                                                &[libc::c_char; 67]>(b"pdf_obj *make_encoding_differences(char **, char **, const char *)\x00")).as_ptr());
    }
    /*
    ...
    while code < 256i32 {
        ...
        if !is_used.is_null() && *is_used.offset(code as isize) == 0 ||
            ...
            skipping = 1i32
                      ...
                      strcmp(*baseenc.offset(code as isize),
                             *enc_vec.offset(code as isize)) != 0i32 {
            /*
             * Difference found.
             */
            if skipping != 0 {
                pdf_add_array(differences,
                              pdf_new_number(code as libc::c_double));
            }
            pdf_add_array(differences,
                          pdf_new_name(*enc_vec.offset(code as isize)));
            skipping = 0i32;
            count += 1
        } else { skipping = 1i32 }
        code += 1
...
unsafe extern "C" fn load_encoding_file(mut filename: *const libc::c_char)
 -> libc::c_int {
    ...
    let mut enc_vec: [*const libc::c_char; 256] =
        [0 as *const libc::c_char; 256];
    let mut code: libc::c_int = 0;
    let mut fsize: libc::c_int = 0;
    let mut enc_id: libc::c_int = 0;
    if filename.is_null() { return -1i32 }
    if verbose != 0 {
        dpx_message(b"(Encoding:%s\x00" as *const u8 as *const libc::c_char,
                    filename);
    }
    handle =
        dpx_tt_open(filename, b".enc\x00" as *const u8 as *const libc::c_char,
                    TTIF_ENC);
    if handle.is_null() { return -1i32 }
    fsize = ttstub_input_get_size(handle) as libc::c_int;
    wbuf_0 =
        new(((fsize + 1i32) as uint32_t as
    ...
    *wbuf_0.offset(fsize as isize) = '\u{0}' as i32 as libc::c_char;
    if ttstub_input_read(handle, wbuf_0, fsize as size_t) !=
           fsize as libc::c_long {
        _tt_abort(b"error reading %s\x00" as *const u8 as *const libc::c_char,
                  filename);
    }
    ...
    if encoding_array.is_null() { pdf_release_obj(enc_name); return -1i32 }
    code = 0i32;
    while code < 256i32 {
        enc_vec[code as usize] =
            pdf_name_value(pdf_get_array(encoding_array, code));
        code += 1
    }
    enc_id =
        pdf_encoding_new_encoding(if !enc_name.is_null() {
                                      pdf_name_value(enc_name)
                                  } else { 0 as *mut libc::c_char }, filename,
                                  enc_vec.as_mut_ptr(),
                                  0 as *const libc::c_char, 0i32);
    if !enc_name.is_null() {
...
static mut enc_cache: C2RustUnnamed =
    {
        let mut init =
            C2RustUnnamed{count: 0i32,
                          capacity: 0i32,
                          encodings:
                              0 as *const pdf_encoding as *mut pdf_encoding,};
        init
    };
#[no_mangle]
pub unsafe extern "C" fn pdf_init_encodings() {
    ...
    enc_cache.encodings =
        new((enc_cache.capacity as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<pdf_encoding>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut pdf_encoding;
    /*
     * PDF Predefined Encodings
     */
    pdf_encoding_new_encoding(b"WinAnsiEncoding\x00" as *const u8 as
                                  *const libc::c_char,
                              b"WinAnsiEncoding\x00" as *const u8 as
                                  *const libc::c_char,
                              WinAnsiEncoding.as_mut_ptr(),
                              0 as *const libc::c_char, 1i32 << 0i32);
    pdf_encoding_new_encoding(b"MacRomanEncoding\x00" as *const u8 as
                                  *const libc::c_char,
                              b"MacRomanEncoding\x00" as *const u8 as
                                  *const libc::c_char,
                              MacRomanEncoding.as_mut_ptr(),
                              0 as *const libc::c_char, 1i32 << 0i32);
    pdf_encoding_new_encoding(b"MacExpertEncoding\x00" as *const u8 as
                                  *const libc::c_char,
                              b"MacExpertEncoding\x00" as *const u8 as
                                  *const libc::c_char,
 ...
 */
unsafe extern "C" fn pdf_encoding_new_encoding(mut enc_name:
                                                   *const libc::c_char,
 ...
 -> libc::c_int {
    ...
    if fresh0 >= enc_cache.capacity {
        ...
        enc_cache.encodings =
            renew(enc_cache.encodings as *mut libc::c_void,
                  (enc_cache.capacity as uint32_t as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<pdf_encoding>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut pdf_encoding
    }
    encoding =
        &mut *enc_cache.encodings.offset(enc_id as isize) as
            *mut pdf_encoding;
    pdf_init_encoding_struct(encoding);
    (*encoding).ident =
        new((strlen(ident).wrapping_add(1i32 as libc::c_ulong) as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_char;
    strcpy((*encoding).ident, ident);
    (*encoding).enc_name =
        new((strlen(enc_name).wrapping_add(1i32 as libc::c_ulong) as uint32_t
                 as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_char;
    ...
    while code < 256i32 {
        if !(*encoding_vec.offset(code as isize)).is_null() &&
               strcmp(*encoding_vec.offset(code as isize),
                      b".notdef\x00" as *const u8 as *const libc::c_char) != 0
           {
            (*encoding).glyphs[code as usize] =
                new((strlen(*encoding_vec.offset(code as
                         ...
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut libc::c_char;
            strcpy((*encoding).glyphs[code as usize],
                   *encoding_vec.offset(code as isize));
        }
        code += 1
    }
    if baseenc_name.is_null() && flags & 1i32 << 0i32 == 0 &&
           is_similar_charset((*encoding).glyphs.as_mut_ptr(),
                              WinAnsiEncoding.as_mut_ptr()) as libc::c_int !=
               0 {
    ...
    if !baseenc_name.is_null() {
        let mut baseenc_id: libc::c_int =
            pdf_encoding_findresource(baseenc_name);
        if baseenc_id < 0i32 || pdf_encoding_is_predefined(baseenc_id) == 0 {
            _tt_abort(b"Illegal base encoding %s for encoding %s\n\x00" as
                          *const u8 as *const libc::c_char, baseenc_name,
                      (*encoding).enc_name);
        }
        (*encoding).baseenc =
            &mut *enc_cache.encodings.offset(baseenc_id as isize) as
                *mut pdf_encoding
    }
...
pub unsafe extern "C" fn pdf_encoding_complete() {
    ...
    while enc_id < enc_cache.count {
        if pdf_encoding_is_predefined(enc_id) == 0 {
             ...
             */
            let mut with_base: libc::c_int =
                ((*encoding).flags & 1i32 << 1i32 == 0 ||
                     pdf_get_version() >= 4i32 as libc::c_uint) as
                    libc::c_int;
            if (*encoding).resource.is_null() {
            } else {
                __assert_fail(b"!encoding->resource\x00" as *const u8 as
                              ...
                              (*::std::mem::transmute::<&[u8; 33],
                                                        &[libc::c_char; 33]>(b"void pdf_encoding_complete(void)\x00")).as_ptr());
            }
            (*encoding).resource =
                create_encoding_resource(encoding,
                                         if with_base != 0 {
                                             (*encoding).baseenc
                                         } else { 0 as *mut pdf_encoding });
            if (*encoding).tounicode.is_null() {
            } else {
                __assert_fail(b"!encoding->tounicode\x00" as *const u8 as
                                  *const libc::c_char,
                              b"dpx-pdfencoding.c\x00" as *const u8 as
                                  *const libc::c_char, 454i32 as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 33],
                                                        &[libc::c_char; 33]>(b"void pdf_encoding_complete(void)\x00")).as_ptr());
            }
            (*encoding).tounicode =
                pdf_create_ToUnicode_CMap((*encoding).enc_name,
...
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_findresource(mut enc_name:
                                                       *const libc::c_char)
 -> libc::c_int {
    let mut enc_id: libc::c_int = 0;
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    if !enc_name.is_null() {
        ...
        __assert_fail(b"enc_name\x00" as *const u8 as *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 490i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 44],
                                                &[libc::c_char; 44]>(b"int pdf_encoding_findresource(const char *)\x00")).as_ptr());
    }
    enc_id = 0i32;
    while enc_id < enc_cache.count {
        encoding =
        ...
        if !(*encoding).ident.is_null() &&
            ...
            if !(*encoding).enc_name.is_null() &&
                   streq_ptr(enc_name, (*encoding).enc_name) as libc::c_int !=
                       0 {
                return enc_id
            }
        }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_get_encoding(mut enc_id: libc::c_int)
 -> *mut *mut libc::c_char {
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    if enc_id < 0i32 || enc_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
                      *const libc::c_char, enc_id);
    }
    encoding =
        &mut *enc_cache.encodings.offset(enc_id as isize) as
            *mut pdf_encoding;
...
#[no_mangle]
pub unsafe extern "C" fn pdf_get_encoding_obj(mut enc_id: libc::c_int)
 -> *mut pdf_obj {
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    if enc_id < 0i32 || enc_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
...
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_is_predefined(mut enc_id: libc::c_int)
 -> libc::c_int {
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    if enc_id < 0i32 || enc_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
    ...
    encoding =
        &mut *enc_cache.encodings.offset(enc_id as isize) as
            *mut pdf_encoding;
    return if (*encoding).flags & 1i32 << 0i32 != 0 { 1i32 } else { 0i32 };
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_get_name(mut enc_id: libc::c_int)
 -> *mut libc::c_char {
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    if enc_id < 0i32 || enc_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
...
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_add_usedchars(mut encoding_id:
                                                        libc::c_int,
                                                    mut is_used:
                                                        *const libc::c_char) {
    let mut encoding: *mut pdf_encoding = 0 as *mut pdf_encoding;
    ...
    if encoding_id < 0i32 || encoding_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
                      *const libc::c_char, encoding_id);
    }
    if is_used.is_null() || pdf_encoding_is_predefined(encoding_id) != 0 {
        return
    }
    encoding =
        &mut *enc_cache.encodings.offset(encoding_id as isize) as
            *mut pdf_encoding;
    code = 0i32;
    while code <= 0xffi32 {
        (*encoding).is_used[code as usize] =
            ((*encoding).is_used[code as usize] as libc::c_int |
                 *is_used.offset(code as isize) as libc::c_int) as
                libc::c_char;
        code += 1
    };
}
#[no_mangle]
pub unsafe extern "C" fn pdf_encoding_get_tounicode(mut encoding_id:
                                                        libc::c_int)
 -> *mut pdf_obj {
    if encoding_id < 0i32 || encoding_id >= enc_cache.count {
        _tt_abort(b"Invalid encoding id: %d\x00" as *const u8 as
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn pdf_create_ToUnicode_CMap(mut enc_name:
                                                       *const libc::c_char,
                                                   mut enc_vec:
                                                       *mut *mut libc::c_char,
                                                   mut is_used:
 ...
 -> *mut pdf_obj {
    ...
    if !enc_name.is_null() && !enc_vec.is_null() {
        ...
        __assert_fail(b"enc_name && enc_vec\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-pdfencoding.c\x00" as *const u8 as
                          *const libc::c_char, 629i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 72],
                                                &[libc::c_char; 72]>(b"pdf_obj *pdf_create_ToUnicode_CMap(const char *, char **, const char *)\x00")).as_ptr());
    }
    cmap_name =
        new((strlen(enc_name).wrapping_add(strlen(b"-UTF16\x00" as *const u8
                                                      as
                 ...
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_char;
    sprintf(cmap_name, b"%s-UTF16\x00" as *const u8 as *const libc::c_char,
            enc_name);
    cmap = CMap_new();
    ...
    CMap_add_codespacerange(cmap, range_min.as_mut_ptr(),
                            range_max.as_mut_ptr(), 1i32 as size_t);
    all_predef = 1i32;
    code = 0i32;
    while code <= 0xffi32 {
        if !(!is_used.is_null() && *is_used.offset(code as isize) == 0) {
                ...
                if pdf_get_version() < 5i32 as libc::c_uint || agln.is_null()
                    ...
                    if len >= 1i32 && fail_count == 0 {
                        CMap_add_bfchar(cmap, wbuf.as_mut_ptr(),
                                        ...
                                        len as size_t);
                        all_predef &=
                            (!agln.is_null() && (*agln).is_predef != 0) as
                                libc::c_int
                    }
                }
            }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn pdf_load_ToUnicode_stream(mut ident:
                                                       *const libc::c_char)
 -> *mut pdf_obj {
    let mut stream: *mut pdf_obj = 0 as *mut pdf_obj;
    let mut cmap: *mut CMap = 0 as *mut CMap;
    let mut handle: rust_input_handle_t = 0 as *mut libc::c_void;
    if ident.is_null() { return 0 as *mut pdf_obj }
    handle = ttstub_input_open(ident, TTIF_CMAP, 0i32);
    if handle.is_null() { return 0 as *mut pdf_obj }
    ...
    if CMap_parse(cmap, handle) < 0i32 {
        dpx_warning(b"Reading CMap file \"%s\" failed.\x00" as *const u8 as
        ...
        if stream.is_null() {
            dpx_warning(b"Failed to creat ToUnicode CMap stream for \"%s\".\x00"
