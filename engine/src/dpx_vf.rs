...
extern "C" {
    ...
    #[no_mangle]
    fn dvi_set_font(font_id: libc::c_int);
    #[no_mangle]
    fn dvi_set(ch: int32_t);
    #[no_mangle]
    fn dvi_put(ch: int32_t);
    #[no_mangle]
    fn dvi_dirchg(dir: libc::c_uchar);
    #[no_mangle]
    fn dvi_do_special(buffer: *const libc::c_void, size: int32_t);
...
*/
    ...
    fn dvi_z0();
    #[no_mangle]
    fn dvi_vf_init(dev_font_id: libc::c_int);
    #[no_mangle]
    fn dvi_y(ch: int32_t);
    #[no_mangle]
    fn dvi_push();
    ...
    fn dvi_y0();
    #[no_mangle]
    fn dvi_locate_font(name: *const libc::c_char, ptsize: spt_t)
     -> libc::c_uint;
    #[no_mangle]
    fn dvi_vf_finish();
    #[no_mangle]
    fn dvi_down(y: int32_t);
    #[no_mangle]
    fn dvi_right(x: int32_t);
    #[no_mangle]
    fn dvi_x(ch: int32_t);
    #[no_mangle]
    fn dvi_w0();
    ...
    fn dvi_x0();
    #[no_mangle]
    fn dvi_rule(width: int32_t, height: int32_t);
    #[no_mangle]
    fn dvi_w(ch: int32_t);
    /* This is dvipdfmx, an eXtended version of dvipdfm by Mark A. Wicks.
...
pub struct vf {
    ...
    pub ptsize: spt_t,
    pub design_size: uint32_t,
    pub num_dev_fonts: libc::c_uint,
    pub max_dev_fonts: libc::c_uint,
    pub dev_fonts: *mut font_def,
    pub ch_pkt: *mut *mut libc::c_uchar,
    pub pkt_len: *mut uint32_t,
    pub num_chars: libc::c_uint,
}
#[derive ( Copy , Clone )]
#[repr(C)]
pub struct font_def {
...
#[no_mangle]
pub unsafe extern "C" fn vf_set_verbose(mut level: libc::c_int) {
    verbose = level as libc::c_uchar;
}
static mut vf_fonts: *mut vf = 0 as *const vf as *mut vf;
static mut num_vf_fonts: libc::c_uint = 0i32 as libc::c_uint;
static mut max_vf_fonts: libc::c_uint = 0i32 as libc::c_uint;
#[no_mangle]
pub unsafe extern "C" fn vf_reset_global_state() {
    num_vf_fonts = 0i32 as libc::c_uint;
    max_vf_fonts = 0i32 as libc::c_uint;
    vf_fonts = 0 as *mut vf;
}
unsafe extern "C" fn read_header(mut vf_handle: rust_input_handle_t,
                                 mut thisfont: libc::c_int) {
    if tt_get_unsigned_byte(vf_handle) as libc::c_int != 247i32 ||
           tt_get_unsigned_byte(vf_handle) as libc::c_int != 202i32 {
        fprintf(stderr,
    ...
    /* skip comment */
    tt_skip_bytes(tt_get_unsigned_byte(vf_handle) as libc::c_uint, vf_handle);
    /* Skip checksum */
    tt_skip_bytes(4i32 as libc::c_uint, vf_handle);
    (*vf_fonts.offset(thisfont as isize)).design_size =
        tt_get_positive_quad(vf_handle,
...
unsafe extern "C" fn resize_vf_fonts(mut size: libc::c_int) {
    ...
    if size as libc::c_uint > max_vf_fonts {
        vf_fonts =
            renew(vf_fonts as *mut libc::c_void,
                  (size as uint32_t as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<vf>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut vf;
        i = max_vf_fonts as libc::c_int;
        while i < size {
            (*vf_fonts.offset(i as isize)).num_dev_fonts =
                0i32 as libc::c_uint;
            (*vf_fonts.offset(i as isize)).max_dev_fonts =
                0i32 as libc::c_uint;
            let ref mut fresh0 = (*vf_fonts.offset(i as isize)).dev_fonts;
            *fresh0 = 0 as *mut font_def;
            i += 1
        }
        max_vf_fonts = size as libc::c_uint
    };
}
unsafe extern "C" fn resize_one_vf_font(mut a_vf: *mut vf,
                                        mut size: libc::c_uint) {
    let mut i: libc::c_uint = 0;
    if size > (*a_vf).num_chars {
        size =
            if size > (*a_vf).num_chars.wrapping_add(256i32 as libc::c_uint) {
                size
            } else { (*a_vf).num_chars.wrapping_add(256i32 as libc::c_uint) };
        (*a_vf).ch_pkt =
            renew((*a_vf).ch_pkt as *mut libc::c_void,
                  (size as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut libc::c_uchar>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut *mut libc::c_uchar;
        (*a_vf).pkt_len =
            renew((*a_vf).pkt_len as *mut libc::c_void,
                  (size as
        ...
        while i < size {
            ...
            *(*a_vf).pkt_len.offset(i as isize) = 0i32 as uint32_t;
            i = i.wrapping_add(1)
        }
        (*a_vf).num_chars = size
    };
}
unsafe extern "C" fn read_a_char_def(mut vf_handle: rust_input_handle_t,
                                     mut thisfont: libc::c_int,
    ...
    if ch >= (*vf_fonts.offset(thisfont as isize)).num_chars {
        resize_one_vf_font(vf_fonts.offset(thisfont as isize),
                           ch.wrapping_add(1i32 as
                                               libc::c_uint)); /* must exist */
    }
    if pkt_len > 0i32 as libc::c_uint {
        ...
        if ttstub_input_read(vf_handle, pkt as *mut libc::c_char,
                             pkt_len as size_t) != pkt_len as libc::c_long {
            _tt_abort(b"VF file ended prematurely.\x00" as *const u8 as
                          *const libc::c_char);
        }
        let ref mut fresh2 =
            *(*vf_fonts.offset(thisfont as isize)).ch_pkt.offset(ch as isize);
        *fresh2 = pkt
    }
    *(*vf_fonts.offset(thisfont as isize)).pkt_len.offset(ch as isize) =
        pkt_len;
}
unsafe extern "C" fn read_a_font_def(mut vf_handle: rust_input_handle_t,
                                     mut font_id: int32_t,
                                     mut thisfont: libc::c_int) {
    ...
    if (*vf_fonts.offset(thisfont as isize)).num_dev_fonts >=
        ...
        *fresh4 =
            renew((*vf_fonts.offset(thisfont as isize)).dev_fonts as
                      *mut libc::c_void,
                  ((*vf_fonts.offset(thisfont as isize)).max_dev_fonts as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<font_def>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut font_def
    }
    dev_font =
        (*vf_fonts.offset(thisfont as
                              isize)).dev_fonts.offset((*vf_fonts.offset(thisfont
                                                                             as
                                                                             isize)).num_dev_fonts
                                                           as isize);
    ...
    (*dev_font).size =
        tt_get_positive_quad(vf_handle,
                             b"VF\x00" as *const u8 as *const libc::c_char,
                             b"font_size\x00" as *const u8 as
                                 *const libc::c_char);
    (*dev_font).design_size =
        tt_get_positive_quad(vf_handle,
                             b"VF\x00" as *const u8 as *const libc::c_char,
                             b"font_design_size\x00" as *const u8 as
                                 *const libc::c_char);
    ...
    if ttstub_input_read(vf_handle, (*dev_font).directory,
                         dir_length as size_t) != dir_length as libc::c_long {
        _tt_abort(b"directory read failed\x00" as *const u8 as
                      *const libc::c_char);
    }
    (*dev_font).name =
    ...
    *fresh5 = (*fresh5).wrapping_add(1i32 as libc::c_uint);
    ...
    (*dev_font).dev_id =
        dvi_locate_font((*dev_font).name,
                        sqxfw((*vf_fonts.offset(thisfont as isize)).ptsize,
                              (*dev_font).size as fixword)) as libc::c_int;
}
unsafe extern "C" fn process_vf_file(mut vf_handle: rust_input_handle_t,
                                     mut thisfont: libc::c_int) {
    let mut eof: libc::c_int = 0i32;
    let mut code: libc::c_int = 0;
    let mut font_id: uint32_t = 0;
    while eof == 0 {
        code = tt_get_unsigned_byte(vf_handle) as libc::c_int;
        match code {
            243 | 244 | 245 | 246 => {
                font_id =
                    tt_get_unsigned_num(vf_handle,
                                        (code - 243i32) as libc::c_uchar);
                read_a_font_def(vf_handle, font_id as int32_t, thisfont);
            }
            _ => {
                if code < 242i32 {
                    /* For a short packet, code is the pkt_len */
                    let mut ch: uint32_t =
                        tt_get_unsigned_byte(vf_handle) as uint32_t;
                    /* Skip over TFM width since we already know it */
                    tt_skip_bytes(3i32 as libc::c_uint, vf_handle);
                    read_a_char_def(vf_handle, thisfont, code as uint32_t,
                                    ch);
                } else if code == 242i32 {
                    let mut pkt_len: uint32_t =
                        tt_get_positive_quad(vf_handle,
                                             b"VF\x00" as *const u8 as
                                                 *const libc::c_char,
                                             b"pkt_len\x00" as *const u8 as
                                                 *const libc::c_char);
                    let mut ch_0: uint32_t = tt_get_unsigned_quad(vf_handle);
                    /* Skip over TFM width since we already know it */
                    tt_skip_bytes(4i32 as libc::c_uint, vf_handle);
...
#[no_mangle]
pub unsafe extern "C" fn vf_locate_font(mut tex_name: *const libc::c_char,
                                        mut ptsize: spt_t) -> libc::c_int {
    ...
    i = 0i32;
    while (i as libc::c_uint) < num_vf_fonts {
        if streq_ptr((*vf_fonts.offset(i as isize)).tex_name, tex_name) as
               libc::c_int != 0 &&
...
unsafe extern "C" fn unsigned_byte(mut start: *mut *mut libc::c_uchar,
                                   mut end: *mut libc::c_uchar)
 -> libc::c_int {
    let mut byte: libc::c_int = 0i32;
    if *start < end {
        let fresh10 = *start;
        *start = (*start).offset(1);
        byte = *fresh10 as libc::c_int
    } else {
...
unsafe extern "C" fn get_pkt_signed_num(mut start: *mut *mut libc::c_uchar,
                                        mut end: *mut libc::c_uchar,
                                        mut num: libc::c_uchar) -> int32_t {
    ...
    if end.wrapping_offset_from(*start) as libc::c_long > num as libc::c_long
        ...
        *start = (*start).offset(1);
        ...
        if val > 0x7fi32 { val -= 0x100i32 }
        let mut current_block_5: u64;
        match num as libc::c_int {
            3 => {
                let fresh12 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh12 as libc::c_int;
                current_block_5 = 9698575669066167445;
            }
            2 => { current_block_5 = 9698575669066167445; }
            1 => { current_block_5 = 18113473374131038547; }
            _ => { current_block_5 = 13183875560443969876; }
        }
        match current_block_5 {
            9698575669066167445 => {
                let fresh13 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh13 as libc::c_int;
        ...
        match current_block_5 {
            18113473374131038547 => {
                let fresh14 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh14 as libc::c_int
            }
            _ => { }
        }
    } else {
...
unsafe extern "C" fn get_pkt_unsigned_num(mut start: *mut *mut libc::c_uchar,
                                          mut end: *mut libc::c_uchar,
                                          mut num: libc::c_uchar) -> int32_t {
    let mut val: int32_t = 0i32;
    if end.wrapping_offset_from(*start) as libc::c_long > num as libc::c_long
        ...
        *start = (*start).offset(1);
        ...
        match num as libc::c_int {
            3 => {
                ...
                let fresh16 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh16 as libc::c_int;
                current_block_5 = 5559910912116893431;
            }
            2 => { current_block_5 = 5559910912116893431; }
            1 => { current_block_5 = 15700427407090132107; }
            _ => { current_block_5 = 13183875560443969876; }
        }
        match current_block_5 {
            5559910912116893431 => {
                let fresh17 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh17 as libc::c_int;
        ...
        match current_block_5 {
            15700427407090132107 => {
                let fresh18 = *start;
                *start = (*start).offset(1);
                val = val << 8i32 | *fresh18 as libc::c_int
            }
            _ => { }
        }
    } else {
...
unsafe extern "C" fn vf_putrule(mut start: *mut *mut libc::c_uchar,
                                mut end: *mut libc::c_uchar,
                                mut ptsize: spt_t) {
    let mut height: int32_t =
        get_pkt_signed_num(start, end, 3i32 as libc::c_uchar);
    let mut width: int32_t =
        get_pkt_signed_num(start, end, 3i32 as libc::c_uchar);
    dvi_rule(sqxfw(ptsize, width), sqxfw(ptsize, height));
}
unsafe extern "C" fn vf_setrule(mut start: *mut *mut libc::c_uchar,
                                mut end: *mut libc::c_uchar,
                                mut ptsize: spt_t) {
    let mut height: int32_t =
        get_pkt_signed_num(start, end, 3i32 as libc::c_uchar);
    let mut s_width: int32_t =
        sqxfw(ptsize, get_pkt_signed_num(start, end, 3i32 as libc::c_uchar));
    dvi_rule(s_width, sqxfw(ptsize, height));
    dvi_right(s_width);
}
unsafe extern "C" fn vf_fnt(mut font_id: int32_t, mut vf_font: libc::c_int) {
    let mut i: libc::c_int = 0;
    i = 0i32;
    while (i as libc::c_uint) <
              (*vf_fonts.offset(vf_font as isize)).num_dev_fonts {
        if font_id ==
               (*(*vf_fonts.offset(vf_font as
                                       isize)).dev_fonts.offset(i as
                                                                    isize)).font_id
           {
            break ;
        }
    ...
    if (i as libc::c_uint) <
           (*vf_fonts.offset(vf_font as isize)).num_dev_fonts {
        /* Font was found */
        dvi_set_font((*(*vf_fonts.offset(vf_font as
                                             isize)).dev_fonts.offset(i as
                                                                          isize)).dev_id);
    } else {
        fprintf(stderr,
...
unsafe extern "C" fn vf_xxx(mut len: int32_t,
    ...
    if *start <= end.offset(-(len as isize)) {
        let mut buffer: *mut libc::c_uchar =
            new(((len + 1i32) as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        memcpy(buffer as *mut libc::c_void, *start as *const libc::c_void,
               len as libc::c_ulong);
        *buffer.offset(len as isize) = '\u{0}' as i32 as libc::c_uchar;
        ...
        /*
             ...
             */
        if memcmp(p as *mut libc::c_char as *const libc::c_void,
                  b"Warning:\x00" as *const u8 as *const libc::c_char as
                      *const libc::c_void, 8i32 as libc::c_ulong) == 0 {
            if verbose != 0 {
                dpx_warning(b"VF:%s\x00" as *const u8 as *const libc::c_char,
                            p.offset(8));
            }
        } else { dvi_do_special(buffer as *const libc::c_void, len); }
        free(buffer as *mut libc::c_void);
    } else {
        _tt_abort(b"Premature end of DVI byte stream in VF font.\x00" as
                      *const u8 as *const libc::c_char);
    }
    *start = (*start).offset(len as isize);
}
#[no_mangle]
pub unsafe extern "C" fn vf_set_char(mut ch: int32_t,
                                     mut vf_font: libc::c_int) {
    ...
    let mut default_font: libc::c_int = -1i32;
    if (vf_font as libc::c_uint) < num_vf_fonts {
        /* Initialize to the first font or -1 if undefined */
        ptsize = (*vf_fonts.offset(vf_font as isize)).ptsize;
        if (*vf_fonts.offset(vf_font as isize)).num_dev_fonts >
               0i32 as libc::c_uint {
            default_font =
                (*(*vf_fonts.offset(vf_font as
                                        isize)).dev_fonts.offset(0)).dev_id
        }
        dvi_vf_init(default_font);
        if ch as libc::c_uint >=
               (*vf_fonts.offset(vf_font as isize)).num_chars ||
               {
                   start =
                       *(*vf_fonts.offset(vf_font as
                                              isize)).ch_pkt.offset(ch as
                                                                        isize);
                   start.is_null()
               } {
            fprintf(stderr,
                    b"\nchar=0x%x(%d)\n\x00" as *const u8 as
                        *const libc::c_char, ch, ch);
            fprintf(stderr,
                    b"Tried to set a nonexistent character in a virtual font\x00"
                        as *const u8 as *const libc::c_char);
            end = 0 as *mut libc::c_uchar;
            start = end
        } else {
            end =
                start.offset(*(*vf_fonts.offset(vf_font as
                                                    isize)).pkt_len.offset(ch
                                                                               as
                                                                               isize)
                                 as isize)
        }
        while !start.is_null() && start < end {
            ...
            match opcode as libc::c_int {
                128 | 129 | 130 => {
                    dvi_set(get_pkt_unsigned_num(&mut start, end,
                                                 (opcode as libc::c_int -
                                                      128i32) as
                                                     libc::c_uchar));
                }
                131 => {
                    _tt_abort(b"Multibyte (>24 bits) character in VF packet.\nI can\'t handle this!\x00"
                                  as *const u8 as *const libc::c_char);
                }
                132 => { vf_setrule(&mut start, end, ptsize); }
                133 | 134 | 135 => {
                    dvi_put(get_pkt_unsigned_num(&mut start, end,
                                                 (opcode as libc::c_int -
                                                      133i32) as
                                                     libc::c_uchar));
                }
                ...
                137 => { vf_putrule(&mut start, end, ptsize); }
                138 => { }
                141 => { dvi_push(); }
                142 => { dpx_dvi_pop(); }
                143 | 144 | 145 | 146 => {
                    dvi_right(sqxfw(ptsize,
                                    get_pkt_signed_num(&mut start, end,
                                                       (opcode as libc::c_int
                                                            - 143i32) as
                                                           libc::c_uchar)));
                }
                ...
                235 | 236 | 237 | 238 => {
                    vf_fnt(get_pkt_signed_num(&mut start, end,
                                              (opcode as libc::c_int - 235i32)
                                                  as libc::c_uchar), vf_font);
                }
                239 | 240 | 241 | 242 => {
                    let mut len: int32_t =
                        get_pkt_unsigned_num(&mut start, end,
                                             (opcode as libc::c_int - 239i32)
                                                 as libc::c_uchar);
                    if len < 0i32 {
                        dpx_warning(b"VF: Special with %d bytes???\x00" as
                                        *const u8 as *const libc::c_char,
                                    len);
                    } else { vf_xxx(len, &mut start, end); }
                }
                255 => {
                    dvi_dirchg(unsigned_byte(&mut start, end) as
                                   libc::c_uchar);
                }
                _ => {
                    if opcode as libc::c_int <= 127i32 {
                        ...
                        vf_fnt(opcode as libc::c_int - 171i32, vf_font);
                    } else {
                        fprintf(stderr,
        ...
        fprintf(stderr,
                b"vf_set_char: font: %d\x00" as *const u8 as
                    *const libc::c_char, vf_font);
    ...
    while i < num_vf_fonts {
        ...
        if !(*vf_fonts.offset(i as isize)).ch_pkt.is_null() {
            ...
            while j < (*vf_fonts.offset(i as isize)).num_chars {
                free(*(*vf_fonts.offset(i as isize)).ch_pkt.offset(j as isize)
                         as *mut libc::c_void);
                j = j.wrapping_add(1)
            }
        ...
        while j < (*vf_fonts.offset(i as isize)).num_dev_fonts {
            one_font =
                &mut *(*vf_fonts.offset(i as
                                            isize)).dev_fonts.offset(j as
                                                                         isize)
                    as *mut font_def;
