...
unsafe extern "C" fn get_unsigned(mut handle: rust_input_handle_t,
                                  mut n: libc::c_int) -> libc::c_uint {
    ...
    loop  {
        ...
        v =
            v.wrapping_mul(0x100u32).wrapping_add(tt_get_unsigned_byte(handle)
                                                      as libc::c_uint)
    }
    return v;
}
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn cff_open(mut handle: rust_input_handle_t,
                                  mut offset: libc::c_int, mut n: libc::c_int)
 -> *mut cff_font {
    let mut cff: *mut cff_font = 0 as *mut cff_font; /* not used */
    let mut idx: *mut cff_index = 0 as *mut cff_index;
    cff =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<cff_font>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut cff_font;
    ...
    ttstub_input_seek((*cff).handle,
                      (*cff).offset.wrapping_add(0i32 as libc::c_uint) as
                          ssize_t, 0i32);
    ...
    if ((*cff).header.offsize as libc::c_int) < 1i32 ||
           (*cff).header.offsize as libc::c_int > 4i32 {
        _tt_abort(b"invalid offsize data\x00" as *const u8 as
                      *const libc::c_char);
    }
    if (*cff).header.major as libc::c_int > 1i32 ||
           (*cff).header.minor as libc::c_int > 0i32 {
        dpx_warning(b"%s: CFF version %u.%u not supported.\x00" as *const u8
    ...
    ttstub_input_seek((*cff).handle,
                      (*cff).offset.wrapping_add((*cff).header.hdr_size as
                                                     libc::c_uint) as ssize_t,
                      0i32);
    /* Name INDEX */
    idx = cff_get_index(cff);
    if n > (*idx).count as libc::c_int - 1i32 {
        dpx_warning(b"%s: Invalid CFF fontset index number.\x00" as *const u8
                        as *const libc::c_char,
                    b"CFF\x00" as *const u8 as *const libc::c_char);
        cff_close(cff);
        return 0 as *mut cff_font
    ...
    idx = cff_get_index(cff);
    if n > (*idx).count as libc::c_int - 1i32 {
        _tt_abort(b"CFF Top DICT not exist...\x00" as *const u8 as
                      *const libc::c_char);
    }
    (*cff).topdict =
        cff_dict_unpack((*idx).data.offset(*(*idx).offset.offset(n as isize)
                                               as isize).offset(-1),
                        (*idx).data.offset(*(*idx).offset.offset((n + 1i32) as
                                                                     isize) as
                                               isize).offset(-1));
    if (*cff).topdict.is_null() {
        _tt_abort(b"Parsing CFF Top DICT data failed...\x00" as *const u8 as
                      *const libc::c_char);
    }
    cff_release_index(idx);
    if cff_dict_known((*cff).topdict,
                      b"CharstringType\x00" as *const u8 as
                          *const libc::c_char) != 0 &&
           cff_dict_get((*cff).topdict,
                        b"CharstringType\x00" as *const u8 as
                            *const libc::c_char, 0i32) !=
               2i32 as libc::c_double {
        dpx_warning(b"Only Type 2 Charstrings supported...\x00" as *const u8
                        as *const libc::c_char);
        cff_close(cff);
        return 0 as *mut cff_font
    }
    if cff_dict_known((*cff).topdict,
                      b"SyntheticBase\x00" as *const u8 as
                          *const libc::c_char) != 0 {
        dpx_warning(b"CFF Synthetic font not supported.\x00" as *const u8 as
                        *const libc::c_char);
        cff_close(cff);
        return 0 as *mut cff_font
    }
    ...
    /* offset to GSubr */
    (*cff).gsubr_offset =
        ttstub_input_seek((*cff).handle, 0i32 as ssize_t,
                          1i32).wrapping_sub(offset as libc::c_ulong) as
            l_offset;
    /* Number of glyphs */
    offset =
        cff_dict_get((*cff).topdict,
                     ...
                     0i32) as libc::c_int;
    ttstub_input_seek((*cff).handle,
                      (*cff).offset.wrapping_add(offset as libc::c_uint) as
                          ssize_t, 0i32);
    ...
    if cff_dict_known((*cff).topdict,
        ...
        if offset == 0i32 {
            /* predefined */
            (*cff).flag |= 1i32 << 3i32
        } else if offset == 1i32 { (*cff).flag |= 1i32 << 4i32 }
    } else { (*cff).flag |= 1i32 << 3i32 }
    /* Check for charset */
    if cff_dict_known((*cff).topdict,
                      b"charset\x00" as *const u8 as *const libc::c_char) != 0
       {
        offset =
            cff_dict_get((*cff).topdict,
        ...
        if offset == 0i32 {
            ...
            (*cff).flag |= 1i32 << 6i32
        } else if offset == 2i32 { (*cff).flag |= 1i32 << 7i32 }
    } else { (*cff).flag |= 1i32 << 5i32 } /* seek back to GSubr */
    ttstub_input_seek((*cff).handle,
                      (*cff).offset.wrapping_add((*cff).gsubr_offset) as
                          ssize_t, 0i32); /* no trailing '\0' */
...
pub unsafe extern "C" fn cff_close(mut cff: *mut cff_font) {
    ...
    if !cff.is_null() {
        ...
        if !(*cff).name.is_null() { cff_release_index((*cff).name); }
        if !(*cff).topdict.is_null() { cff_release_dict((*cff).topdict); }
        if !(*cff).string.is_null() { cff_release_index((*cff).string); }
        if !(*cff).gsubr.is_null() { cff_release_index((*cff).gsubr); }
        ...
        if !(*cff).cstrings.is_null() { cff_release_index((*cff).cstrings); }
        ...
        if !(*cff)._string.is_null() { cff_release_index((*cff)._string); }
        free(cff as *mut libc::c_void);
    };
}
#[no_mangle]
pub unsafe extern "C" fn cff_get_name(mut cff: *mut cff_font)
 -> *mut libc::c_char {
    ...
    idx = (*cff).name;
    len =
        (*(*idx).offset.offset(((*cff).index + 1i32) as
                                   isize)).wrapping_sub(*(*idx).offset.offset((*cff).index
                                                                                  as
                                                                                  isize));
    fontname =
    ...
    memcpy(fontname as *mut libc::c_void,
           (*idx).data.offset(*(*idx).offset.offset((*cff).index as isize) as
...
#[no_mangle]
pub unsafe extern "C" fn cff_set_name(mut cff: *mut cff_font,
                                      mut name: *mut libc::c_char)
 -> libc::c_int {
    let mut idx: *mut cff_index = 0 as *mut cff_index;
    if strlen(name) > 127i32 as libc::c_ulong {
        _tt_abort(b"FontName string length too large...\x00" as *const u8 as
                      *const libc::c_char);
    }
    if !(*cff).name.is_null() { cff_release_index((*cff).name); }
    idx =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<cff_index>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut cff_index;
    (*cff).name = idx;
    (*idx).count = 1i32 as card16;
    (*idx).offsize = 1i32 as c_offsize;
    (*idx).offset =
        new((2i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<l_offset>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut l_offset;
    *(*idx).offset.offset(0) = 1i32 as l_offset;
    *(*idx).offset.offset(1) =
        strlen(name).wrapping_add(1i32 as libc::c_ulong) as l_offset;
    (*idx).data =
        new((strlen(name) as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>() as
                                                 libc::c_ulong) as uint32_t)
            as *mut card8;
    memmove((*idx).data as *mut libc::c_void, name as *const libc::c_void,
            strlen(name));
    return (5i32 as libc::c_ulong).wrapping_add(strlen(name)) as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn cff_put_header(mut cff: *mut cff_font,
                                        mut dest: *mut card8,
                                        mut destlen: libc::c_int)
 -> libc::c_int {
    if destlen < 4i32 {
        _tt_abort(b"Not enough space available...\x00" as *const u8 as
                      *const libc::c_char);
    }
    let fresh1 = dest;
    dest = dest.offset(1);
...
#[no_mangle]
pub unsafe extern "C" fn cff_get_index_header(mut cff: *mut cff_font)
 -> *mut cff_index {
    let mut idx: *mut cff_index = 0 as *mut cff_index;
    let mut i: card16 = 0;
    ...
    if count as libc::c_int > 0i32 {
        ...
        if ((*idx).offsize as libc::c_int) < 1i32 ||
               (*idx).offsize as libc::c_int > 4i32 {
            _tt_abort(b"invalid offsize data\x00" as *const u8 as
                          *const libc::c_char);
        }
        (*idx).offset =
            new(((count as libc::c_int + 1i32) as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<l_offset>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut l_offset;
        i = 0i32 as card16;
        ...
        if *(*idx).offset.offset(0) != 1i32 as libc::c_uint {
            _tt_abort(b"cff_get_index(): invalid index data\x00" as *const u8
                          as *const libc::c_char);
        }
...
#[no_mangle]
pub unsafe extern "C" fn cff_get_index(mut cff: *mut cff_font)
 -> *mut cff_index {
    let mut idx: *mut cff_index = 0 as *mut cff_index;
    let mut i: card16 = 0;
    let mut count: card16 = 0;
    ...
    if count as libc::c_int > 0i32 {
        ...
        if *(*idx).offset.offset(0) != 1i32 as libc::c_uint {
            _tt_abort(b"Invalid CFF Index offset data\x00" as *const u8 as
                          *const libc::c_char);
        }
        length =
            (*(*idx).offset.offset(count as
                                       isize)).wrapping_sub(*(*idx).offset.offset(0))
                as libc::c_int;
        (*idx).data =
            new((length as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut card8;
        offset = 0i32;
        while length > 0i32 {
            nb_read =
                ttstub_input_read((*cff).handle,
                                  ((*idx).data as
                                       *mut libc::c_char).offset(offset as
                                                                     isize),
                                  length as size_t) as libc::c_int;
            offset += nb_read;
            length -= nb_read
        }
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn cff_pack_index(mut idx: *mut cff_index,
                                        mut dest: *mut card8,
                                        mut destlen: libc::c_int)
 -> libc::c_int {
    ...
    datalen =
        (*(*idx).offset.offset((*idx).count as
                                   isize)).wrapping_sub(1i32 as libc::c_uint)
            as size_t;
    if destlen < len {
        _tt_abort(b"Not enough space available...\x00" as *const u8 as
                      *const libc::c_char);
    ...
    *fresh6 = ((*idx).count as libc::c_int & 0xffi32) as card8;
    if datalen < 0xffu64 {
        ...
        *fresh7 = 1i32 as card8;
        ...
        while i as libc::c_int <= (*idx).count as libc::c_int {
            ...
            dest = dest.offset(1);
            *fresh8 =
                (*(*idx).offset.offset(i as isize) & 0xffi32 as libc::c_uint)
                    as card8;
            i = i.wrapping_add(1)
        }
        ...
        *fresh9 = 2i32 as card8;
        ...
        while i as libc::c_int <= (*idx).count as libc::c_int {
            ...
            dest = dest.offset(1);
            *fresh10 =
                (*(*idx).offset.offset(i as isize) >> 8i32 &
                     0xffi32 as libc::c_uint) as card8;
            let fresh11 = dest;
            dest = dest.offset(1);
            *fresh11 =
                (*(*idx).offset.offset(i as isize) & 0xffi32 as libc::c_uint)
        ...
        *fresh12 = 3i32 as card8;
        ...
        while i as libc::c_int <= (*idx).count as libc::c_int {
            ...
            dest = dest.offset(1);
            *fresh13 =
                (*(*idx).offset.offset(i as isize) >> 16i32 &
                     0xffi32 as libc::c_uint) as card8;
            let fresh14 = dest;
            dest = dest.offset(1);
            *fresh14 =
        ...
        *fresh16 = 4i32 as card8;
        ...
        while i as libc::c_int <= (*idx).count as libc::c_int {
            ...
            dest = dest.offset(1);
            *fresh17 =
                (*(*idx).offset.offset(i as isize) >> 24i32 &
                     0xffi32 as libc::c_uint) as card8;
            let fresh18 = dest;
            dest = dest.offset(1);
            *fresh18 =
    ...
    memmove(dest as *mut libc::c_void, (*idx).data as *const libc::c_void,
            (*(*idx).offset.offset((*idx).count as
...
#[no_mangle]
pub unsafe extern "C" fn cff_index_size(mut idx: *mut cff_index)
 -> libc::c_int {
    if (*idx).count as libc::c_int > 0i32 {
        ...
        if (datalen as libc::c_ulong) < 0xffu64 {
            ...
            (*idx).offsize = 3i32 as c_offsize
        } else { (*idx).offsize = 4i32 as c_offsize }
        return ((3i32 +
                     (*idx).offsize as libc::c_int *
                         ((*idx).count as libc::c_int + 1i32)) as
                    libc::c_uint).wrapping_add(datalen) as libc::c_int
    } else { return 2i32 };
}
#[no_mangle]
    ...
    if count as libc::c_int > 0i32 {
        (*idx).offset =
            new(((count as libc::c_int + 1i32) as uint32_t as
                    ...
                    uint32_t) as *mut l_offset;
        *(*idx).offset.offset(0) = 1i32 as l_offset
    } else { (*idx).offset = 0 as *mut l_offset }
    (*idx).data = 0 as *mut card8;
    return idx;
}
#[no_mangle]
pub unsafe extern "C" fn cff_release_index(mut idx: *mut cff_index) {
    if !idx.is_null() {
        if !(*idx).data.is_null() { free((*idx).data as *mut libc::c_void); }
...
/* Strings */
#[no_mangle]
pub unsafe extern "C" fn cff_get_string(mut cff: *mut cff_font, mut id: s_SID)
 -> *mut libc::c_char {
    ...
    if (id as libc::c_int) < 391i32 {
        ...
        *result.offset(len as isize) = '\u{0}' as i32 as libc::c_char
        ...
        if (id as libc::c_int) < (*strings).count as libc::c_int {
            len =
                (*(*strings).offset.offset((id as libc::c_int + 1i32) as
                                               isize)).wrapping_sub(*(*strings).offset.offset(id
                                                                                                  as
                                                                                                  isize))
                    as libc::c_int;
            result =
                new(((len + 1i32) as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_char>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut libc::c_char;
            memmove(result as *mut libc::c_void,
                    (*strings).data.offset(*(*strings).offset.offset(id as
                                                                         isize)
                                               as isize).offset(-1) as
...
#[no_mangle]
pub unsafe extern "C" fn cff_get_sid(mut cff: *mut cff_font,
                                     mut str: *const libc::c_char)
 -> libc::c_int {
    let mut i: card16 = 0;
    if cff.is_null() || str.is_null() { return -1i32 }
    ...
    if !cff.is_null() && !(*cff).string.is_null() {
        ...
        while (i as libc::c_int) < (*idx).count as libc::c_int {
            if strlen(str) ==
                   (*(*idx).offset.offset((i as libc::c_int + 1i32) as
                                              isize)).wrapping_sub(*(*idx).offset.offset(i
                                                                                             as
                                                                                             isize))
                       as libc::c_ulong &&
                   memcmp(str as *const libc::c_void,
                          (*idx).data.offset(*(*idx).offset.offset(i as isize)
                                                 as isize).offset(-1) as
                              *const libc::c_void, strlen(str)) == 0 {
    ...
    while (i as libc::c_int) < 391i32 {
        if streq_ptr(str, cff_stdstr[i as usize]) { return i as libc::c_int }
        i = i.wrapping_add(1)
    }
    return -1i32;
}
#[no_mangle]
pub unsafe extern "C" fn cff_get_seac_sid(mut cff: *mut cff_font,
                                          mut str: *const libc::c_char)
 -> libc::c_int {
    let mut i: card16 = 0;
    if cff.is_null() || str.is_null() { return -1i32 }
...
unsafe extern "C" fn cff_match_string(mut cff: *mut cff_font,
                                      mut str: *const libc::c_char,
                                      mut sid: s_SID) -> libc::c_int {
    let mut i: card16 = 0;
    if (sid as libc::c_int) < 391i32 {
        ...
        if strlen(str) ==
               (*(*(*cff).string).offset.offset((i as libc::c_int + 1i32) as
                                                    isize)).wrapping_sub(*(*(*cff).string).offset.offset(i
                                                                                                             as
                                                                                                             isize))
                   as libc::c_ulong {
            return if memcmp(str as *const libc::c_void,
                             (*(*cff).string).data.offset(*(*(*cff).string).offset.offset(i
                                                                                              as
                                                                                              isize)
                                                              as
                                                              isize).offset(-1)
                                 as *const libc::c_void, strlen(str)) == 0 {
                       1i32
                   } else { 0i32 }
        }
    }
...
pub unsafe extern "C" fn cff_update_string(mut cff: *mut cff_font) {
    if cff.is_null() {
        _tt_abort(b"CFF font not opened.\x00" as *const u8 as
                      *const libc::c_char);
    }
    if !(*cff).string.is_null() { cff_release_index((*cff).string); }
...
/* String */
#[no_mangle]
pub unsafe extern "C" fn cff_add_string(mut cff: *mut cff_font,
                                        mut str: *const libc::c_char,
                                        mut unique: libc::c_int) -> s_SID 
 /* Setting unique == 1 eliminates redundant or predefined strings. */
 {
    ...
    if unique != 0 {
        ...
        while (idx as libc::c_int) < 391i32 {
            if streq_ptr(cff_stdstr[idx as usize], str) { return idx }
            idx = idx.wrapping_add(1)
        }
        idx = 0i32 as card16;
        while (idx as libc::c_int) < (*strings).count as libc::c_int {
            size =
                (*(*strings).offset.offset((idx as libc::c_int + 1i32) as
                                               isize)).wrapping_sub(*(*strings).offset.offset(idx
                                                                                                  as
                                                                                                  isize));
            offset = *(*strings).offset.offset(idx as isize);
            if size as libc::c_ulong == len &&
                   memcmp((*strings).data.offset(offset as isize).offset(-1)
                              as *const libc::c_void,
                          str as *const libc::c_void, len) == 0 {
    ...
    (*strings).offset =
        renew((*strings).offset as *mut libc::c_void,
              (((*strings).count as libc::c_int + 2i32) as uint32_t as
                   libc::c_ulong).wrapping_mul(::std::mem::size_of::<l_offset>()
                                                   as libc::c_ulong) as
                  uint32_t) as *mut l_offset;
    if (*strings).count as libc::c_int == 0i32 {
    ...
    *(*strings).offset.offset((*strings).count as isize) =
    ...
    (*strings).data =
        renew((*strings).data as *mut libc::c_void,
              ((offset as
                    libc::c_ulong).wrapping_add(len).wrapping_sub(1i32 as
                                                                      libc::c_ulong)
                   as uint32_t as
                   libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                   as libc::c_ulong) as
                  uint32_t) as *mut card8;
    memcpy((*strings).data.offset(offset as isize).offset(-1) as
               *mut libc::c_void, str as *const libc::c_void, len);
    return (idx as libc::c_int + 391i32) as s_SID;
}
/*
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn cff_read_encoding(mut cff: *mut cff_font)
 -> libc::c_int {
    ...
    let mut i: card8 = 0;
    if (*cff).topdict.is_null() {
        _tt_abort(b"Top DICT data not found\x00" as *const u8 as
                      *const libc::c_char);
    }
    if cff_dict_known((*cff).topdict,
                      b"Encoding\x00" as *const u8 as *const libc::c_char) ==
    ...
    match (*encoding).format as libc::c_int & !0x80i32 {
        0 => {
            ...
            (*encoding).data.codes =
                new(((*encoding).num_entries as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut card8;
            i = 0i32 as card8;
            while (i as libc::c_int) < (*encoding).num_entries as libc::c_int
                  {
                *(*encoding).data.codes.offset(i as isize) =
        ...
        1 => {
            ...
            while (i as libc::c_int) < (*encoding).num_entries as libc::c_int
                  {
                (*ranges.offset(i as isize)).first =
                    tt_get_unsigned_byte((*cff).handle) as s_SID;
                (*ranges.offset(i as isize)).n_left =
                    tt_get_unsigned_byte((*cff).handle);
                i = i.wrapping_add(1)
            }
        ...
        _ => {
            ...
            _tt_abort(b"Unknown Encoding format\x00" as *const u8 as
                          *const libc::c_char);
        }
    }
    /* Supplementary data */
    if (*encoding).format as libc::c_int & 0x80i32 != 0 {
        ...
        map =
            new(((*encoding).num_supps as uint32_t as
        ...
        while (i as libc::c_int) < (*encoding).num_supps as libc::c_int {
            (*map.offset(i as isize)).code =
                tt_get_unsigned_byte((*cff).handle);
            (*map.offset(i as isize)).glyph =
...
#[no_mangle]
pub unsafe extern "C" fn cff_pack_encoding(mut cff: *mut cff_font,
                                           mut dest: *mut card8,
                                           mut destlen: libc::c_int)
 -> libc::c_int {
    let mut len: libc::c_int = 0i32;
    ...
    if (*cff).flag & (1i32 << 3i32 | 1i32 << 4i32) != 0 ||
           (*cff).encoding.is_null() {
        return 0i32
    }
    if destlen < 2i32 {
        _tt_abort(b"in cff_pack_encoding(): Buffer overflow\x00" as *const u8
                      as *const libc::c_char);
    }
    ...
    match (*encoding).format as libc::c_int & !0x80i32 {
        0 => {
            if destlen < len + (*encoding).num_entries as libc::c_int {
                _tt_abort(b"in cff_pack_encoding(): Buffer overflow\x00" as
        ...
        1 => {
            ...
            while (i as libc::c_int) < (*encoding).num_entries as libc::c_int
                ...
                *dest.offset(fresh24 as isize) =
                    ((*(*encoding).data.range1.offset(i as isize)).first as
                         libc::c_int & 0xffi32) as card8;
                let fresh25 = len;
                len = len + 1;
    ...
    if (*encoding).format as libc::c_int & 0x80i32 != 0 {
        ...
        *dest.offset(fresh26 as isize) = (*encoding).num_supps;
        ...
        while (i as libc::c_int) < (*encoding).num_supps as libc::c_int {
            let fresh27 = len;
            len = len + 1;
            *dest.offset(fresh27 as isize) =
                (*(*encoding).supp.offset(i as isize)).code;
            let fresh28 = len;
            len = len + 1;
            *dest.offset(fresh28 as isize) =
                ((*(*encoding).supp.offset(i as isize)).glyph as libc::c_int
                     >> 8i32 & 0xffi32) as card8;
            let fresh29 = len;
            len = len + 1;
...
#[no_mangle]
pub unsafe extern "C" fn cff_encoding_lookup(mut cff: *mut cff_font,
                                             mut code: card8) -> card16 {
    let mut gid: card16 = 0i32 as card16;
    let mut encoding: *mut cff_encoding = 0 as *mut cff_encoding;
    let mut i: card16 = 0;
    if (*cff).flag & (1i32 << 3i32 | 1i32 << 4i32) != 0 {
        _tt_abort(b"Predefined CFF encoding not supported yet\x00" as
                      *const u8 as *const libc::c_char);
    } else {
        if (*cff).encoding.is_null() {
            _tt_abort(b"Encoding data not available\x00" as *const u8 as
                          *const libc::c_char);
        }
    }
    encoding = (*cff).encoding;
        ...
        0 => {
            ...
            while (i as libc::c_int) < (*encoding).num_entries as libc::c_int
                ...
                if code as libc::c_int ==
                       *(*encoding).data.codes.offset(i as isize) as
                           libc::c_int {
            ...
            while (i as libc::c_int) < (*encoding).num_entries as libc::c_int
                ...
                if code as libc::c_int >=
                       (*(*encoding).data.range1.offset(i as isize)).first as
                           libc::c_int &&
                       code as libc::c_int <=
                           (*(*encoding).data.range1.offset(i as isize)).first
                               as libc::c_int +
                               (*(*encoding).data.range1.offset(i as
                                                                    isize)).n_left
                                   as libc::c_int {
                    gid =
                        (gid as libc::c_int +
    ...
    if gid as libc::c_int == 0i32 &&
           (*encoding).format as libc::c_int & 0x80i32 != 0 {
        let mut map: *mut cff_map = 0 as *mut cff_map;
        if (*encoding).supp.is_null() {
            _tt_abort(b"No CFF supplementary encoding data read.\x00" as
                          *const u8 as *const libc::c_char);
        }
        map = (*encoding).supp;
        i = 0i32 as card16;
        while (i as libc::c_int) < (*encoding).num_supps as libc::c_int {
            if code as libc::c_int ==
                   (*map.offset(i as isize)).code as libc::c_int {
                gid =
                    cff_charsets_lookup(cff, (*map.offset(i as isize)).glyph);
                break ;
            } else { i = i.wrapping_add(1) }
        }
    }
...
#[no_mangle]
pub unsafe extern "C" fn cff_release_encoding(mut encoding:
                                                  *mut cff_encoding) {
    if !encoding.is_null() {
        match (*encoding).format as libc::c_int & !0x80i32 {
            0 => { free((*encoding).data.codes as *mut libc::c_void); }
            1 => { free((*encoding).data.range1 as *mut libc::c_void); }
            _ => {
                _tt_abort(b"Unknown Encoding format.\x00" as *const u8 as
                              *const libc::c_char);
            }
...
#[no_mangle]
pub unsafe extern "C" fn cff_read_charsets(mut cff: *mut cff_font)
 -> libc::c_int {
    ...
    let mut i: card16 = 0;
    if (*cff).topdict.is_null() {
        _tt_abort(b"Top DICT not available\x00" as *const u8 as
                      *const libc::c_char);
    }
    if cff_dict_known((*cff).topdict,
                      b"charset\x00" as *const u8 as *const libc::c_char) == 0
    ...
    match (*charset).format as libc::c_int {
        0 => {
            (*charset).num_entries =
                ((*cff).num_glyphs as libc::c_int - 1i32) as
                    card16; /* no .notdef */
            (*charset).data.glyphs =
                new(((*charset).num_entries as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<s_SID>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut s_SID; /* no-overrap */
            length +=
                (*charset).num_entries as libc::c_int *
                    2i32; /* non-overrapping */
            i = 0i32 as card16; /* or CID */
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                *(*charset).data.glyphs.offset(i as isize) =
                    tt_get_unsigned_pair((*cff).handle);
                i = i.wrapping_add(1)
            }
        ...
        1 => {
            ...
            while count as libc::c_int > 0i32 &&
                      ((*charset).num_entries as libc::c_int) <
                          (*cff).num_glyphs as libc::c_int {
                ranges =
                    renew(ranges as *mut libc::c_void,
                ...
                count =
                    (count as libc::c_int -
                         ((*ranges.offset((*charset).num_entries as
                                              isize)).n_left as libc::c_int +
                              1i32)) as card16;
                (*charset).num_entries =
        ...
        2 => {
            ...
            while count as libc::c_int > 0i32 &&
                      ((*charset).num_entries as libc::c_int) <
                          (*cff).num_glyphs as libc::c_int {
                ranges_0 =
                    renew(ranges_0 as *mut libc::c_void,
                          (((*charset).num_entries as libc::c_int + 1i32) as
                               uint32_t as
                               libc::c_ulong).wrapping_mul(::std::mem::size_of::<cff_range2>()
                                                               as
                                                               libc::c_ulong)
                              as uint32_t) as *mut cff_range2;
                ...
                count =
                    (count as libc::c_int -
                         ((*ranges_0.offset((*charset).num_entries as
                                                isize)).n_left as libc::c_int
                              + 1i32)) as card16;
                (*charset).num_entries =
                    ((*charset).num_entries as libc::c_int + 1i32) as card16
        ...
        _ => {
            ...
            _tt_abort(b"Unknown Charset format\x00" as *const u8 as
                          *const libc::c_char);
        }
    }
    if count as libc::c_int > 0i32 {
        _tt_abort(b"Charset data possibly broken\x00" as *const u8 as
                      *const libc::c_char);
    }
    return length;
}
#[no_mangle]
pub unsafe extern "C" fn cff_pack_charsets(mut cff: *mut cff_font,
                                           mut dest: *mut card8,
                                           mut destlen: libc::c_int)
 -> libc::c_int {
    let mut len: libc::c_int = 0i32;
    ...
    if (*cff).flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 ||
           (*cff).charsets.is_null() {
        return 0i32
    }
    if destlen < 1i32 {
        _tt_abort(b"in cff_pack_charsets(): Buffer overflow\x00" as *const u8
                      as *const libc::c_char);
    }
    ...
    match (*charset).format as libc::c_int {
        0 => {
            if destlen < len + (*charset).num_entries as libc::c_int * 2i32 {
                _tt_abort(b"in cff_pack_charsets(): Buffer overflow\x00" as
                              *const u8 as *const libc::c_char);
            }
            i = 0i32 as card16;
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                ...
                let fresh31 = len;
                len = len + 1;
                *dest.offset(fresh31 as isize) =
                    (sid as libc::c_int >> 8i32 & 0xffi32) as card8;
                let fresh32 = len;
                len = len + 1;
                *dest.offset(fresh32 as isize) =
                    (sid as libc::c_int & 0xffi32) as card8;
                i = i.wrapping_add(1)
            }
        }
        1 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                ...
                *dest.offset(fresh33 as isize) =
                    ((*(*charset).data.range1.offset(i as isize)).first as
                         libc::c_int >> 8i32 & 0xffi32) as card8;
                let fresh34 = len;
                len = len + 1;
        ...
        2 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                ...
                *dest.offset(fresh36 as isize) =
                    ((*(*charset).data.range2.offset(i as isize)).first as
                         libc::c_int >> 8i32 & 0xffi32) as card8;
                let fresh37 = len;
                len = len + 1;
                ...
                *dest.offset(fresh38 as isize) =
                    ((*(*charset).data.range2.offset(i as isize)).n_left as
                         libc::c_int >> 8i32 & 0xffi32) as card8;
                let fresh39 = len;
                len = len + 1;
...
#[no_mangle]
pub unsafe extern "C" fn cff_get_glyphname(mut cff: *mut cff_font,
                                           mut gid: card16)
 -> *mut libc::c_char {
    let mut sid: s_SID = 0;
...
#[no_mangle]
pub unsafe extern "C" fn cff_glyph_lookup(mut cff: *mut cff_font,
 ...
 -> card16 {
    ...
    let mut n: card16 = 0;
    if (*cff).flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 {
        _tt_abort(b"Predefined CFF charsets not supported yet\x00" as
                      *const u8 as *const libc::c_char);
    } else {
        if (*cff).charsets.is_null() {
            _tt_abort(b"Charsets data not available\x00" as *const u8 as
                          *const libc::c_char);
        }
    }
    ...
    if glyph.is_null() ||
           streq_ptr(glyph,
                     b".notdef\x00" as *const u8 as *const libc::c_char) as
               libc::c_int != 0 {
    ...
    match (*charset).format as libc::c_int {
        0 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                ...
                if cff_match_string(cff, glyph,
                                    *(*charset).data.glyphs.offset(i as
                                                                       isize))
                       != 0 {
                    return gid
                }
        ...
        1 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                ...
                while n as libc::c_int <=
                    ...
                    gid = gid.wrapping_add(1);
                    if cff_match_string(cff, glyph,
                                        ((*(*charset).data.range1.offset(i as
                                                                             isize)).first
                                             as libc::c_int +
                                             n as libc::c_int) as s_SID) != 0
                       {
                        return gid
                    }
                    n = n.wrapping_add(1)
        ...
        2 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                n = 0i32 as card16;
                while n as libc::c_int <=
                          (*(*charset).data.range2.offset(i as isize)).n_left
                              as libc::c_int {
                    gid = gid.wrapping_add(1);
                    if cff_match_string(cff, glyph,
                                        ((*(*charset).data.range2.offset(i as
                                                                             isize)).first
                                             as libc::c_int +
                                             n as libc::c_int) as s_SID) != 0
                       {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn cff_charsets_lookup(mut cff: *mut cff_font,
                                             mut cid: card16) -> card16 {
    if (*cff).flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 {
        _tt_abort(b"Predefined CFF charsets not supported yet\x00" as
                      *const u8 as *const libc::c_char);
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn cff_charsets_lookup_gid(mut charset:
                                                     *mut cff_charsets,
                                                 mut cid: card16) -> card16 {
    let mut gid: card16 = 0i32 as card16;
    let mut i: card16 = 0;
        ...
        0 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                if cid as libc::c_int ==
                       *(*charset).data.glyphs.offset(i as isize) as
                           libc::c_int {
                    gid = (i as libc::c_int + 1i32) as card16;
                    return gid
                }
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                if cid as libc::c_int >=
                       (*(*charset).data.range1.offset(i as isize)).first as
                           libc::c_int &&
                       cid as libc::c_int <=
                           (*(*charset).data.range1.offset(i as isize)).first
                               as libc::c_int +
                               (*(*charset).data.range1.offset(i as
                                                                   isize)).n_left
                                   as libc::c_int {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                if cid as libc::c_int >=
                       (*(*charset).data.range2.offset(i as isize)).first as
                           libc::c_int &&
                       cid as libc::c_int <=
                           (*(*charset).data.range2.offset(i as isize)).first
                               as libc::c_int +
                               (*(*charset).data.range2.offset(i as
                                                                   isize)).n_left
                                   as libc::c_int {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn cff_charsets_lookup_inverse(mut cff: *mut cff_font,
                                                     mut gid: card16)
 -> card16 {
    if (*cff).flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 {
        _tt_abort(b"Predefined CFF charsets not supported yet\x00" as
...
#[no_mangle]
pub unsafe extern "C" fn cff_charsets_lookup_cid(mut charset:
                                                     *mut cff_charsets,
                                                 mut gid: card16) -> card16 {
    let mut sid: card16 = 0i32 as card16;
    let mut i: card16 = 0;
    match (*charset).format as libc::c_int {
        0 => {
            if gid as libc::c_int - 1i32 >=
                   (*charset).num_entries as libc::c_int {
                _tt_abort(b"Invalid GID.\x00" as *const u8 as
                              *const libc::c_char);
            }
            sid =
        ...
        1 => {
            ...
            while (i as libc::c_int) < (*charset).num_entries as libc::c_int {
                if gid as libc::c_int <=
                       (*(*charset).data.range1.offset(i as isize)).n_left as
                           libc::c_int + 1i32 {
                    sid =
                        (gid as libc::c_int +
                             (*(*charset).data.range1.offset(i as
...
#[no_mangle]
pub unsafe extern "C" fn cff_release_charsets(mut charset:
                                                  *mut cff_charsets) {
    if !charset.is_null() {
        match (*charset).format as libc::c_int {
            0 => { free((*charset).data.glyphs as *mut libc::c_void); }
            1 => { free((*charset).data.range1 as *mut libc::c_void); }
            2 => { free((*charset).data.range2 as *mut libc::c_void); }
            _ => { }
        }
        free(charset as *mut libc::c_void);
    };
...
/* CID-Keyed font specific */
#[no_mangle]
pub unsafe extern "C" fn cff_read_fdselect(mut cff: *mut cff_font)
 -> libc::c_int {
    ...
    if (*cff).flag & 1i32 << 0i32 == 0 { return 0i32 }
    offset =
        cff_dict_get((*cff).topdict,
    ...
    match (*fdsel).format as libc::c_int {
        0 => {
            ...
            (*fdsel).data.fds =
                new(((*fdsel).num_entries as uint32_t as
                         libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                         as libc::c_ulong) as
                        uint32_t) as *mut card8;
            i = 0i32 as card16;
            while (i as libc::c_int) < (*fdsel).num_entries as libc::c_int {
                *(*fdsel).data.fds.offset(i as isize) =
                    tt_get_unsigned_byte((*cff).handle);
                i = i.wrapping_add(1)
            }
        ...
        3 => {
            ...
            while (i as libc::c_int) < (*fdsel).num_entries as libc::c_int {
                (*ranges.offset(i as isize)).first =
                    tt_get_unsigned_pair((*cff).handle);
                (*ranges.offset(i as isize)).fd =
                    tt_get_unsigned_byte((*cff).handle);
                i = i.wrapping_add(1)
            }
            if (*ranges.offset(0)).first as libc::c_int != 0i32 {
                _tt_abort(b"Range not starting with 0.\x00" as *const u8 as
                              *const libc::c_char);
            }
            if (*cff).num_glyphs as libc::c_int !=
                   tt_get_unsigned_pair((*cff).handle) as libc::c_int {
                _tt_abort(b"Sentinel value mismatched with number of glyphs.\x00"
        ...
        _ => {
            ...
            _tt_abort(b"Unknown FDSelect format.\x00" as *const u8 as
                          *const libc::c_char);
        }
    }
    return length;
...
#[no_mangle]
pub unsafe extern "C" fn cff_pack_fdselect(mut cff: *mut cff_font,
                                           mut dest: *mut card8,
                                           mut destlen: libc::c_int)
 -> libc::c_int {
    let mut fdsel: *mut cff_fdselect = 0 as *mut cff_fdselect;
    let mut len: libc::c_int = 0i32;
    let mut i: card16 = 0;
    if (*cff).fdselect.is_null() { return 0i32 }
    if destlen < 1i32 {
        _tt_abort(b"in cff_pack_fdselect(): Buffur overflow\x00" as *const u8
                      as *const libc::c_char);
    }
    fdsel = (*cff).fdselect;
    let fresh40 = len;
    ...
    *dest.offset(fresh40 as isize) = (*fdsel).format;
    match (*fdsel).format as libc::c_int {
        0 => {
            if (*fdsel).num_entries as libc::c_int !=
                   (*cff).num_glyphs as libc::c_int {
                _tt_abort(b"in cff_pack_fdselect(): Invalid data\x00" as
                              *const u8 as *const libc::c_char);
            }
            if destlen < len + (*fdsel).num_entries as libc::c_int {
                _tt_abort(b"in cff_pack_fdselect(): Buffer overflow\x00" as
                              *const u8 as *const libc::c_char);
            }
            i = 0i32 as card16;
            while (i as libc::c_int) < (*fdsel).num_entries as libc::c_int {
        ...
        3 => {
            ...
            while (i as libc::c_int) < (*fdsel).num_entries as libc::c_int {
                if destlen < len + 3i32 {
                    _tt_abort(b"in cff_pack_fdselect(): Buffer overflow\x00"
                                  as *const u8 as *const libc::c_char);
                }
                let fresh42 = len;
                len = len + 1;
                *dest.offset(fresh42 as isize) =
                    ((*(*fdsel).data.ranges.offset(i as isize)).first as
                         libc::c_int >> 8i32 & 0xffi32) as card8;
                let fresh43 = len;
                len = len + 1;
                *dest.offset(fresh43 as isize) =
                ...
                let fresh44 = len;
                len = len + 1;
                *dest.offset(fresh44 as isize) =
                    (*(*fdsel).data.ranges.offset(i as isize)).fd;
                i = i.wrapping_add(1)
            }
            if destlen < len + 2i32 {
                _tt_abort(b"in cff_pack_fdselect(): Buffer overflow\x00" as
            ...
            *dest.offset(fresh45 as isize) =
            ...
            let fresh46 = len;
            len = len + 1;
            *dest.offset(fresh46 as isize) =
                ((*cff).num_glyphs as libc::c_int & 0xffi32) as card8;
...
#[no_mangle]
pub unsafe extern "C" fn cff_release_fdselect(mut fdselect:
                                                  *mut cff_fdselect) {
    if !fdselect.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn cff_fdselect_lookup(mut cff: *mut cff_font,
                                             mut gid: card16) -> card8 {
    let mut fd: card8 = 0xffi32 as card8;
    let mut fdsel: *mut cff_fdselect = 0 as *mut cff_fdselect;
    if (*cff).fdselect.is_null() {
        _tt_abort(b"in cff_fdselect_lookup(): FDSelect not available\x00" as
                      *const u8 as *const libc::c_char);
    }
    fdsel = (*cff).fdselect;
    if gid as libc::c_int >= (*cff).num_glyphs as libc::c_int {
        _tt_abort(b"in cff_fdselect_lookup(): Invalid glyph index\x00" as
                      *const u8 as *const libc::c_char);
    }
    match (*fdsel).format as libc::c_int {
        ...
        3 => {
            if gid as libc::c_int == 0i32 {
                ...
                fd =
                    (*(*fdsel).data.ranges.offset((i as libc::c_int - 1i32) as
                                                      isize)).fd
            }
        }
        _ => {
            _tt_abort(b"in cff_fdselect_lookup(): Invalid FDSelect format\x00"
                          as *const u8 as *const libc::c_char);
        }
    }
    if fd as libc::c_int >= (*cff).num_fds as libc::c_int {
        _tt_abort(b"in cff_fdselect_lookup(): Invalid Font DICT index\x00" as
                      *const u8 as *const libc::c_char);
    }
    return fd;
}
#[no_mangle]
pub unsafe extern "C" fn cff_read_subrs(mut cff: *mut cff_font)
 -> libc::c_int {
    let mut len: libc::c_int = 0i32;
    let mut offset: libc::c_int = 0;
    ...
    if (*cff).private.is_null() { cff_read_private(cff); }
    ...
    (*cff).subrs =
        new(((*cff).num_fds as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut cff_index>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut *mut cff_index;
    if (*cff).flag & 1i32 << 0i32 != 0 {
        i = 0i32;
        while i < (*cff).num_fds as libc::c_int {
            if (*(*cff).private.offset(i as isize)).is_null() ||
                   cff_dict_known(*(*cff).private.offset(i as isize),
                                  b"Subrs\x00" as *const u8 as
                                      *const libc::c_char) == 0 {
                let ref mut fresh47 = *(*cff).subrs.offset(i as isize);
                *fresh47 = 0 as *mut cff_index
            } else {
                offset =
                    cff_dict_get(*(*cff).fdarray.offset(i as isize),
                                 b"Private\x00" as *const u8 as
                                     *const libc::c_char, 1i32) as
                        libc::c_int;
                offset =
                    (offset as libc::c_double +
                         cff_dict_get(*(*cff).private.offset(i as isize),
                                      b"Subrs\x00" as *const u8 as
                                          *const libc::c_char, 0i32)) as
                        libc::c_int;
                ttstub_input_seek((*cff).handle,
                                  (*cff).offset.wrapping_add(offset as
                                                                 libc::c_uint)
                                      as ssize_t, 0i32);
            ...
            i += 1
                  ...
                  cff_dict_known(*(*cff).private.offset(0),
                                 b"Subrs\x00" as *const u8 as
                                     *const libc::c_char) == 0 {
        ...
        *fresh49 = 0 as *mut cff_index
        ...
        offset =
            (offset as libc::c_double +
                 cff_dict_get(*(*cff).private.offset(0),
                              b"Subrs\x00" as *const u8 as
                                  *const libc::c_char, 0i32)) as libc::c_int;
        ttstub_input_seek((*cff).handle,
                          (*cff).offset.wrapping_add(offset as libc::c_uint)
                              as ssize_t, 0i32);
...
#[no_mangle]
pub unsafe extern "C" fn cff_read_fdarray(mut cff: *mut cff_font)
 -> libc::c_int {
    let mut len: libc::c_int = 0i32;
    let mut idx: *mut cff_index = 0 as *mut cff_index;
    let mut offset: libc::c_int = 0;
    ...
    let mut i: card16 = 0;
    if (*cff).topdict.is_null() {
        _tt_abort(b"in cff_read_fdarray(): Top DICT not found\x00" as
                      *const u8 as *const libc::c_char);
    }
    if (*cff).flag & 1i32 << 0i32 == 0 { return 0i32 }
    /* must exist */
    ...
    (*cff).fdarray =
        new(((*idx).count as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut cff_dict>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut *mut cff_dict;
    i = 0i32 as card16;
    while (i as libc::c_int) < (*idx).count as libc::c_int {
        let mut data: *mut card8 =
            (*idx).data.offset(*(*idx).offset.offset(i as isize) as
                                   isize).offset(-1);
        size =
            (*(*idx).offset.offset((i as libc::c_int + 1i32) as
                                       isize)).wrapping_sub(*(*idx).offset.offset(i
...
/* Private DICT(s) */
#[no_mangle]
pub unsafe extern "C" fn cff_read_private(mut cff: *mut cff_font)
 -> libc::c_int {
    let mut len: libc::c_int = 0i32;
    ...
    if (*cff).flag & 1i32 << 0i32 != 0 {
        let mut i: libc::c_int = 0;
        if (*cff).fdarray.is_null() { cff_read_fdarray(cff); }
        (*cff).private =
            new(((*cff).num_fds as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut cff_dict>()
                                                     as libc::c_ulong) as
        ...
        while i < (*cff).num_fds as libc::c_int {
            if !(*(*cff).fdarray.offset(i as isize)).is_null() &&
                   cff_dict_known(*(*cff).fdarray.offset(i as isize),
                                  b"Private\x00" as *const u8 as
                                      *const libc::c_char) != 0 &&
                   {
                       size =
                ...
                if ttstub_input_read((*cff).handle, data as *mut libc::c_char,
                                     size as size_t) != size as libc::c_long {
                    _tt_abort(b"reading file failed\x00" as *const u8 as
                                  *const libc::c_char);
                }
