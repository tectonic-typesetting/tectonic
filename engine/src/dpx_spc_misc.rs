 ...
 */
    ...
    #[no_mangle]
    fn mps_scan_bbox(pp: *mut *const libc::c_char,
                     endptr: *const libc::c_char, bbox: *mut pdf_rect)
     -> libc::c_int;
    #[no_mangle]
...
*/
unsafe extern "C" fn spc_handler_postscriptbox(mut spe: *mut spc_env,
                                               mut ap: *mut spc_arg)
 -> libc::c_int {
    ...
    if !spe.is_null() && !ap.is_null() {
        ...
        __assert_fail(b"spe && ap\x00" as *const u8 as *const libc::c_char,
                      b"dpx-spc_misc.c\x00" as *const u8 as
                          *const libc::c_char, 51i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 66],
                                                &[libc::c_char; 66]>(b"int spc_handler_postscriptbox(struct spc_env *, struct spc_arg *)\x00")).as_ptr());
    }
    if (*ap).curptr >= (*ap).endptr {
        spc_warn(spe,
                 b"No width/height/filename given for postscriptbox special.\x00"
                     as *const u8 as *const libc::c_char);
        return -1i32
    }
    /* input is not NULL terminated */
    len =
        (*ap).endptr.wrapping_offset_from((*ap).curptr) as libc::c_long as
            libc::c_int;
    len = if 511i32 < len { 511i32 } else { len };
    memcpy(buf.as_mut_ptr() as *mut libc::c_void,
           (*ap).curptr as *const libc::c_void, len as libc::c_ulong);
    buf[len as usize] = '\u{0}' as i32 as libc::c_char;
    transform_info_clear(&mut ti);
    spc_warn(spe, b"%s\x00" as *const u8 as *const libc::c_char,
             buf.as_mut_ptr());
    if sscanf(buf.as_mut_ptr(),
              b"{%lfpt}{%lfpt}{%255[^}]}\x00" as *const u8 as
                  *const libc::c_char, &mut ti.width as *mut libc::c_double,
              &mut ti.height as *mut libc::c_double, filename.as_mut_ptr()) !=
           3i32 {
        spc_warn(spe,
                 b"Syntax error in postscriptbox special?\x00" as *const u8 as
                     *const libc::c_char);
        return -1i32
    }
    (*ap).curptr = (*ap).endptr;
    ...
    if handle.is_null() {
        spc_warn(spe,
                 b"Could not open image file: %s\x00" as *const u8 as
    ...
    loop  {
        let mut p: *const libc::c_char =
            tt_mfgets(buf.as_mut_ptr(), 512i32, handle);
        if p.is_null() { break ; }
        if !(mps_scan_bbox(&mut p, p.offset(strlen(p) as isize), &mut ti.bbox)
                 >= 0i32) {
            continue ;
        }
        ti.flags |= 1i32 << 0i32;
    ...
    if form_id < 0i32 {
        spc_warn(spe,
                 b"Failed to load image file: %s\x00" as *const u8 as
                     *const libc::c_char, filename.as_mut_ptr());
        return -1i32
    }
...
static mut misc_handlers: [spc_handler; 6] =
    unsafe {
        [{
             let mut init =
                 spc_handler{key:
                                 b"postscriptbox\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_postscriptbox as
         ...
         },
         {
             let mut init =
                 spc_handler{key:
                                 b"landscape\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
...
#[no_mangle]
pub unsafe extern "C" fn spc_misc_check_special(mut buffer:
                                                    *const libc::c_char,
                                                mut size: libc::c_int)
 -> bool {
...
*/
#[no_mangle]
pub unsafe extern "C" fn spc_misc_setup_handler(mut handle: *mut spc_handler,
                                                mut spe: *mut spc_env,
 ...
 -> libc::c_int {
    ...
    while i <
        ...
        if keylen as libc::c_ulong == strlen(misc_handlers[i as usize].key) &&
               strncmp(key, misc_handlers[i as usize].key,
