...
#[no_mangle]
pub unsafe extern "C" fn check_for_bmp(mut handle: rust_input_handle_t)
 -> libc::c_int {
    ...
    if ttstub_input_read(handle, sigbytes.as_mut_ptr() as *mut libc::c_char,
                         ::std::mem::size_of::<[libc::c_uchar; 2]>() as
...
unsafe extern "C" fn get_density(mut xdensity: *mut libc::c_double,
    ...
    if (*hdr).x_pix_per_meter > 0i32 as libc::c_uint &&
        ...
        *xdensity =
            72.0f64 / ((*hdr).x_pix_per_meter as libc::c_double * 0.0254f64);
        *ydensity =
            72.0f64 / ((*hdr).y_pix_per_meter as libc::c_double * 0.0254f64)
    } else { *xdensity = 1.0f64; *ydensity = 1.0f64 };
}
#[no_mangle]
pub unsafe extern "C" fn bmp_get_bbox(mut handle: rust_input_handle_t,
 ...
 -> libc::c_int {
    ...
    let mut hdr: hdr_info =
        {
            let mut init =
                hdr_info{offset: 0i32 as libc::c_uint,
    ...
    *width = hdr.width;
    *height =
        (if hdr.height < 0i32 { -hdr.height } else { hdr.height }) as
            libc::c_uint;
...
*/
#[no_mangle]
pub unsafe extern "C" fn bmp_include_image(mut ximage: *mut pdf_ximage,
 ...
 -> libc::c_int {
    ...
    stream = stream_dict;
    ttstub_input_seek(handle, 0i32 as ssize_t, 0i32);
    if read_header(handle, &mut hdr) < 0i32 { return -1i32 }
    ...
    if (hdr.bit_count as libc::c_int) < 24i32 {
        if hdr.bit_count as libc::c_int != 1i32 &&
            ...
            dpx_warning(b"Unsupported palette size: %hu\x00" as *const u8 as
                            *const libc::c_char,
                        hdr.bit_count as libc::c_int);
            return -1i32
        }
        num_palette =
            hdr.offset.wrapping_sub(hdr.hsize).wrapping_sub(14i32 as
                                                                libc::c_uint).wrapping_div(hdr.psize
                                                                                               as
                                                                                               libc::c_uint)
                as libc::c_int;
        ...
        dpx_warning(b"Unkown/Unsupported BMP bitCount value: %hu\x00" as
                        *const u8 as *const libc::c_char,
                    hdr.bit_count as libc::c_int);
        return -1i32
    }
    if info.width == 0i32 || info.height == 0i32 || num_palette < 1i32 {
        dpx_warning(b"Invalid BMP file: width=%u, height=%d, #palette=%d\x00"
                        as *const u8 as *const libc::c_char, info.width,
                    info.height, num_palette);
        return -1i32
    }
    ...
    if (hdr.bit_count as libc::c_int) < 24i32 {
        ...
        palette =
            new(((num_palette * 3i32 + 1i32) as uint32_t as
        ...
        while i < num_palette {
            if ttstub_input_read(handle,
                ...
                dpx_warning(b"Reading file failed...\x00" as *const u8 as
                                *const libc::c_char);
        ...
        lookup =
            pdf_new_string(palette as *const libc::c_void,
    ...
    if hdr.compression == 0i32 {
        ...
        padding =
            if rowbytes % 4i32 != 0 { 4i32 - rowbytes % 4i32 } else { 0i32 };
        dib_rowbytes = rowbytes + padding;
        stream_data_ptr =
            new(((rowbytes * info.height + padding) as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        n = 0i32;
        while n < info.height {
            ...
            if ttstub_input_read(handle, p as *mut libc::c_char,
                                 dib_rowbytes as size_t) !=
                   dib_rowbytes as libc::c_long {
                dpx_warning(b"Reading BMP raster data failed...\x00" as
                                *const u8 as *const libc::c_char);
        ...
        stream_data_ptr =
            new(((rowbytes * info.height) as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        if read_raster_rle8(stream_data_ptr, info.width, info.height, handle)
               < 0i32 {
            dpx_warning(b"Reading BMP raster data failed...\x00" as *const u8
        ...
        stream_data_ptr =
            new(((rowbytes * info.height) as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        if read_raster_rle4(stream_data_ptr, info.width, info.height, handle)
               < 0i32 {
            dpx_warning(b"Reading BMP raster data failed...\x00" as *const u8
                            as *const libc::c_char);
            pdf_release_obj(stream);
        ...
        dpx_warning(b"Unknown/Unsupported compression type for BMP image: %d\x00"
                        as *const u8 as *const libc::c_char, hdr.compression);
        pdf_release_obj(stream);
        return -1i32
    }
    /* gbr --> rgb */
    if hdr.bit_count as libc::c_int == 24i32 {
        ...
        while n < info.width * info.height * 3i32 {
            ...
            *stream_data_ptr.offset(n as isize) =
                *stream_data_ptr.offset((n + 2i32) as isize);
...
unsafe extern "C" fn read_header(mut handle: rust_input_handle_t,
                                 mut hdr: *mut hdr_info) -> libc::c_int {
    let mut buf: [libc::c_uchar; 142] = [0; 142];
    let mut p: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    p = buf.as_mut_ptr();
    if ttstub_input_read(handle, buf.as_mut_ptr() as *mut libc::c_char,
                         (14i32 + 4i32) as size_t) !=
           (14i32 + 4i32) as libc::c_long {
        dpx_warning(b"Could not read BMP file header...\x00" as *const u8 as
                        *const libc::c_char);
        return -1i32
    }
    if *p.offset(0) as libc::c_int != 'B' as i32 ||
           *p.offset(1) as libc::c_int != 'M' as i32 {
        dpx_warning(b"File not starting with \'B\' \'M\'... Not a BMP file?\x00"
    ...
    /* fsize  = ULONG_LE(p); */
    p = p.offset(4);
    if *p.offset(0) as libc::c_int + ((*p.offset(1) as libc::c_int) << 8i32) +
           ((*p.offset(2) as libc::c_int) << 16i32) +
           ((*p.offset(3) as libc::c_int) << 24i32) != 0i32 {
        dpx_warning(b"Not a BMP file???\x00" as *const u8 as
                        *const libc::c_char);
        return -1i32
    }
    p = p.offset(4);
    (*hdr).offset =
        (*p.offset(0) as libc::c_int + ((*p.offset(1) as libc::c_int) << 8i32)
             + ((*p.offset(2) as libc::c_int) << 16i32) +
    ...
    (*hdr).hsize =
        (*p.offset(0) as libc::c_int + ((*p.offset(1) as libc::c_int) << 8i32)
             + ((*p.offset(2) as libc::c_int) << 16i32) +
             ((*p.offset(3) as libc::c_int) << 24i32)) as
            libc::c_uint; /* undefined. FIXME */
    p = p.offset(4); /* undefined. FIXME */
    if ttstub_input_read(handle, p as *mut libc::c_char,
                         (*hdr).hsize.wrapping_sub(4i32 as libc::c_uint) as
                             size_t) !=
    ...
    match (*hdr).hsize {
        12 => {
            (*hdr).width =
                (*p.offset(0) as libc::c_int +
                     ((*p.offset(1) as libc::c_int) << 8i32)) as libc::c_uint;
            p = p.offset(2);
            (*hdr).height =
                *p.offset(0) as libc::c_int +
            ...
            if *p.offset(0) as libc::c_int +
                   ((*p.offset(1) as libc::c_int) << 8i32) != 1i32 {
                dpx_warning(b"Unknown bcPlanes value in BMP COREHEADER.\x00"
                                as *const u8 as *const libc::c_char);
                return -1i32
            }
            p = p.offset(2);
            (*hdr).bit_count =
                (*p.offset(0) as libc::c_int +
                     ((*p.offset(1) as libc::c_int) << 8i32)) as
                    libc::c_ushort;
            p = p.offset(2);
        ...
        40 | 64 | 108 | 124 => {
            ...
            if *p.offset(0) as libc::c_int +
                   ((*p.offset(1) as libc::c_int) << 8i32) != 1i32 {
                dpx_warning(b"Unknown biPlanes value in BMP INFOHEADER.\x00"
                                as *const u8 as *const libc::c_char);
                return -1i32
            }
            p = p.offset(2);
        ...
        _ => {
            dpx_warning(b"Unknown BMP header type.\x00" as *const u8 as
                            *const libc::c_char);
            return -1i32
        }
    }
...
unsafe extern "C" fn read_raster_rle8(mut data_ptr: *mut libc::c_uchar,
 ...
 -> libc::c_int {
    ...
    p = data_ptr;
    rowbytes = width;
    memset(data_ptr as *mut libc::c_void, 0i32,
            ...
            if b0 as libc::c_int == 0i32 {
                match b1 as libc::c_int {
                    ...
                    _ => {
                        h += b1 as libc::c_int;
                        if h > width {
                            dpx_warning(b"RLE decode failed...\x00" as
                                            *const u8 as *const libc::c_char);
                            return -1i32
                        }
                        if ttstub_input_read(handle, p as *mut libc::c_char,
                ...
                h += b0 as libc::c_int;
                if h > width {
                    dpx_warning(b"RLE decode failed...\x00" as *const u8 as
                                    *const libc::c_char);
                    return -1i32
                }
                memset(p as *mut libc::c_void, b1 as libc::c_int,
                       b0 as libc::c_ulong);
            }
        }
        /* next row ... */
...
unsafe extern "C" fn read_raster_rle4(mut data_ptr: *mut libc::c_uchar,
                                      mut width: libc::c_int,
                                      mut height: libc::c_int,
                                      mut handle: rust_input_handle_t)
 -> libc::c_int {
    ...
    while v < height && eoi == 0 {
        ...
        while h < width && eol == 0 {
            ...
            count += 2i32;
            p =
                data_ptr.offset((v * rowbytes) as
                                    isize).offset((h / 2i32) as isize);
            if b0 as libc::c_int == 0i32 {
                match b1 as libc::c_int {
                    0 => {
                            ...
                            while i < nbytes {
                                ...
                                *fresh0 =
                                    (*fresh0 as libc::c_int |
                                         b as libc::c_int >> 4i32 & 0xfi32) as
                                        libc::c_uchar;
                                *p =
                                    ((b as libc::c_int) << 4i32 & 0xf0i32) as
                                        libc::c_uchar;
                                i += 1
                            }
                        } else if ttstub_input_read(handle,
                ...
                if h % 2i32 != 0 {
                    ...
                    p = p.offset(1);
                    *fresh1 =
                        (b1 as libc::c_int >> 4i32 & 0xfi32) as libc::c_uchar;
                    b1 =
                        ((b1 as libc::c_int) << 4i32 & 0xf0i32 |
                             b1 as libc::c_int >> 4i32 & 0xfi32) as
                            libc::c_uchar;
                ...
                if h % 2i32 != 0 {
                    ...
                    *fresh2 =
                        (*fresh2 as libc::c_int & 0xf0i32) as libc::c_uchar
                }
            }
        }
        /* next row ... */
        if eol == 0 && eoi == 0 {
            ...
            b1 = tt_get_unsigned_byte(handle);
            if b0 as libc::c_int != 0i32 {
                dpx_warning(b"No EOL/EOI marker. RLE decode failed...\x00" as
                                *const u8 as *const libc::c_char);
                return -1i32
            } else {
                if b1 as libc::c_int == 0x1i32 {
                    ...
                    dpx_warning(b"No EOL/EOI marker. RLE decode failed...\x00"
                                    as *const u8 as *const libc::c_char);
                    return -1i32
                }
            }
