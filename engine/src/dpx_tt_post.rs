...
unsafe extern "C" fn read_v2_post_names(mut post: *mut tt_post_table,
    ...
    indices =
        new(((*post).numberOfGlyphs as uint32_t as
    ...
    while (i as libc::c_int) < (*post).numberOfGlyphs as libc::c_int {
        ...
        if idx as libc::c_int >= 258i32 {
            if idx as libc::c_int > maxidx as libc::c_int { maxidx = idx }
           ...
           We show a warning only once, instead of thousands of times */
                static mut warning_issued: libc::c_char =
                    0i32 as libc::c_char;
                if warning_issued == 0 {
                    dpx_warning(b"TrueType post table name index %u > 32767\x00"
                ...
                *fresh0 =
                ...
                ttstub_input_read((*sfont).handle,
                                  *(*post).names.offset(i as isize),
                                  len as size_t);
                *(*(*post).names.offset(i as isize)).offset(len as isize) =
                    0i32 as libc::c_char
            } else {
    ...
    (*post).glyphNamePtr =
        new(((*post).numberOfGlyphs as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<*const libc::c_char>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut *const libc::c_char;
    i = 0i32 as USHORT;
    while (i as libc::c_int) < (*post).numberOfGlyphs as libc::c_int {
        ...
        if (idx as libc::c_int) < 258i32 {
            ...
            *fresh3 =
                *(*post).names.offset((idx as libc::c_int - 258i32) as isize)
        } else {
            dpx_warning(b"Invalid glyph name index number: %u (>= %u)\x00" as
...
#[no_mangle]
pub unsafe extern "C" fn tt_read_post_table(mut sfont: *mut sfnt)
 -> *mut tt_post_table {
    let mut post: *mut tt_post_table = 0 as *mut tt_post_table;
    /* offset = */
    sfnt_locate_table(sfont,
                      b"post\x00" as *const u8 as
                          *const libc::c_char); /* Fixed */
    post =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<tt_post_table>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut tt_post_table; /* Fixed */
    ...
    (*post).underlinePosition =
        tt_get_signed_pair((*sfont).handle); /* wrong */
    ...
    if (*post).Version as libc::c_ulong == 0x10000u64 {
        ...
        dpx_warning(b"TrueType \'post\' version 2.5 found (deprecated)\x00" as
                        *const u8 as *const libc::c_char);
    } else if (*post).Version as libc::c_ulong == 0x20000u64 {
        if read_v2_post_names(post, sfont) < 0i32 {
            dpx_warning(b"Invalid version 2.0 \'post\' table\x00" as *const u8
        ...
        dpx_warning(b"Unknown \'post\' version: %08X, assuming version 3.0\x00"
                        as *const u8 as *const libc::c_char, (*post).Version);
    }
    return post;
}
#[no_mangle]
pub unsafe extern "C" fn tt_lookup_post_table(mut post: *mut tt_post_table,
                                              mut glyphname:
                                                  *const libc::c_char)
 -> USHORT {
    ...
    if !post.is_null() && !glyphname.is_null() {
        ...
        __assert_fail(b"post && glyphname\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-tt_post.c\x00" as *const u8 as
                          *const libc::c_char, 157i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 66],
                                                &[libc::c_char; 66]>(b"USHORT tt_lookup_post_table(struct tt_post_table *, const char *)\x00")).as_ptr());
    }
    gid = 0i32 as USHORT;
    while (gid as libc::c_int) < (*post).count as libc::c_int {
        if !(*(*post).glyphNamePtr.offset(gid as isize)).is_null() &&
               streq_ptr(glyphname,
                         *(*post).glyphNamePtr.offset(gid as isize)) as
                   libc::c_int != 0 {
            return gid
        }
...
#[no_mangle]
pub unsafe extern "C" fn tt_get_glyphname(mut post: *mut tt_post_table,
                                          mut gid: USHORT)
 -> *mut libc::c_char {
...
pub unsafe extern "C" fn tt_release_post_table(mut post: *mut tt_post_table) {
    ...
    if !post.is_null() {
        ...
        __assert_fail(b"post\x00" as *const u8 as *const libc::c_char,
                      b"dpx-tt_post.c\x00" as *const u8 as
                          *const libc::c_char, 182i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 51],
                                                &[libc::c_char; 51]>(b"void tt_release_post_table(struct tt_post_table *)\x00")).as_ptr());
    }
    if !(*post).glyphNamePtr.is_null() &&
           (*post).Version as libc::c_ulong != 0x10000u64 {
