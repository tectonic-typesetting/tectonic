...
unsafe extern "C" fn match_expr(mut expr: *mut bt_node,
...
unsafe extern "C" fn bt_new_tree() -> *mut bt_node {
    ...
    expr =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<bt_node>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut bt_node;
    (*expr).flag = 0i32;
    (*expr).left = 0 as *mut bt_node;
    (*expr).right = 0 as *mut bt_node;
    memset((*expr).data.as_mut_ptr() as *mut libc::c_void, 0i32,
           4i32 as libc::c_ulong);
    return expr;
}
unsafe extern "C" fn bt_release_tree(mut tree: *mut bt_node) {
    if !tree.is_null() {
        if !(*tree).left.is_null() { bt_release_tree((*tree).left); }
        if !(*tree).right.is_null() { bt_release_tree((*tree).right); }
        free(tree as *mut libc::c_void);
    };
}
unsafe extern "C" fn parse_expr(mut pp: *mut *const libc::c_char,
                                mut endptr: *const libc::c_char)
 -> *mut bt_node {
    let mut root: *mut bt_node = 0 as *mut bt_node;
    let mut curr: *mut bt_node = 0 as *mut bt_node;
    if *pp >= endptr { return 0 as *mut bt_node }
    curr = bt_new_tree();
    root = curr;
    while *pp < endptr {
        match **pp as libc::c_int {
            ...
            40 => {
                *pp = (*pp).offset(1);
                if *pp < endptr {
                    ...
                    memcpy((*curr).data.as_mut_ptr() as *mut libc::c_void,
            ...
            42 => {
                memset((*curr).data.as_mut_ptr() as *mut libc::c_void,
                       '?' as i32, 4i32 as libc::c_ulong);
                *pp = (*pp).offset(1)
            }
            _ => {
                if (*pp).offset(4) <= endptr {
                    ...
                    while i < 4i32 {
                        if **pp as libc::c_int == ' ' as i32 ||
                            ...
                            (*curr).data[i as usize] =
                                ' ' as i32 as libc::c_char
                        } else {
                            dpx_warning(b"Invalid char in tag: %c\n\x00" as
                                            *const u8 as *const libc::c_char,
                                        **pp as libc::c_int);
                            bt_release_tree(root);
                            return 0 as *mut bt_node
...
pub unsafe extern "C" fn otl_new_opt() -> *mut otl_opt {
    ...
    opt =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<otl_opt>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut otl_opt;
    (*opt).rule = 0 as *mut bt_node;
    return opt as *mut otl_opt;
}
#[no_mangle]
pub unsafe extern "C" fn otl_release_opt(mut opt: *mut otl_opt) {
    if !(*opt).rule.is_null() { bt_release_tree((*opt).rule); }
    (*opt).rule = 0 as *mut bt_node;
    free(opt as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn otl_parse_optstring(mut opt: *mut otl_opt,
 ...
 -> libc::c_int {
    ...
    if !opt.is_null() {
        ...
        __assert_fail(b"opt\x00" as *const u8 as *const libc::c_char,
                      b"dpx-otl_opt.c\x00" as *const u8 as
                          *const libc::c_char, 237i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 49],
                                                &[libc::c_char; 49]>(b"int otl_parse_optstring(otl_opt *, const char *)\x00")).as_ptr());
    }
    if !optstr.is_null() {
        p = optstr;
...
*/
#[no_mangle]
pub unsafe extern "C" fn otl_match_optrule(mut opt: *mut otl_opt,
                                           mut tag: *const libc::c_char)
 -> libc::c_int {
    if !tag.is_null() {
    } else {
        __assert_fail(b"tag\x00" as *const u8 as *const libc::c_char,
                      b"dpx-otl_opt.c\x00" as *const u8 as
                          *const libc::c_char, 251i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 47],
                                                &[libc::c_char; 47]>(b"int otl_match_optrule(otl_opt *, const char *)\x00")).as_ptr());
    }
    if opt.is_null() || (*opt).rule.is_null() { return 1i32 }
