 ...
 */
unsafe extern "C" fn clear_stack(mut dest: *mut *mut card8,
                                 mut limit: *mut card8) {
    ...
    while i < stack_top {
       ...
       */
            _tt_abort(b"%s: Argument value too large. (This is bug)\x00" as
                          *const u8 as *const libc::c_char,
                      b"Type2 Charstring Parser\x00" as *const u8 as
                          *const libc::c_char);
        } else {
            if fabs(value - ivalue as libc::c_double) > 3.0e-5f64 {
                ...
                *fresh12 = (ivalue & 0xffi32) as card8
                ...
                _tt_abort(b"%s: Unexpected error.\x00" as *const u8 as
                              *const libc::c_char,
                          b"Type2 Charstring Parser\x00" as *const u8 as
                              *const libc::c_char);
            }
 ...
 */
unsafe extern "C" fn do_operator1(mut dest: *mut *mut card8,
                                  mut limit: *mut card8,
    ...
    match op as libc::c_int {
        ...
        14 => {
            if stack_top == 1i32 {
                ...
                dpx_warning(b"\"seac\" character deprecated in Type 2 charstring.\x00"
                                as *const u8 as *const libc::c_char);
                status = -1i32;
                return
            } else {
                if stack_top > 0i32 {
                    dpx_warning(b"%s: Operand stack not empty.\x00" as
                                    *const u8 as *const libc::c_char,
                                b"Type2 Charstring Parser\x00" as *const u8 as
                                    *const libc::c_char);
                }
        ...
        5 | 6 | 7 | 8 | 24 | 25 | 26 | 27 | 30 | 31 => {
            ...
            if phase < 2i32 {
                dpx_warning(b"%s: Broken Type 2 charstring.\x00" as *const u8
  ...
  /* no output */
            _tt_abort(b"%s: Unexpected call(g)subr/return\x00" as *const u8 as
                          *const libc::c_char,
                      b"Type2 Charstring Parser\x00" as *const u8 as
                          *const libc::c_char);
        }
        _ => {
            /* no-op ? */
            dpx_warning(b"%s: Unknown charstring operator: 0x%02x\x00" as
                            *const u8 as *const libc::c_char,
                        b"Type2 Charstring Parser\x00" as *const u8 as
                            *const libc::c_char, op as libc::c_int);
            status = -1i32
        }
    };
}
 ...
 *  random: How random ?
 */
unsafe extern "C" fn do_operator2(mut dest: *mut *mut card8,
                                  mut limit: *mut card8,
                                  mut data: *mut *mut card8,
                                  mut endptr: *mut card8) {
    let mut op: card8 = 0;
    ...
    *data = (*data).offset(1);
    match op as libc::c_int {
        0 => {
            /* deprecated */
            dpx_warning(b"Operator \"dotsection\" deprecated in Type 2 charstring.\x00"
                            as *const u8 as *const libc::c_char);
            status = -1i32;
            return
        }
  ...
  /* no output */
            if stack_top < 2i32 { status = -2i32; return }
            stack_top -= 1;
            if arg_stack[stack_top as usize] != 0. &&
                   arg_stack[(stack_top - 1i32) as usize] != 0. {
                arg_stack[(stack_top - 1i32) as usize] = 1.0f64
            } else { arg_stack[(stack_top - 1i32) as usize] = 0.0f64 }
        }
        4 => {
            if stack_top < 2i32 { status = -2i32; return }
            stack_top -= 1;
        ...
        5 => {
            if stack_top < 1i32 { status = -2i32; return }
            if arg_stack[(stack_top - 1i32) as usize] != 0. {
                arg_stack[(stack_top - 1i32) as usize] = 0.0f64
            } else { arg_stack[(stack_top - 1i32) as usize] = 1.0f64 }
        }
        9 => {
            if stack_top < 1i32 { status = -2i32; return }
            arg_stack[(stack_top - 1i32) as usize] =
                fabs(arg_stack[(stack_top - 1i32) as usize])
        }
        10 => {
            if stack_top < 2i32 { status = -2i32; return }
        ...
        15 => {
            if stack_top < 2i32 { status = -2i32; return }
            stack_top -= 1;
            if arg_stack[stack_top as usize] ==
                   arg_stack[(stack_top - 1i32) as usize] {
                arg_stack[(stack_top - 1i32) as usize] = 1.0f64
            } else { arg_stack[(stack_top - 1i32) as usize] = 0.0f64 }
        }
        ...
        20 => {
            ...
            if 32i32 < idx { status = -2i32; return }
            stack_top -= 1;
        ...
        21 => {
            ...
            let mut idx_0: libc::c_int =
                arg_stack[(stack_top - 1i32) as usize] as libc::c_int;
            if 32i32 < idx_0 { status = -2i32; return }
            arg_stack[(stack_top - 1i32) as usize] = trn_array[idx_0 as usize]
        }
        22 => {
            if stack_top < 4i32 { status = -2i32; return }
            stack_top -= 3i32;
            if arg_stack[(stack_top + 1i32) as usize] >
                   arg_stack[(stack_top + 2i32) as usize] {
                arg_stack[(stack_top - 1i32) as usize] =
        ...
        26 => {
            if stack_top < 1i32 { status = -2i32; return }
            arg_stack[(stack_top - 1i32) as usize] =
                sqrt(arg_stack[(stack_top - 1i32) as usize])
        }
        27 => {
            if stack_top < 1i32 { status = -2i32; return }
            if 48i32 < stack_top + 1i32 { status = -2i32; return }
            arg_stack[stack_top as usize] =
                arg_stack[(stack_top - 1i32) as usize];
            stack_top += 1
        }
        ...
        29 => {
            ...
            let mut idx_1: libc::c_int =
                arg_stack[(stack_top - 1i32) as usize] as libc::c_int;
            if idx_1 < 0i32 {
                arg_stack[(stack_top - 1i32) as usize] =
                    arg_stack[(stack_top - 2i32) as usize]
            } else {
                if stack_top < idx_1 + 2i32 { status = -2i32; return }
                arg_stack[(stack_top - 1i32) as usize] =
        ...
        30 => {
            ...
            if stack_top < N { status = -2i32; return }
            if J > 0i32 {
                J = J % N;
                loop  {
                    let fresh21 = J;
                    ...
                    let mut save_0: libc::c_double =
                        arg_stack[(stack_top - 1i32) as usize];
                    let mut i: libc::c_int = stack_top - 1i32;
                    while i > stack_top - N {
                        arg_stack[i as usize] =
                            arg_stack[(i - 1i32) as usize];
                        i -= 1
                    }
                ...
                loop  {
                    ...
                    let mut save_1: libc::c_double =
                        arg_stack[(stack_top - N) as usize];
                    let mut i_0: libc::c_int = stack_top - N;
                    while i_0 < stack_top - 1i32 {
                        arg_stack[i_0 as usize] =
                            arg_stack[(i_0 + 1i32) as usize];
                        i_0 += 1
                    }
        ...
        23 => {
            dpx_warning(b"%s: Charstring operator \"random\" found.\x00" as
                            *const u8 as *const libc::c_char,
                        b"Type2 Charstring Parser\x00" as *const u8 as
                            *const libc::c_char);
            if 48i32 < stack_top + 1i32 { status = -2i32; return }
        ...
        _ => {
            /* no-op ? */
            dpx_warning(b"%s: Unknown charstring operator: 0x0c%02x\x00" as
                            *const u8 as *const libc::c_char,
                        b"Type2 Charstring Parser\x00" as *const u8 as
                            *const libc::c_char, op as libc::c_int);
            status = -1i32
 ...
 */
unsafe extern "C" fn get_fixed(mut data: *mut *mut card8,
                               mut endptr: *mut card8) {
    let mut ivalue: libc::c_int = 0;
    ...
    *data = (*data).offset(1);
    ...
    ivalue =
        **data as libc::c_int * 0x100i32 + *(*data).offset(1) as libc::c_int;
    rvalue =
        (if ivalue as libc::c_long > 0x7fffi64 {
             ivalue as libc::c_long - 0x10000i64
         } else { ivalue as libc::c_long }) as libc::c_double;
    ivalue =
        *(*data).offset(2) as libc::c_int * 0x100i32 +
 ...
 */
unsafe extern "C" fn get_subr(mut subr: *mut *mut card8,
                              mut len: *mut libc::c_int,
                              mut subr_idx: *mut cff_index,
                              mut id: libc::c_int) {
    let mut count: card16 = 0;
    if subr_idx.is_null() {
        _tt_abort(b"%s: Subroutine called but no subroutine found.\x00" as
                      *const u8 as *const libc::c_char,
                  b"Type2 Charstring Parser\x00" as *const u8 as
                      *const libc::c_char);
    }
    ...
    if id > count as libc::c_int {
        _tt_abort(b"%s: Invalid Subr index: %d (max=%u)\x00" as *const u8 as
                      *const libc::c_char,
                  b"Type2 Charstring Parser\x00" as *const u8 as
                      *const libc::c_char, id, count as libc::c_int);
    }
    *len =
        (*(*subr_idx).offset.offset((id + 1i32) as
                                        isize)).wrapping_sub(*(*subr_idx).offset.offset(id
                                                                                            as
                                                                                            isize))
            as libc::c_int;
    *subr =
        (*subr_idx).data.offset(*(*subr_idx).offset.offset(id as isize) as
                                    isize).offset(-1);
}
/*
 * NOTE:
 ...
 *  Type 1 format.
 */
unsafe extern "C" fn do_charstring(mut dest: *mut *mut card8,
                                   mut limit: *mut card8,
                                   mut data: *mut *mut card8,
                                   mut endptr: *mut card8,
    ...
    if nest > 10i32 {
        _tt_abort(b"%s: Subroutine nested too deeply.\x00" as *const u8 as
                      *const libc::c_char,
                  b"Type2 Charstring Parser\x00" as *const u8 as
                      *const libc::c_char);
    }
    ...
    while *data < endptr && status == 0i32 {
        ...
        if b0 as libc::c_int == 255i32 {
            ...
            if stack_top < 1i32 {
                ...
                stack_top -= 1;
                get_subr(&mut subr, &mut len, gsubr_idx,
                         arg_stack[stack_top as usize] as libc::c_int);
                if (*dest).offset(len as isize) > limit {
                    _tt_abort(b"%s: Possible buffer overflow.\x00" as
                                  *const u8 as *const libc::c_char,
                              b"Type2 Charstring Parser\x00" as *const u8 as
                                  *const libc::c_char);
                }
                do_charstring(dest, limit, &mut subr,
                              subr.offset(len as isize), gsubr_idx, subr_idx);
                *data = (*data).offset(1)
            }
            ...
            if stack_top < 1i32 {
                ...
                stack_top -= 1;
                get_subr(&mut subr, &mut len, subr_idx,
                         arg_stack[stack_top as usize] as libc::c_int);
    ...
    if status == 2i32 {
        ...
        dpx_warning(b"%s: Garbage after endchar.\x00" as *const u8 as
                        *const libc::c_char,
                    b"Type2 Charstring Parser\x00" as *const u8 as
                        *const libc::c_char);
    } else if status < 0i32 {
        /* error */
        _tt_abort(b"%s: Parsing charstring failed: (status=%d, stack=%d)\x00"
                      as *const u8 as *const libc::c_char,
                  b"Type2 Charstring Parser\x00" as *const u8 as
                      *const libc::c_char, status, stack_top);
    }
    nest -= 1;
}
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn cs_copy_charstring(mut dst: *mut card8,
 ...
 -> libc::c_int {
    ...
    have_width = 0i32;
    /* expand call(g)subrs */
    do_charstring(&mut dst, dst.offset(dstlen as isize), &mut src,
                  src.offset(srclen as isize), gsubr, subr); /* not used */
    if !ginfo.is_null() {
        (*ginfo).flags = 0i32;
        if have_width != 0 {
            (*ginfo).wx = nominal_width + width
        } else { (*ginfo).wx = default_width }
    }
