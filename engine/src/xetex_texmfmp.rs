...
extern "C" {
    ...
    #[no_mangle]
    fn ttstub_get_file_md5(path: *const libc::c_char,
                           digest: *mut libc::c_char) -> libc::c_int;
    #[no_mangle]
    fn ttstub_get_data_md5(data: *const libc::c_char, len: size_t,
                           digest: *mut libc::c_char) -> libc::c_int;
    #[no_mangle]
...
static mut last_source_name: *mut libc::c_char =
    0 as *const libc::c_char as *mut libc::c_char;
static mut last_lineno: libc::c_int = 0;
#[no_mangle]
pub unsafe extern "C" fn get_date_and_time(mut minutes: *mut int32_t,
                                           mut day: *mut int32_t,
                                           mut month: *mut int32_t,
                                           mut year: *mut int32_t) {
    let mut tmptr: *mut tm =
        0 as *mut tm; /* in the XeTeX case, this may be more than enough */
...
unsafe extern "C" fn checkpool_pointer(mut pool_ptr_0: pool_pointer,
                                       mut len: size_t) {
    if (pool_ptr_0 as libc::c_ulong).wrapping_add(len) >=
           pool_size as libc::c_ulong {
        _tt_abort(b"string pool overflow [%i bytes]\x00" as *const u8 as
                      *const libc::c_char, pool_size);
    };
}
#[no_mangle]
pub unsafe extern "C" fn maketexstring(mut s: *const libc::c_char)
 -> libc::c_int {
    ...
    loop  {
        ...
        if !(rval != 0i32 as libc::c_uint) { break ; }
        ...
        match extraBytes as libc::c_int {
            ...
            4 => { current_block_19 = 15420705083065539194; }
            3 => { current_block_19 = 17593909170536150684; }
            2 => { current_block_19 = 9565569445570550704; }
            1 => { current_block_19 = 4209676304665092873; }
            0 | _ => { current_block_19 = 11194104282611034094; }
        }
...
#[no_mangle]
pub unsafe extern "C" fn gettexstring(mut s: str_number)
 -> *mut libc::c_char {
    ...
    if s as libc::c_long >= 65536i64 {
        len =
            *str_start.offset(((s + 1i32) as libc::c_long - 65536i64) as
                                  isize) -
    ...
    while i < len {
            ...
            let mut lo: uint32_t =
                *str_pool.offset((i +
                                      *str_start.offset((s as libc::c_long -
                                                            ...
                                                            isize)) as isize)
                    as uint32_t;
            if lo >= 0xdc00i32 as libc::c_uint &&
                   lo <= 0xdfffi32 as libc::c_uint {
                c =
                    c.wrapping_sub(0xd800i32 as
                                       libc::c_uint).wrapping_mul(0x400i32 as
                                                                      libc::c_uint).wrapping_add(lo).wrapping_sub(0xdc00i32
                                                                                                                      as
                                                                                                                      libc::c_uint).wrapping_add(0x10000i32
                                                                                                                                                     as
                                                                                                                                                     libc::c_uint)
            } else { c = 0xfffdi32 as uint32_t }
        }
        ...
        j =
            (j as libc::c_uint).wrapping_add(bytesToWrite) as pool_pointer as
                pool_pointer;
        ...
        match bytesToWrite {
            4 => {
                ...
                *name.offset(j as isize) =
                    ((c | 0x80i32 as libc::c_uint) & 0xbfi32 as libc::c_uint)
                        as libc::c_char;
                c >>= 6i32;
                current_block_28 = 9281751456159701257;
            }
            3 => { current_block_28 = 9281751456159701257; }
            2 => { current_block_28 = 13645261163415976511; }
            1 => { current_block_28 = 4925739576308592327; }
            _ => { current_block_28 = 2891135413264362348; }
        }
        ...
        match current_block_28 {
            4925739576308592327 => {
                j -= 1;
                *name.offset(j as isize) =
                    (c | firstByteMark[bytesToWrite as usize] as libc::c_uint)
                        as libc::c_char
            }
            _ => { }
        }
...
unsafe extern "C" fn compare_paths(mut p1: *const libc::c_char,
 ...
 -> libc::c_int {
    ...
    ret = if ret < 0i32 { -1i32 } else if ret > 0i32 { 1i32 } else { 0i32 };
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn is_new_source(mut srcfilename: str_number,
                                       mut lineno: libc::c_int) -> bool {
    let mut name: *mut libc::c_char = gettexstring(srcfilename);
    return compare_paths(name, last_source_name) != 0i32 ||
               lineno != last_lineno;
}
#[no_mangle]
pub unsafe extern "C" fn remember_source_info(mut srcfilename: str_number,
...
#[no_mangle]
pub unsafe extern "C" fn make_src_special(mut srcfilename: str_number,
                                          mut lineno: libc::c_int)
 -> pool_pointer {
   ...
   */
    sprintf(buf.as_mut_ptr(),
            b"src:%d \x00" as *const u8 as *const libc::c_char, lineno);
    if (pool_ptr as
            libc::c_ulong).wrapping_add(strlen(buf.as_mut_ptr())).wrapping_add(strlen(filename))
           >= pool_size as size_t {
        _tt_abort(b"string pool overflow\x00" as *const u8 as
                      *const libc::c_char);
    }
    s = buf.as_mut_ptr();
    while *s != 0 {
 ...
 */
unsafe extern "C" fn convertStringToHexString(mut in_0: *const libc::c_char,
    ...
    while i < lin {
        ...
        let fresh13 = j;
        j = j + 1;
        *out.offset(fresh13 as isize) =
            hexchars[(c as libc::c_int >> 4i32 & 0xfi32) as usize];
        let fresh14 = j;
        j = j + 1;
        *out.offset(fresh14 as isize) =
            hexchars[(c as libc::c_int & 0xfi32) as usize];
        i += 1
    }
