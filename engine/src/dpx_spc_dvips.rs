...
*/
    ...
    #[no_mangle]
    fn mps_exec_inline(buffer: *mut *const libc::c_char,
                       endptr: *const libc::c_char, x_user: libc::c_double,
                       y_user: libc::c_double) -> libc::c_int;
    #[no_mangle]
...
unsafe extern "C" fn spc_handler_ps_header(mut spe: *mut spc_env,
                                           mut args: *mut spc_arg)
 -> libc::c_int {
    let mut pro: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut ps_header: *mut rust_input_handle_t =
        0 as *mut rust_input_handle_t;
    skip_white(&mut (*args).curptr, (*args).endptr);
    if (*args).curptr.offset(1) >= (*args).endptr ||
           *(*args).curptr.offset(0) as libc::c_int != '=' as i32 {
        spc_warn(spe,
                 b"No filename specified for PSfile special.\x00" as *const u8
                     as *const libc::c_char);
        return -1i32
    }
    (*args).curptr = (*args).curptr.offset(1);
    pro =
        xmalloc(((*args).endptr.wrapping_offset_from((*args).curptr) as
                     libc::c_long + 1i32 as libc::c_long) as size_t) as
            *mut libc::c_char;
    strncpy(pro, (*args).curptr,
            (*args).endptr.wrapping_offset_from((*args).curptr) as
                libc::c_long as libc::c_ulong);
    *pro.offset((*args).endptr.wrapping_offset_from((*args).curptr) as
                    libc::c_long as isize) = 0i32 as libc::c_char;
    ps_header =
        ttstub_input_open(pro, TTIF_TEX_PS_HEADER, 0i32) as
            *mut rust_input_handle_t;
    if ps_header.is_null() {
        spc_warn(spe,
                 b"PS header %s not found.\x00" as *const u8 as
                     *const libc::c_char, pro);
    ...
    if num_ps_headers & 0xfi32 == 0 {
        ps_headers =
            xrealloc(ps_headers as *mut libc::c_void,
                     (::std::mem::size_of::<*mut libc::c_char>() as
                          libc::c_ulong).wrapping_mul((num_ps_headers + 16i32)
                                                          as libc::c_ulong))
...
unsafe extern "C" fn parse_filename(mut pp: *mut *const libc::c_char,
 ...
 -> *mut libc::c_char {
    ...
    if p.is_null() || p >= endptr {
        ...
        if *p as libc::c_int == '\"' as i32 ||
            ...
            p = p.offset(1);
            qchar = *fresh2
        } else { qchar = ' ' as i32 as libc::c_char }
    }
    n = 0i32;
    q = p;
...
/* =filename ... */
unsafe extern "C" fn spc_handler_ps_file(mut spe: *mut spc_env,
                                         mut args: *mut spc_arg)
 -> libc::c_int {
    ...
    if !spe.is_null() && !args.is_null() {
        ...
        __assert_fail(b"spe && args\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 60],
                                                &[libc::c_char; 60]>(b"int spc_handler_ps_file(struct spc_env *, struct spc_arg *)\x00")).as_ptr());
    }
    skip_white(&mut (*args).curptr, (*args).endptr);
    if (*args).curptr.offset(1) >= (*args).endptr ||
           *(*args).curptr.offset(0) as libc::c_int != '=' as i32 {
    ...
    if form_id < 0i32 {
        spc_warn(spe,
                 b"Failed to read image file: %s\x00" as *const u8 as
...
unsafe extern "C" fn spc_handler_ps_plotfile(mut spe: *mut spc_env,
                                             mut args: *mut spc_arg)
 -> libc::c_int {
    let mut error: libc::c_int = 0i32; /* xscale = 1.0, yscale = -1.0 */
    let mut form_id: libc::c_int = 0;
    let mut filename: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut p: transform_info =
        transform_info{width: 0.,
                       height: 0.,
                       depth: 0.,
                       matrix:
    ...
    if !spe.is_null() && !args.is_null() {
        ...
        __assert_fail(b"spe && args\x00" as *const u8 as *const libc::c_char,
                      b"dpx-spc_dvips.c\x00" as *const u8 as
                          *const libc::c_char, 185i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 64],
                                                &[libc::c_char; 64]>(b"int spc_handler_ps_plotfile(struct spc_env *, struct spc_arg *)\x00")).as_ptr());
    }
    spc_warn(spe,
             b"\"ps: plotfile\" found (not properly implemented)\x00" as
                 *const u8 as *const libc::c_char);
    skip_white(&mut (*args).curptr, (*args).endptr);
    filename = parse_filename(&mut (*args).curptr, (*args).endptr);
    if filename.is_null() {
        spc_warn(spe,
                 b"Expecting filename but not found...\x00" as *const u8 as
                     *const libc::c_char);
        return -1i32
    }
    form_id = pdf_ximage_findresource(filename, options);
    if form_id < 0i32 {
        spc_warn(spe,
                 b"Could not open PS file: %s\x00" as *const u8 as
                     *const libc::c_char, filename);
        error = -1i32
    } else {
        transform_info_clear(&mut p);
        p.matrix.d = -1.0f64;
        pdf_dev_put_image(form_id, &mut p, 0i32 as libc::c_double,
                          0i32 as libc::c_double);
    }
    free(filename as *mut libc::c_void);
    return error;
}
unsafe extern "C" fn spc_handler_ps_literal(mut spe: *mut spc_env,
                                            mut args: *mut spc_arg)
 -> libc::c_int {
    ...
    if !spe.is_null() && !args.is_null() && (*args).curptr <= (*args).endptr {
        ...
        __assert_fail(b"spe && args && args->curptr <= args->endptr\x00" as
                          *const u8 as *const libc::c_char,
                      b"dpx-spc_dvips.c\x00" as *const u8 as
                          *const libc::c_char, 218i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 63],
                                                &[libc::c_char; 63]>(b"int spc_handler_ps_literal(struct spc_env *, struct spc_arg *)\x00")).as_ptr());
    }
    if (*args).curptr.offset(strlen(b":[begin]\x00" as *const u8 as
        ...
        y_user = pending_y;
        (*args).curptr =
            (*args).curptr.offset(strlen(b":[begin]\x00" as *const u8 as
                                             *const libc::c_char) as isize)
    } else if (*args).curptr.offset(strlen(b":[end]\x00" as *const u8 as
                                               *const libc::c_char) as isize)
                  <= (*args).endptr &&
                  !strstartswith((*args).curptr,
        ...
        if block_pending <= 0i32 {
            spc_warn(spe,
                     b"No corresponding ::[begin] found.\x00" as *const u8 as
                         *const libc::c_char);
            return -1i32
        }
        block_pending -= 1;
        ...
        y_user = pending_y;
        (*args).curptr =
            (*args).curptr.offset(strlen(b":[end]\x00" as *const u8 as
                                             *const libc::c_char) as isize)
    } else if (*args).curptr < (*args).endptr &&
                  *(*args).curptr.offset(0) as libc::c_int == ':' as i32 {
        x_user = if position_set != 0 { pending_x } else { (*spe).x_user };
        y_user = if position_set != 0 { pending_y } else { (*spe).y_user };
        (*args).curptr = (*args).curptr.offset(1)
    } else {
        position_set = 1i32;
        pending_x = (*spe).x_user;
    ...
    if (*args).curptr < (*args).endptr {
        ...
        error =
            mps_exec_inline(&mut (*args).curptr, (*args).endptr, x_user,
                            y_user);
        if error != 0 {
            spc_warn(spe,
                     b"Interpreting PS code failed!!! Output might be broken!!!\x00"
                         as *const u8 as *const libc::c_char);
            pdf_dev_grestore_to(gs_depth);
        } else if st_depth != mps_stack_depth() {
            spc_warn(spe,
                     b"Stack not empty after execution of inline PostScript code.\x00"
                         as *const u8 as *const libc::c_char);
            spc_warn(spe,
                     b">> Your macro package makes some assumption on internal behaviour of DVI drivers.\x00"
                         as *const u8 as *const libc::c_char);
            spc_warn(spe,
                     b">> It may not compatible with dvipdfmx.\x00" as
                         *const u8 as *const libc::c_char);
        }
    }
    return error;
}
unsafe extern "C" fn spc_handler_ps_trickscmd(mut spe: *mut spc_env,
                                              mut args: *mut spc_arg)
 -> libc::c_int {
    dpx_warning(b"PSTricks commands are disallowed in Tectonic\x00" as
                    *const u8 as *const libc::c_char);
    (*args).curptr = (*args).endptr;
    return -1i32;
}
unsafe extern "C" fn spc_handler_ps_tricksobj(mut spe: *mut spc_env,
                                              mut args: *mut spc_arg)
 -> libc::c_int {
    dpx_warning(b"PSTricks commands are disallowed in Tectonic\x00" as
                    *const u8 as *const libc::c_char);
...
unsafe extern "C" fn spc_handler_ps_default(mut spe: *mut spc_env,
                                            mut args: *mut spc_arg)
 -> libc::c_int {
    ...
    if !spe.is_null() && !args.is_null() {
        ...
        __assert_fail(b"spe && args\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 63],
                                                &[libc::c_char; 63]>(b"int spc_handler_ps_default(struct spc_env *, struct spc_arg *)\x00")).as_ptr());
    }
    pdf_dev_gsave();
    st_depth = mps_stack_depth();
    gs_depth = pdf_dev_current_depth();
    ...
    error =
        mps_exec_inline(&mut (*args).curptr, (*args).endptr, (*spe).x_user,
                        (*spe).y_user);
    M.e = -(*spe).x_user;
    M.f = -(*spe).y_user;
    pdf_dev_concat(&mut M);
    if error != 0 {
        ...
        spc_warn(spe,
                 b">> Your macro package makes some assumption on internal behaviour of DVI drivers.\x00"
                     as *const u8 as *const libc::c_char);
        spc_warn(spe,
                 b">> It may not compatible with dvipdfmx.\x00" as *const u8
                     as *const libc::c_char);
    }
    pdf_dev_grestore_to(gs_depth);
    pdf_dev_grestore();
    ...
    unsafe {
        [{
         ...
         },
         {
             let mut init =
                 spc_handler{key:
                                 b"PSfile\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_file as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
         ...
         },
         {
             let mut init =
                 spc_handler{key:
                                 b"ps: plotfile \x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_plotfile as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
             ...
             let mut init =
                 spc_handler{key:
                                 b"PS:\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_literal as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
         ...
         },
         {
             let mut init =
                 spc_handler{key:
                                 b"PST:\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_trickscmd as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
             ...
             let mut init =
                 spc_handler{key:
                                 b"pst:\x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_tricksobj as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
             ...
             let mut init =
                 spc_handler{key:
                                 b"\" \x00" as *const u8 as
                                     *const libc::c_char,
                             exec:
                                 Some(spc_handler_ps_default as
                                          unsafe extern "C" fn(_:
                                                                   *mut spc_env,
                                                               _:
                                                                   *mut spc_arg)
...
pub unsafe extern "C" fn spc_dvips_at_end_document() -> libc::c_int {
    if !ps_headers.is_null() {
        while num_ps_headers > 0i32 {
            num_ps_headers -= 1;
            free(*ps_headers.offset(num_ps_headers as isize) as
                     *mut libc::c_void);
        }
        ps_headers =
            mfree(ps_headers as *mut libc::c_void) as *mut *mut libc::c_char
    }
    return 0i32;
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn spc_dvips_check_special(mut buf: *const libc::c_char,
                                                 mut len: libc::c_int)
 -> bool {
    ...
    while i <
        ...
        if len as libc::c_ulong >= strlen(dvips_handlers[i as usize].key) &&
               memcmp(p as *const libc::c_void,
                      ...
                      strlen(dvips_handlers[i as usize].key)) == 0 {
            return 1i32 != 0
        }
        i = i.wrapping_add(1)
    }
...
*/
#[no_mangle]
pub unsafe extern "C" fn spc_dvips_setup_handler(mut handle: *mut spc_handler,
                                                 mut spe: *mut spc_env,
                                                 mut args: *mut spc_arg)
 -> libc::c_int {
    ...
    while (*args).curptr < (*args).endptr &&
              *(*__ctype_b_loc()).offset(*(*args).curptr.offset(0) as
    ...
    if (*args).curptr < (*args).endptr &&
           *(*args).curptr.offset(0) as libc::c_int == ':' as i32 {
        (*args).curptr = (*args).curptr.offset(1);
        if (*args).curptr.offset(strlen(b" plotfile \x00" as *const u8 as
                                            *const libc::c_char) as isize) <=
               (*args).endptr &&
               !strstartswith((*args).curptr,
                              b" plotfile \x00" as *const u8 as
                                  *const libc::c_char).is_null() {
            (*args).curptr =
                (*args).curptr.offset(strlen(b" plotfile \x00" as *const u8 as
    ...
    keylen =
        (*args).curptr.wrapping_offset_from(key) as libc::c_long as
            libc::c_int;
    if keylen < 1i32 {
        spc_warn(spe,
                 b"Not ps: special???\x00" as *const u8 as
                     *const libc::c_char);
        return -1i32
    }
    i = 0i32 as size_t;
    while i <
        ...
        if keylen as libc::c_ulong == strlen(dvips_handlers[i as usize].key)
               &&
               strncmp(key, dvips_handlers[i as usize].key,
                       keylen as libc::c_ulong) == 0 {
            skip_white(&mut (*args).curptr, (*args).endptr);
