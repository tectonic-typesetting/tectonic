...
extern "C" {
    ...
    #[no_mangle]
    fn iccp_get_rendering_intent(profile: *const libc::c_void,
                                 proflen: libc::c_int) -> *mut pdf_obj;
    #[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn check_for_jpeg(mut handle: rust_input_handle_t)
 -> libc::c_int {
    let mut jpeg_sig: [libc::c_uchar; 2] = [0; 2];
    ttstub_input_seek(handle, 0i32 as ssize_t, 0i32);
    if ttstub_input_read(handle, jpeg_sig.as_mut_ptr() as *mut libc::c_char,
                         2i32 as size_t) != 2i32 as libc::c_long {
        return 0i32
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn jpeg_include_image(mut ximage: *mut pdf_ximage,
 ...
 -> libc::c_int {
    ...
    if check_for_jpeg(handle) == 0 {
        dpx_warning(b"%s: Not a JPEG file?\x00" as *const u8 as
                        *const libc::c_char,
    ...
    match j_info.num_components as libc::c_int {
        ...
        4 => { colortype = -4i32 }
        _ => {
            dpx_warning(b"%s: Unknown color space (num components: %d)\x00" as
    ...
    pdf_add_dict(stream_dict,
                 pdf_new_name(b"Filter\x00" as *const u8 as
                                  *const libc::c_char),
                 pdf_new_name(b"DCTDecode\x00" as *const u8 as
                                  *const libc::c_char));
    /* XMP Metadata */
    ...
    if j_info.flags & 1i32 << 2i32 != 0 {
        ...
        if icc_stream.is_null() {
            ...
            if iccp_check_colorspace(colortype,
                                     ...
                                     pdf_stream_length(icc_stream)) < 0i32 {
                colorspace = 0 as *mut pdf_obj
            } else {
                cspc_id =
                    iccp_load_profile(0 as *const libc::c_char,
                                      pdf_stream_dataptr(icc_stream),
                ...
                if cspc_id < 0i32 {
                    ...
                    intent =
                        iccp_get_rendering_intent(pdf_stream_dataptr(icc_stream),
                                                  pdf_stream_length(icc_stream));
                    if !intent.is_null() {
                        pdf_add_dict(stream_dict,
                                     pdf_new_name(b"Intent\x00" as *const u8
    ...
    if colorspace.is_null() {
        match colortype {
            ...
            -4 => {
                colorspace =
                    pdf_new_name(b"DeviceCMYK\x00" as *const u8 as
                                     *const libc::c_char)
            }
            _ => { }
        }
    ...
    if j_info.flags & 1i32 << 1i32 != 0 &&
           j_info.num_components as libc::c_int == 4i32 {
        let mut decode: *mut pdf_obj = 0 as *mut pdf_obj;
        let mut i: libc::c_uint = 0;
        dpx_warning(b"Adobe CMYK JPEG: Inverted color assumed.\x00" as
                        *const u8 as *const libc::c_char);
        ...
        pdf_add_dict(stream_dict,
                     pdf_new_name(b"Decode\x00" as *const u8 as
                                      *const libc::c_char), decode);
    }
    /* Copy file */
...
unsafe extern "C" fn jpeg_get_density(mut j_info: *mut JPEG_info,
...
unsafe extern "C" fn JPEG_release_APPn_data(mut marker: JPEG_marker,
    ...
    if marker as libc::c_uint == JM_APP0 as libc::c_int as libc::c_uint &&
        ...
        (*data_0).chunk =
            mfree((*data_0).chunk as *mut libc::c_void) as *mut libc::c_uchar;
        free(data_0 as *mut libc::c_void);
    } else if marker as libc::c_uint ==
                  JM_APP14 as libc::c_int as libc::c_uint &&
                  app_sig as libc::c_uint ==
                      JS_APPn_ADOBE as libc::c_int as libc::c_uint {
...
unsafe extern "C" fn JPEG_info_clear(mut j_info: *mut JPEG_info) {
    if (*j_info).num_appn > 0i32 && !(*j_info).appn.is_null() {
        ...
        while i < (*j_info).num_appn {
            JPEG_release_APPn_data((*(*j_info).appn.offset(i as
                                                               isize)).marker,
                                   (*(*j_info).appn.offset(i as
                                                               isize)).app_sig,
                                   (*(*j_info).appn.offset(i as
                                                               isize)).app_data);
            i += 1
...
unsafe extern "C" fn JPEG_get_iccp(mut j_info: *mut JPEG_info)
 -> *mut pdf_obj {
    let mut icc_stream: *mut pdf_obj = 0 as *mut pdf_obj;
    ...
    while i < (*j_info).num_appn {
        if !((*(*j_info).appn.offset(i as isize)).marker as libc::c_uint !=
                 ...
                 (*(*j_info).appn.offset(i as isize)).app_sig as libc::c_uint
                     != JS_APPn_ICC as libc::c_int as libc::c_uint) {
            icc =
                (*(*j_info).appn.offset(i as isize)).app_data as
                    *mut JPEG_APPn_ICC;
            if num_icc_seg < 0i32 && prev_id == 0i32 {
                ...
                dpx_warning(b"Invalid JPEG ICC chunk: %d (p:%d, n:%d)\x00" as
                                *const u8 as *const libc::c_char,
                            (*icc).seq_id as libc::c_int, prev_id,
                            (*icc).num_chunks as libc::c_int);
                pdf_release_obj(icc_stream);
                icc_stream = 0 as *mut pdf_obj;
                break ;
            }
            pdf_add_stream(icc_stream, (*icc).chunk as *const libc::c_void,
...
unsafe extern "C" fn JPEG_get_XMP(mut j_info: *mut JPEG_info)
 -> *mut pdf_obj {
    let mut XMP_stream: *mut pdf_obj = 0 as *mut pdf_obj;
    let mut stream_dict: *mut pdf_obj = 0 as *mut pdf_obj;
    ...
    while i < (*j_info).num_appn {
        ...
        if !((*(*j_info).appn.offset(i as isize)).marker as libc::c_uint !=
            ...
            XMP =
                (*(*j_info).appn.offset(i as isize)).app_data as
                    *mut JPEG_APPn_XMP;
            pdf_add_stream(XMP_stream, (*XMP).packet as *const libc::c_void,
    ...
    if count > 1i32 {
        dpx_warning(b"%s: Multiple XMP segments found in JPEG file. (untested)\x00"
                        as *const u8 as *const libc::c_char,
                    b"JPEG\x00" as *const u8 as *const libc::c_char);
    }
    return XMP_stream;
}
unsafe extern "C" fn JPEG_get_marker(mut handle: rust_input_handle_t)
 -> JPEG_marker {
    let mut c: libc::c_int = 0;
    c = ttstub_input_getc(handle);
    if c != 255i32 { return 4294967295 as JPEG_marker }
    loop  {
        c = ttstub_input_getc(handle);
        if c < 0i32 {
            return 4294967295 as JPEG_marker
        } else { if c > 0i32 && c < 255i32 { return c as JPEG_marker } }
    };
}
unsafe extern "C" fn add_APPn_marker(mut j_info: *mut JPEG_info,
 ...
 -> libc::c_int {
    ...
    if (*j_info).num_appn >= (*j_info).max_appn {
        ...
        (*j_info).appn =
            renew((*j_info).appn as *mut libc::c_void,
                  ((*j_info).max_appn as uint32_t as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<JPEG_ext>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut JPEG_ext
    }
    n = (*j_info).num_appn;
...
unsafe extern "C" fn read_APP14_Adobe(mut j_info: *mut JPEG_info,
                                      mut handle: rust_input_handle_t)
 -> libc::c_ushort {
    let mut app_data: *mut JPEG_APPn_Adobe = 0 as *mut JPEG_APPn_Adobe;
    app_data =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<JPEG_APPn_Adobe>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut JPEG_APPn_Adobe;
    ...
    add_APPn_marker(j_info, JM_APP14, JS_APPn_ADOBE as libc::c_int,
                    app_data as *mut libc::c_void);
    return 7i32 as libc::c_ushort;
}
unsafe extern "C" fn read_exif_bytes(mut pp: *mut *mut libc::c_uchar,
                                     mut n: libc::c_int,
                                     mut endian: libc::c_int) -> libc::c_int {
...
unsafe extern "C" fn read_APP1_Exif(mut info: *mut JPEG_info,
                                    mut handle: rust_input_handle_t,
    ...
    if !(r < 0i32 as libc::c_long || r as size_t != length) {
        ...
        if !(p.offset(8) >= endptr) {
            tiff_header = p;
            if *p as libc::c_int == 'M' as i32 &&
                   *p.offset(1) as libc::c_int == 'M' as i32 {
                bigendian = 0i32 as libc::c_char;
                current_block = 1109700713171191020;
            } else if *p as libc::c_int == 'I' as i32 &&
                          *p.offset(1) as libc::c_int == 'I' as i32 {
                bigendian = 1i32 as libc::c_char;
                current_block = 1109700713171191020;
            } else {
                dpx_warning(b"JPEG: Invalid value in Exif TIFF header.\x00" as
                                *const u8 as *const libc::c_char);
            ...
            match current_block {
                ...
                _ => {
                    p = p.offset(2);
                    i =
                        read_exif_bytes(&mut p, 2i32,
                                        bigendian as libc::c_int);
                    if i != 42i32 {
                        dpx_warning(b"JPEG: Invalid value in Exif TIFF header.\x00"
                                        as *const u8 as *const libc::c_char);
                    } else {
                        i =
                            read_exif_bytes(&mut p, 4i32,
                                            bigendian as libc::c_int);
                        ...
                        loop  {
                            ...
                            match tag {
                                ...
                                20753 => {
                                    ...
                                    if type_0 != 4i32 || count != 1i32 {
                                        dpx_warning(b"%s: Invalid data for PixelPerUnitX in Exif chunk.\x00"
                                                        as *const u8 as
                                                        *const libc::c_char,
                                                    b"JPEG\x00" as *const u8
                                                        as
                                                        *const libc::c_char);
                                        current_block = 10568945602212496329;
                                        break ;
                                ...
                                20754 => {
                                    /* PixelPerUnitY */
                                    if type_0 != 4i32 || count != 1i32 {
                                        dpx_warning(b"%s: Invalid data for PixelPerUnitY in Exif chunk.\x00"
                                                        as *const u8 as
                                                        *const libc::c_char,
                                                    b"JPEG\x00" as *const u8
                                                        as
                            ...
                            if type_0 != 1i32 || count != 1i32 {
                                dpx_warning(b"%s: Invalid data for ResolutionUnit in Exif chunk.\x00"
                                                as *const u8 as
                                                *const libc::c_char,
                                            b"JPEG\x00" as *const u8 as
                                                *const libc::c_char); /* Unit is meter */
                                current_block = 10568945602212496329;
                                break ;
                            } else {
                                value =
                                    read_exif_bytes(&mut p, 1i32,
                                                    bigendian as libc::c_int);
                                p = p.offset(3);
                                if value == 1i32 {
                                    res_unit_ms = 0.0254f64
                                } else { res_unit_ms = 0.0f64 }
                            }
                        }
                        match current_block {
                            10568945602212496329 => { }
                            _ => {
                                ...
                                if xres > 0.0f64 && yres > 0.0f64 {
                                    ...
                                    exifydpi = yres * res_unit
                                              ...
                                              && res_unit_ms > 0.0f64 {
                                    exifxdpi =
                                        xres_ms as libc::c_double *
                                            res_unit_ms;
                                    exifydpi =
                                        yres_ms as libc::c_double *
                                            res_unit_ms
                                } else {
                                ...
                                if (*info).xdpi < 0.1f64 &&
                                    ...
                                    let mut xxx1: libc::c_double =
                                        floor(exifxdpi + 0.5f64);
                                    let mut xxx2: libc::c_double =
                                        floor((*info).xdpi + 0.5f64);
                                    let mut yyy1: libc::c_double =
                                        floor(exifydpi + 0.5f64);
                                    let mut yyy2: libc::c_double =
                                        floor((*info).ydpi + 0.5f64);
...
unsafe extern "C" fn read_APP0_JFIF(mut j_info: *mut JPEG_info,
                                    mut handle: rust_input_handle_t)
 -> size_t {
    ...
    app_data =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<JPEG_APPn_JFIF>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut JPEG_APPn_JFIF;
    (*app_data).version = tt_get_unsigned_pair(handle);
    ...
    thumb_data_len =
        (3i32 * (*app_data).Xthumbnail as libc::c_int *
             (*app_data).Ythumbnail as libc::c_int) as size_t;
    if thumb_data_len > 0i32 as libc::c_ulong {
        (*app_data).thumbnail =
            new((thumb_data_len as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_uchar;
        ttstub_input_read(handle, (*app_data).thumbnail as *mut libc::c_char,
                          thumb_data_len);
    } else { (*app_data).thumbnail = 0 as *mut libc::c_uchar }
    add_APPn_marker(j_info, JM_APP0, JS_APPn_JFIF as libc::c_int,
                    app_data as *mut libc::c_void);
    match (*app_data).units as libc::c_int {
        ...
        2 => {
            ...
            (*j_info).xdpi =
                (*app_data).Xdensity as libc::c_int as libc::c_double *
                    2.54f64;
            (*j_info).ydpi =
                (*app_data).Ydensity as libc::c_int as libc::c_double *
                    2.54f64
        }
        _ => {
...
unsafe extern "C" fn read_APP0_JFXX(mut handle: rust_input_handle_t,
                                    mut length: size_t) -> size_t {
    ...
    ttstub_input_seek(handle,
                      ...
                      1i32); /* Thunbnail image */
    /* Ignore */
    return length; /* Starting at 1 */
}
unsafe extern "C" fn read_APP1_XMP(mut j_info: *mut JPEG_info,
                                   mut handle: rust_input_handle_t,
                                   mut length: size_t) -> size_t {
    let mut app_data: *mut JPEG_APPn_XMP = 0 as *mut JPEG_APPn_XMP;
    app_data =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<JPEG_APPn_XMP>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut JPEG_APPn_XMP;
    (*app_data).length = length;
    (*app_data).packet =
        new(((*app_data).length as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_uchar;
    ttstub_input_read(handle, (*app_data).packet as *mut libc::c_char,
                      (*app_data).length);
    add_APPn_marker(j_info, JM_APP1, JS_APPn_XMP as libc::c_int,
                    app_data as *mut libc::c_void);
    return length;
}
unsafe extern "C" fn read_APP2_ICC(mut j_info: *mut JPEG_info,
                                   mut handle: rust_input_handle_t,
                                   mut length: size_t) -> size_t {
    let mut app_data: *mut JPEG_APPn_ICC = 0 as *mut JPEG_APPn_ICC;
    app_data =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<JPEG_APPn_ICC>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut JPEG_APPn_ICC;
    ...
    (*app_data).chunk =
        new(((*app_data).length as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_uchar;
    ttstub_input_read(handle, (*app_data).chunk as *mut libc::c_char,
                      (*app_data).length);
    add_APPn_marker(j_info, JM_APP2, JS_APPn_ICC as libc::c_int,
                    app_data as *mut libc::c_void);
    return length;
}
unsafe extern "C" fn JPEG_copy_stream(mut j_info: *mut JPEG_info,
 ...
 -> libc::c_int {
    ...
    while found_SOFn == 0 && count < 1024i32 &&
              {
                  ...
                  marker as libc::c_uint !=
                      4294967295 as JPEG_marker as libc::c_uint
              } {
        if marker as libc::c_uint == JM_SOI as libc::c_int as libc::c_uint ||
               marker as libc::c_uint >=
                   JM_RST0 as libc::c_int as libc::c_uint &&
                   marker as libc::c_uint <=
                       JM_RST7 as libc::c_int as libc::c_uint {
            ...
            *work_buffer.as_mut_ptr().offset(1) = marker as libc::c_char;
            ...
            match marker as libc::c_uint {
                192 | 193 | 194 | 195 | 197 | 198 | 199 | 201 | 202 | 203 |
                205 | 206 | 207 => {
                    *work_buffer.as_mut_ptr().offset(0) =
                        0xffi32 as libc::c_char;
                    *work_buffer.as_mut_ptr().offset(1) =
                        marker as libc::c_char;
                    ...
                    *work_buffer.as_mut_ptr().offset(3) =
                    ...
                    while length > 0i32 {
                        let mut nb_read: libc::c_int =
                            ttstub_input_read(handle,
                                              work_buffer.as_mut_ptr(),
                                              (if length < 1024i32 {
                                                   length
                                               } else { 1024i32 }) as size_t)
                                as libc::c_int;
                        if nb_read > 0i32 {
                            pdf_add_stream(stream,
                                           work_buffer.as_mut_ptr() as
                                               *const libc::c_void, nb_read);
                        }
                        length -= nb_read
                    }
                    found_SOFn = 1i32
                ...
                _ => {
                    if (*j_info).skipbits[(count / 8i32) as usize] as
                           libc::c_int & 1i32 << 7i32 - count % 8i32 != 0 {
    ...
    loop  {
        length =
            ttstub_input_read(handle, work_buffer.as_mut_ptr(),
                              (if (1024i32 as libc::c_ulong) <
                                      total_size.wrapping_sub(pos) {
                                   1024i32 as libc::c_ulong
                               } else { total_size.wrapping_sub(pos) })) as
                libc::c_int;
        if !(length > 0i32) { break ; }
        pdf_add_stream(stream,
                       work_buffer.as_mut_ptr() as *const libc::c_void,
                       length);
        pos =
...
unsafe extern "C" fn JPEG_scan_file(mut j_info: *mut JPEG_info,
                                    mut handle: rust_input_handle_t)
 -> libc::c_int {
    let mut marker: JPEG_marker = 0 as JPEG_marker;
    let mut found_SOFn: libc::c_int = 0;
    ...
    while found_SOFn == 0 &&
        ...
        if marker as libc::c_uint != JM_SOI as libc::c_int as libc::c_uint &&
            ...
            let mut length: libc::c_int =
                tt_get_unsigned_pair(handle) as libc::c_int - 2i32;
            match marker as libc::c_uint {
                192 | 193 | 194 | 195 | 197 | 198 | 199 | 201 | 202 | 203 |
                205 | 206 | 207 => {
                    (*j_info).bits_per_component =
                        tt_get_unsigned_byte(handle);
                ...
                224 => {
                    if length > 5i32 {
                        if ttstub_input_read(handle, app_sig.as_mut_ptr(),
                                             5i32 as size_t) !=
                               5i32 as libc::c_long {
                            return -1i32
                        }
                        length -= 5i32;
                        if memcmp(app_sig.as_mut_ptr() as *const libc::c_void,
                                  b"JFIF\x00\x00" as *const u8 as
                            ...
                            length =
                                (length as
                                     libc::c_ulong).wrapping_sub(read_APP0_JFIF(j_info,
                                                                                handle))
                                    as libc::c_int as libc::c_int
                        } else if memcmp(app_sig.as_mut_ptr() as
                                             *const libc::c_void,
                                         b"JFXX\x00" as *const u8 as
                                             *const libc::c_char as
                                             *const libc::c_void,
                                         5i32 as libc::c_ulong) == 0 {
                            length =
                                (length as
                                     libc::c_ulong).wrapping_sub(read_APP0_JFXX(handle,
                                                                                length
                                                                                    as
                                                                                    size_t))
                                    as libc::c_int as libc::c_int
                ...
                225 => {
                    if length > 5i32 {
                        ...
                        if memcmp(app_sig.as_mut_ptr() as *const libc::c_void,
                            ...
                            length =
                                (length as
                                     libc::c_ulong).wrapping_sub(read_APP1_Exif(j_info,
                                                                                handle,
                                                                                length
                                                                                    as
                                                                                    size_t))
                            ...
                            length -= 24i32;
                            if memcmp(app_sig.as_mut_ptr() as
                                          *const libc::c_void,
                                      b"//ns.adobe.com/xap/1.0/\x00\x00" as
                                          *const u8 as *const libc::c_char as
                                          *const libc::c_void,
                                      24i32 as libc::c_ulong) == 0 {
                                (*j_info).flags |= 1i32 << 4i32;
                                length =
                                    (length as
                                         libc::c_ulong).wrapping_sub(read_APP1_XMP(j_info,
                                                                                   handle,
                                                                                   length
                                                                                       as
                                                                                       size_t))
                                        as libc::c_int as libc::c_int;
                                if count < 1024i32 {
                                    (*j_info).skipbits[(count / 8i32) as
                                                           usize] =
                                        ((*j_info).skipbits[(count / 8i32) as
                                                                usize] as
                                             libc::c_int |
                                             1i32 << 7i32 - count % 8i32) as
                                            libc::c_char
                                }
                            }
                        }
                ...
                226 => {
                    if length >= 14i32 {
                        ...
                        length -= 12i32;
                        if memcmp(app_sig.as_mut_ptr() as *const libc::c_void,
                                  b"ICC_PROFILE\x00\x00" as *const u8 as
                                      *const libc::c_char as
                                      *const libc::c_void,
                                  12i32 as libc::c_ulong) == 0 {
                            (*j_info).flags |= 1i32 << 2i32;
                            length =
                                (length as
                                     libc::c_ulong).wrapping_sub(read_APP2_ICC(j_info,
                                                                               handle,
                                                                               length
                                                                                   as
                                                                                   size_t))
                ...
                238 => {
                    if length > 5i32 {
                        ...
                        if memcmp(app_sig.as_mut_ptr() as *const libc::c_void,
                            ...
                            length -=
                                read_APP14_Adobe(j_info, handle) as
                                    libc::c_int
                ...
                _ => {
                    ttstub_input_seek(handle, length as ssize_t, 1i32);
                    if marker as libc::c_uint >=
                           JM_APP0 as libc::c_int as libc::c_uint &&
                           marker as libc::c_uint <=
                               JM_APP15 as libc::c_int as libc::c_uint {
                        if count < 1024i32 {
                            (*j_info).skipbits[(count / 8i32) as usize] =
                                ((*j_info).skipbits[(count / 8i32) as usize]
                                     as libc::c_int |
...
*/
#[no_mangle]
pub unsafe extern "C" fn jpeg_get_bbox(mut handle: rust_input_handle_t,
