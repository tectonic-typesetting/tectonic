...
*/
    ...
    #[no_mangle]
    fn pst_get_token(inbuf: *mut *mut libc::c_uchar,
...
unsafe extern "C" fn t1_decrypt(mut key: libc::c_ushort,
                                mut dst: *mut libc::c_uchar,
                                mut src: *const libc::c_uchar,
                                mut skip: libc::c_int, mut len: libc::c_int) {
    len -= skip;
    loop  {
        let fresh0 = skip;
        skip = skip - 1;
        ...
        src = src.offset(1);
        key =
            ((key as libc::c_int + *fresh1 as libc::c_int) as
                 libc::c_uint).wrapping_mul(52845u32).wrapping_add(22719u32)
                as libc::c_ushort
    }
    loop  {
        let fresh2 = len;
        ...
        dst = dst.offset(1);
        *fresh4 =
            (c as libc::c_int ^ key as libc::c_int >> 8i32) as libc::c_uchar;
        key =
            ((key as libc::c_int + c as libc::c_int) as
                 libc::c_uint).wrapping_mul(52845u32).wrapping_add(22719u32)
                as libc::c_ushort
    };
}
/* T1CRYPT */
unsafe extern "C" fn get_next_key(mut start: *mut *mut libc::c_uchar,
                                  mut end: *mut libc::c_uchar)
 -> *mut libc::c_char {
    let mut key: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut tok: *mut pst_obj = 0 as *mut pst_obj;
    while *start < end && { tok = pst_get_token(start, end); !tok.is_null() }
          {
...
unsafe extern "C" fn seek_operator(mut start: *mut *mut libc::c_uchar,
                                   mut end: *mut libc::c_uchar,
 ...
 -> libc::c_int {
    ...
    while *start < end && { tok = pst_get_token(start, end); !tok.is_null() }
          {
        if !tok.is_null() && pst_type_of(tok) < 0i32 &&
               !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                              op).is_null() {
            break ;
        }
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
    }
    if tok.is_null() { return -1i32 }
    if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
    return 0i32;
}
unsafe extern "C" fn parse_svalue(mut start: *mut *mut libc::c_uchar,
                                  mut end: *mut libc::c_uchar,
...
unsafe extern "C" fn parse_bvalue(mut start: *mut *mut libc::c_uchar,
                                  mut end: *mut libc::c_uchar,
...
unsafe extern "C" fn parse_nvalue(mut start: *mut *mut libc::c_uchar,
                                  mut end: *mut libc::c_uchar,
                                  mut value: *mut libc::c_double,
    ...
    if (pst_type_of(tok) == 2i32 || pst_type_of(tok) == 3i32) && max > 0i32 {
        *value.offset(0) = pst_getRV(tok);
        ...
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        while *start < end &&
                  { tok = pst_get_token(start, end); !tok.is_null() } &&
        ...
        if !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
               ...
               !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
                     !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                    b"}\x00" as *const u8 as
                                        *const libc::c_char).is_null()) {
            argn = -1i32
 ...
 */
unsafe extern "C" fn try_put_or_putinterval(mut enc_vec:
                                                *mut *mut libc::c_char,
                                            mut start:
 ...
 -> libc::c_int {
    ...
    tok = pst_get_token(start, end);
    if tok.is_null() || !(pst_type_of(tok) == 2i32) ||
           { num1 = pst_getIV(tok); num1 > 255i32 } || num1 < 0i32 {
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        return -1i32
    }
    if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
    ...
    if tok.is_null() {
        ...
        if !tok.is_null() && pst_type_of(tok) < 0i32 &&
            ...
            tok = pst_get_token(start, end);
            if tok.is_null() || !(pst_type_of(tok) == 2i32) ||
                   { num2 = pst_getIV(tok); num2 > 255i32 } || num2 < 0i32 {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
                }
            ...
            *fresh6 = xstrdup(*enc_vec.offset(num2 as isize))
                      ...
                      { num2 = pst_getIV(tok); num2 + num1 <= 255i32 } &&
                      num2 >= 0i32 {
            if !tok.is_null() {
                pst_release_obj(tok);
                tok = 0 as *mut pst_obj
            ...
            if !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
                     !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                    b"getinterval\x00" as *const u8 as
                                        *const libc::c_char).is_null()) {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
            ...
            tok = pst_get_token(start, end);
            if tok.is_null() || !(pst_type_of(tok) == 2i32) ||
                   { num3 = pst_getIV(tok); num3 + num2 > 255i32 } ||
                   num3 < 0i32 {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
            ...
            if !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
                     !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                    b"putinterval\x00" as *const u8 as
                                        *const libc::c_char).is_null()) {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
            ...
            while i < num2 {
                if !(*enc_vec.offset((num1 + i) as isize)).is_null() {
                    ...
                    *fresh7 =
                        mfree(*enc_vec.offset((num3 + i) as isize) as
...
unsafe extern "C" fn parse_encoding(mut enc_vec: *mut *mut libc::c_char,
 ...
 -> libc::c_int {
    ...
    if !tok.is_null() && pst_type_of(tok) < 0i32 &&
           !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                          b"StandardEncoding\x00" as *const u8 as
                              *const libc::c_char).is_null() {
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        if !enc_vec.is_null() {
            code = 0i32;
            while code < 256i32 {
                if !StandardEncoding[code as usize].is_null() &&
                    ...
                    *fresh9 =
                    ...
                    strcpy(*enc_vec.offset(code as isize),
    ...
    } else if !tok.is_null() && pst_type_of(tok) < 0i32 &&
                  !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                 b"ISOLatin1Encoding\x00" as *const u8 as
                                     *const libc::c_char).is_null() {
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        if !enc_vec.is_null() {
            code = 0i32;
            while code < 256i32 {
                if !ISOLatin1Encoding[code as usize].is_null() &&
                       strcmp(ISOLatin1Encoding[code as usize],
                              b".notdef\x00" as *const u8 as
                                  *const libc::c_char) != 0i32 {
                    let ref mut fresh11 = *enc_vec.offset(code as isize);
                    *fresh11 =
                        new((strlen(ISOLatin1Encoding[code as
                                                          usize]).wrapping_add(1i32
                                                                                   as
                                                                                   libc::c_ulong)
                                 as uint32_t as
                ...
                code += 1
                  ...
                  !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                 b"ExpertEncoding\x00" as *const u8 as
                                     *const libc::c_char).is_null() {
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        if !enc_vec.is_null() {
            dpx_warning(b"ExpertEncoding not supported.\x00" as *const u8 as
                            *const libc::c_char);
            if !tok.is_null() {
                pst_release_obj(tok);
                tok = 0 as *mut pst_obj
        ...
        while *start < end &&
            ...
            if !tok.is_null() && pst_type_of(tok) < 0i32 &&
                   !strstartswith(pst_data_ptr(tok) as *const libc::c_char,
                                  b"def\x00" as *const u8 as
                                      *const libc::c_char).is_null() ||
                   !tok.is_null() && pst_type_of(tok) < 0i32 &&
                       !strstartswith(pst_data_ptr(tok) as
                                          *const libc::c_char,
                                      b"readonly\x00" as *const u8 as
                                          *const libc::c_char).is_null() {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
             ...
             */
                ...
                if !tok.is_null() && pst_type_of(tok) < 0i32 &&
                    ...
                    } else { try_put_or_putinterval(enc_vec, start, end); }
                    if !tok.is_null() {
                        pst_release_obj(tok);
                        tok = 0 as *mut pst_obj
                    }
                } else if tok.is_null() || !(pst_type_of(tok) == 2i32) ||
                              { code = pst_getIV(tok); code > 255i32 } ||
                              code < 0i32 {
                    if !tok.is_null() {
                        pst_release_obj(tok);
                        tok = 0 as *mut pst_obj
                    ...
                    if tok.is_null() || !(pst_type_of(tok) == 6i32) {
                        ...
                        if !enc_vec.is_null() {
                            free(*enc_vec.offset(code as isize) as
                                     *mut libc::c_void);
                            let ref mut fresh13 =
                        ...
                        if !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
                                                    ...
                                                    *const libc::c_char).is_null())
                           {
                            let ref mut fresh14 =
                                *enc_vec.offset(code as isize);
                            *fresh14 =
                                mfree(*enc_vec.offset(code as isize) as
                                          *mut libc::c_void) as
                                    *mut libc::c_char;
                            if !tok.is_null() {
                                pst_release_obj(tok);
...
unsafe extern "C" fn parse_subrs(mut font: *mut cff_font,
                                 mut start: *mut *mut libc::c_uchar,
                                 mut end: *mut libc::c_uchar,
    ...
    if !(pst_type_of(tok) == 2i32) || pst_getIV(tok) < 0i32 {
        dpx_warning(b"Parsing Subrs failed.\x00" as *const u8 as
                        *const libc::c_char);
        if !tok.is_null() { pst_release_obj(tok); tok = 0 as *mut pst_obj }
        return -1i32
    }
        ...
        lengths =
            new((count as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_int>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut libc::c_int;
        memset(offsets as *mut libc::c_void, 0i32,
               (::std::mem::size_of::<libc::c_int>() as
                    libc::c_ulong).wrapping_mul(count as libc::c_ulong));
        memset(lengths as *mut libc::c_void, 0i32,
               (::std::mem::size_of::<libc::c_int>() as
                    libc::c_ulong).wrapping_mul(count as libc::c_ulong));
    } else {
        max_size = 0i32;
                   ...
                   !(!tok.is_null() && pst_type_of(tok) < 0i32 &&
                         !strstartswith(pst_data_ptr(tok) as
                                        ...
                                        b"-|\x00" as *const u8 as
                                            *const libc::c_char).is_null()) &&
                   seek_operator(start, end,
                                 b"readstring\x00" as *const u8 as
                                     *const libc::c_char) < 0i32 {
                if !tok.is_null() {
                    pst_release_obj(tok);
                    tok = 0 as *mut pst_obj
            ...
            *start = (*start).offset(1);
            ...
            if mode != 1i32 {
                if offset + len >= max_size {
                    max_size += 65536i32;
                    data =
                        renew(data as *mut libc::c_void,
                              (max_size as uint32_t as
                                   libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                                   as
                                                                   libc::c_ulong)
                ...
                if lenIV >= 0i32 {
                    t1_decrypt(4330u32 as libc::c_ushort,
                               data.offset(offset as isize), *start, lenIV,
                               len);
                    ...
                    *lengths.offset(idx as isize) = len;
                    memcpy(&mut *data.offset(offset as isize) as *mut card8 as
                               *mut libc::c_void,
    ...
    if mode != 1i32 {
        if (*(*font).subrs.offset(0)).is_null() {
            ...
            *fresh17 = cff_new_index(count as card16);
            ...
            i = 0i32;
            while i < count {
                *(*subrs).offset.offset(i as isize) =
                    (offset + 1i32) as l_offset;
                if *lengths.offset(i as isize) > 0i32 {
                    memcpy((*subrs).data.offset(offset as isize) as
                               *mut libc::c_void,
                           data.offset(*offsets.offset(i as isize) as isize)
            ...
            *(*subrs).offset.offset(count as isize) =
                (offset + 1i32) as l_offset
        } else {
            ...
            dpx_warning(b"Already found /Subrs; ignores the other /Subrs dicts.\x00"
                            as *const u8 as *const libc::c_char);
        }
        free(data as *mut libc::c_void);
        free(offsets as *mut libc::c_void);
...
unsafe extern "C" fn parse_charstrings(mut font: *mut cff_font,
                                       mut start: *mut *mut libc::c_uchar,
                                       mut end: *mut libc::c_uchar,
                                       mut lenIV: libc::c_int,
                                       mut mode: libc::c_int) -> libc::c_int {
    ...
    if !(pst_type_of(tok) == 2i32) || pst_getIV(tok) < 0i32 ||
           pst_getIV(tok) > 64999i32 {
        let mut s: *mut libc::c_uchar = pst_getSV(tok);
        dpx_warning(b"Ignores non dict \"/CharStrings %s ...\"\x00" as
        ...
        (*charstrings).data =
            new((max_size as uint32_t as
                     libc::c_ulong).wrapping_mul(::std::mem::size_of::<card8>()
                                                     as libc::c_ulong) as
                    uint32_t) as *mut card8
    } else { charstrings = 0 as *mut cff_index; max_size = 0i32 }
    ...
    (*charset).data.glyphs =
        new(((count - 1i32) as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<s_SID>() as
                                                 libc::c_ulong) as uint32_t)
            as *mut s_SID;
    memset((*charset).data.glyphs as *mut libc::c_void, 0i32,
           (::std::mem::size_of::<s_SID>() as
                libc::c_ulong).wrapping_mul((count - 1i32) as libc::c_ulong));
    offset = 0i32;
    have_notdef = 0i32;
    ...
    while i < count {
        ...
        if pst_type_of(tok) == 6i32 {
            ...
            if glyph_name.is_null() {
                ...
                if streq_ptr(glyph_name,
                    ...
                    dpx_warning(b"No .notdef glyph???\x00" as *const u8 as
                                    *const libc::c_char);
                    return -1i32
                } else { gid = i + 1i32 }
            }
         ...
         */
            ...
            if (*start).offset(len as isize).offset(1) >= end { return -1i32 }
            if mode != 1i32 {
                if offset + len >= max_size {
                ...
                if gid == 0i32 {
                    if lenIV >= 0i32 {
                        memmove((*charstrings).data.offset(len as
                                                               isize).offset(-(lenIV
                                                                                   as
                                                                                   isize))
                                    as *mut libc::c_void,
                        ...
                        while j <= i {
                            ...
                            *fresh18 =
                                (*fresh18 as
                                     libc::c_uint).wrapping_add((len - lenIV)
                                                                    as
                                                                    libc::c_uint)
                        ...
                        memmove((*charstrings).data.offset(len as isize) as
                                    *mut libc::c_void,
                                (*charstrings).data as *const libc::c_void,
                                offset as libc::c_ulong);
                        j = 1i32;
                        while j <= i {
                            ...
                            *fresh19 =
                                (*fresh19 as
                                     libc::c_uint).wrapping_add(len as
                                                                    libc::c_uint)
                                    as l_offset as l_offset;
                            j += 1
                        }
                ...
                if lenIV >= 0i32 {
                    ...
                    *(*charstrings).offset.offset(gid as isize) =
                    ...
                    if gid == 0i32 {
                        *(*charstrings).offset.offset(gid as isize) =
                            1i32 as l_offset;
                        memcpy(&mut *(*charstrings).data.offset(0) as
                                   *mut card8 as *mut libc::c_void,
                               *start as *const libc::c_void,
                               len as libc::c_ulong);
                    } else {
                        *(*charstrings).offset.offset(gid as isize) =
                            (offset + 1i32) as l_offset;
                        memcpy(&mut *(*charstrings).data.offset(offset as
                                                                    isize) as
                                   *mut card8 as *mut libc::c_void,
                               *start as *const libc::c_void,
                               len as libc::c_ulong);
    ...
    if mode != 1i32 {
        *(*charstrings).offset.offset(count as isize) =
            (offset + 1i32) as l_offset
    }
    (*font).num_glyphs = count as card16;
    return 0i32;
}
unsafe extern "C" fn parse_part2(mut font: *mut cff_font,
                                 mut start: *mut *mut libc::c_uchar,
                                 mut end: *mut libc::c_uchar,
                                 mut mode: libc::c_int) -> libc::c_int {
    let mut key: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    while *start < end && { key = get_next_key(start, end); !key.is_null() } {
        if streq_ptr(key, b"Subrs\x00" as *const u8 as *const libc::c_char) {
        ...
        } else if streq_ptr(key,
                            b"CharStrings\x00" as *const u8 as
                                *const libc::c_char) {
            if parse_charstrings(font, start, end, lenIV, mode) < 0i32 {
                ...
                return -1i32
                            ...
                            b"lenIV\x00" as *const u8 as *const libc::c_char)
         {
            argn = parse_nvalue(start, end, argv.as_mut_ptr(), 1i32);
            if argn != 1i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
                                *const u8 as *const libc::c_char, 1i32, argn);
                free(key as *mut libc::c_void);
                return -1i32
            }
            lenIV = argv[0] as libc::c_int
        } else if streq_ptr(key,
                            b"BlueValues\x00" as *const u8 as
                                *const libc::c_char) as libc::c_int != 0 ||
                      streq_ptr(key,
                                b"OtherBlues\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"FamilyBlues\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"FamilyOtherBlues\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"StemSnapH\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"StemSnapV\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0 {
            /*
            ...
            if argn < 0i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
                                *const u8 as *const libc::c_char, 0i32, argn);
                free(key as *mut libc::c_void);
                return -1i32
            }
            ...
            loop  {
                let fresh20 = argn;
                argn = argn - 1;
                if !(fresh20 > 0i32) { break ; }
                cff_dict_set(*(*font).private.offset(0), key, argn,
                             if argn == 0i32 {
                                 argv[argn as usize]
                             } else {
                                 argv[argn as usize] -
                                     argv[(argn - 1i32) as usize]
                             });
            }
        } else if streq_ptr(key,
                            b"StdHW\x00" as *const u8 as *const libc::c_char)
                      as libc::c_int != 0 ||
                      streq_ptr(key,
                                b"StdVW\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"BlueScale\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"BlueShift\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"BlueFuzz\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"LanguageGroup\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"ExpansionFactor\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0 {
            /*
            ...
            cff_dict_set(*(*font).private.offset(0), key, 0i32, argv[0]);
                            ...
                            b"ForceBold\x00" as *const u8 as
                                *const libc::c_char) {
            argn =
            ...
            if argv[0] != 0i32 as libc::c_double {
                cff_dict_add(*(*font).private.offset(0), key, 1i32);
                cff_dict_set(*(*font).private.offset(0), key, 0i32,
         ...
         */
        free(key as
                 *mut libc::c_void); /* Macro CHECK_ARGN_XX assume 'argn' is used. */
    }
    return 0i32;
}
unsafe extern "C" fn parse_part1(mut font: *mut cff_font,
                                 mut enc_vec: *mut *mut libc::c_char,
                                 mut start: *mut *mut libc::c_uchar,
                                 mut end: *mut libc::c_uchar) -> libc::c_int {
    let mut key: *mut libc::c_char = 0 as *mut libc::c_char;
     ...
     */
    if seek_operator(start, end,
                     b"begin\x00" as *const u8 as *const libc::c_char) < 0i32
       {
        return -1i32
    }
    while *start < end && { key = get_next_key(start, end); !key.is_null() } {
        if streq_ptr(key, b"Encoding\x00" as *const u8 as *const libc::c_char)
           {
            ...
            if strlen(strval) > 127i32 as libc::c_ulong {
                dpx_warning(b"FontName too long: %s (%zu bytes)\x00" as
                                *const u8 as *const libc::c_char, strval,
                            strlen(strval));
            ...
            if argv[0] != 1.0f64 {
                dpx_warning(b"FontType %d not supported.\x00" as *const u8 as
                                *const libc::c_char, argv[0] as libc::c_int);
                free(key as *mut libc::c_void);
                return -1i32
            }
        } else if streq_ptr(key,
                            b"ItalicAngle\x00" as *const u8 as
                                *const libc::c_char) as libc::c_int != 0 ||
                      streq_ptr(key,
                                b"StrokeWidth\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"PaintType\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0 {
            argn = parse_nvalue(start, end, argv.as_mut_ptr(), 1i32);
            if argn != 1i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
            ...
            if argv[0] != 0.0f64 {
                ...
                cff_dict_set((*font).topdict, key, 0i32, argv[0]);
                            ...
                            b"UnderLinePosition\x00" as *const u8 as
                                *const libc::c_char) as libc::c_int != 0 ||
                      streq_ptr(key,
                                b"UnderLineThickness\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0 {
            argn = parse_nvalue(start, end, argv.as_mut_ptr(), 1i32);
            if argn != 1i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
            ...
            if argn != 4i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
                                *const u8 as *const libc::c_char, 4i32, argn);
                free(key as *mut libc::c_void);
                return -1i32
            }
            ...
            if argn != 6i32 {
                dpx_warning(b"%d values expected but only %d read.\x00" as
                                *const u8 as *const libc::c_char, 6i32, argn);
                free(key as *mut libc::c_void);
                return -1i32
            }
            if argv[0] != 0.001f64 || argv[1] != 0.0f64 || argv[2] != 0.0f64
                   || argv[3] != 0.001f64 || argv[4] != 0.0f64 ||
                   argv[5] != 0.0f64 {
                cff_dict_add((*font).topdict, key, 6i32);
                loop  {
                    let fresh22 = argn;
                    argn = argn - 1;
                    if !(fresh22 > 0i32) { break ; }
                    cff_dict_set((*font).topdict, key, argn,
                                 argv[argn as usize]);
                }
            }
        } else if streq_ptr(key,
                            b"version\x00" as *const u8 as
                                *const libc::c_char) as libc::c_int != 0 ||
                      streq_ptr(key,
                                b"Notice\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"FullName\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                                b"FamilyName\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0
                      ||
                      streq_ptr(key,
                      ...
                      ||
                      streq_ptr(key,
                                b"Copyright\x00" as *const u8 as
                                    *const libc::c_char) as libc::c_int != 0 {
            /*
             * FontInfo
             */
            ...
            /*
                 ...
                 * later a subset font of this font will be generated.
                 */
            cff_dict_set((*font).topdict, key, 0i32,
                         sid as libc::c_double); /* No Global Subr */
            free(strval as *mut libc::c_void);
        } else if streq_ptr(key,
...
pub unsafe extern "C" fn is_pfb(mut handle: rust_input_handle_t) -> bool {
    ...
    if ch != 128i32 || { ch = ttstub_input_getc(handle); ch < 0i32 } ||
           ch > 3i32 {
        return 0i32 != 0
    }
    i = 0i32;
    ...
    if memcmp(sig.as_mut_ptr() as *const libc::c_void,
              b"%!PS-AdobeFont\x00" as *const u8 as *const libc::c_char as
                  *const libc::c_void, 14i32 as libc::c_ulong) == 0 ||
           memcmp(sig.as_mut_ptr() as *const libc::c_void,
                  b"%!FontType1\x00" as *const u8 as *const libc::c_char as
                      *const libc::c_void, 11i32 as libc::c_ulong) == 0 {
        return 1i32 != 0
    }
    if memcmp(sig.as_mut_ptr() as *const libc::c_void,
              b"%!PS\x00" as *const u8 as *const libc::c_char as
                  *const libc::c_void, 4i32 as libc::c_ulong) == 0 {
        sig[14] = '\u{0}' as i32 as libc::c_char;
        dpx_warning(b"Ambiguous PostScript resource type: %s\x00" as *const u8
                        as *const libc::c_char, sig.as_mut_ptr());
        return 1i32 != 0
    }
    dpx_warning(b"Not a PFB font file?\x00" as *const u8 as
                    *const libc::c_char);
    return 0i32 != 0;
}
unsafe extern "C" fn get_pfb_segment(mut handle: rust_input_handle_t,
 ...
 -> *mut libc::c_uchar {
    ...
    loop  {
        ...
        if ch < 0i32 { break ; }
        if ch != 128i32 {
            _tt_abort(b"Not a pfb file?\x00" as *const u8 as
                          *const libc::c_char);
        }
        ch = ttstub_input_getc(handle);
        if ch < 0i32 || ch != expected_type {
            ...
            buffer =
                renew(buffer as *mut libc::c_void,
                      ((bytesread + slen) as uint32_t as
            ...
            while slen > 0i32 {
                rlen =
                    ttstub_input_read(handle,
                                      (buffer as
                                           *mut libc::c_char).offset(bytesread
                                                                         as
                                                                         isize),
                                      slen as size_t) as libc::c_int;
                if rlen < 0i32 {
                    free(buffer as *mut libc::c_void);
                    return 0 as *mut libc::c_uchar
                }
    ...
    if bytesread == 0i32 {
        _tt_abort(b"PFB segment length zero?\x00" as *const u8 as
                      *const libc::c_char);
    }
    buffer =
        renew(buffer as *mut libc::c_void,
              ((bytesread + 1i32) as uint32_t as
                   libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                   as libc::c_ulong) as
                  uint32_t) as *mut libc::c_uchar;
    *buffer.offset(bytesread as isize) = 0i32 as libc::c_uchar;
    if !length.is_null() { *length = bytesread }
    return buffer;
}
#[no_mangle]
pub unsafe extern "C" fn t1_get_standard_glyph(mut code: libc::c_int)
 -> *const libc::c_char {
    if StandardEncoding[code as usize].is_null() {
        return 0 as *const libc::c_char
    }
...
#[no_mangle]
pub unsafe extern "C" fn t1_get_fontname(mut handle: rust_input_handle_t,
 ...
 -> libc::c_int {
    ...
    if buffer.is_null() || length == 0i32 {
        _tt_abort(b"Reading PFB (ASCII part) file failed.\x00" as *const u8 as
                      *const libc::c_char);
    }
    start = buffer;
    end = buffer.offset(length as isize);
    if seek_operator(&mut start, end,
                     b"begin\x00" as *const u8 as *const libc::c_char) < 0i32
       {
        free(buffer as *mut libc::c_void);
        return -1i32
...
*/
#[no_mangle]
pub unsafe extern "C" fn t1_load_font(mut enc_vec: *mut *mut libc::c_char,
                                      mut mode: libc::c_int,
 ...
 -> *mut cff_font {
    ...
    if buffer.is_null() || length == 0i32 {
        cff_close(cff);
        free(buffer as *mut libc::c_void);
        _tt_abort(b"Reading PFB (BINARY part) file failed.\x00" as *const u8
