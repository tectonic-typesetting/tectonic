...
pub unsafe extern "C" fn tex_round(mut r: libc::c_double) -> int32_t {
     ...
     * code. */
    if r >= 0.0f64 { return (r + 0.5f64) as int32_t }
    return (r - 0.5f64) as int32_t;
}
#[no_mangle]
pub unsafe extern "C" fn half(mut x: int32_t) -> int32_t {
    if x & 1i32 != 0 { return (x + 1i32) / 2i32 }
    return x / 2i32;
}
#[no_mangle]
pub unsafe extern "C" fn mult_and_add(mut n: int32_t, mut x: scaled_t,
                                      mut y: scaled_t,
                                      mut max_answer: scaled_t) -> scaled_t {
    if n < 0i32 { x = -x; n = -n }
    if n == 0i32 {
        return y
    } else if x <= (max_answer - y) / n && -x <= (max_answer + y) / n {
        return n * x + y
    } else { arith_error = 1i32 != 0; return 0i32 };
}
#[no_mangle]
pub unsafe extern "C" fn x_over_n(mut x: scaled_t, mut n: int32_t)
 -> scaled_t {
    if n == 0i32 {
        arith_error = 1i32 != 0;
        tex_remainder = x;
        ...
        if x >= 0i32 {
            tex_remainder = x % n;
            return x / n
        } else { tex_remainder = -(-x % n); return -(-x / n) }
    };
}
/* xetex-errors */
/* xetex-math */
...
/* xetex-scaledmath */
#[no_mangle]
pub unsafe extern "C" fn xn_over_d(mut x: scaled_t, mut n: int32_t,
                                   mut d: int32_t) -> scaled_t {
    ...
    if x >= 0i32 {
        positive = 1i32 != 0
    } else { x = -x; positive = 0i32 != 0 }
    t = (x as libc::c_long % 32768i64 * n as libc::c_long) as int32_t;
    u =
        (x as libc::c_long / 32768i64 * n as libc::c_long +
             t as libc::c_long / 32768i64) as int32_t;
    v =
        ((u % d) as libc::c_long * 32768i64 + t as libc::c_long % 32768i64) as
            int32_t;
    if (u / d) as libc::c_long >= 32768i64 {
        arith_error = 1i32 != 0
    } else {
        u =
            (32768i64 * (u / d) as libc::c_long + (v / d) as libc::c_long) as
                int32_t
    }
    if positive {
        tex_remainder = v % d;
        return u
    } else { tex_remainder = -(v % d); return -u };
}
#[no_mangle]
pub unsafe extern "C" fn round_xn_over_d(mut x: scaled_t, mut n: int32_t,
                                         mut d: int32_t) -> scaled_t {
    let mut positive: bool = false;
    let mut t: int32_t = 0;
    let mut u: int32_t = 0;
    ...
    v = v % d;
    if 2i32 * v >= d { u += 1 }
    if positive { return u } else { return -u };
}
