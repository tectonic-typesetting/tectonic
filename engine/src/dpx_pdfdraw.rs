...
unsafe extern "C" fn init_a_path(mut p: *mut pdf_path) {
    if !p.is_null() {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            212i32 as u32,
            (*::std::mem::transmute::<&[u8; 29], &[i8; 29]>(b"void init_a_path(pdf_path *)\x00"))
                .as_ptr(),
        );
    }
    (*p).num_paths = 0i32 as u32;
    (*p).max_paths = 0i32 as u32;
...
unsafe extern "C" fn pdf_path__growpath(mut p: *mut pdf_path, mut max_pe: u32) -> i32 {
    ...
    (*p).max_paths = if (*p).max_paths.wrapping_add(8i32 as u32) > max_pe {
        (*p).max_paths.wrapping_add(8i32 as u32)
    } else {
        max_pe
    };
    (*p).path = renew(
...
unsafe extern "C" fn clear_a_path(mut p: *mut pdf_path) {
    if !p.is_null() {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            246i32 as u32,
            (*::std::mem::transmute::<&[u8; 30], &[i8; 30]>(b"void clear_a_path(pdf_path *)\x00"))
                .as_ptr(),
        );
    }
...
unsafe extern "C" fn pdf_path__moveto(
    ...
    pdf_path__growpath(pa, (*pa).num_paths.wrapping_add(1i32 as u32));
    if (*pa).num_paths > 0i32 as u32 {
        pe = &mut *(*pa)
            .path
            .offset((*pa).num_paths.wrapping_sub(1i32 as u32) as isize)
            as *mut pa_elem;
        if (*pe).type_0 == 0i32 {
            (*cp).x = (*p0).x;
            (*pe).p[0].x = (*cp).x;
...
unsafe extern "C" fn pdf_path__next_pe(
    ...
    let mut pe: *mut pa_elem = 0 as *mut pa_elem;
    pdf_path__growpath(pa, (*pa).num_paths.wrapping_add(2i32 as u32));
    if (*pa).num_paths == 0i32 as u32 {
    ...
    pe = &mut *(*pa)
        .path
        .offset((*pa).num_paths.wrapping_sub(1i32 as u32) as isize) as *mut pa_elem;
    match (*pe).type_0 {
        0 => {
            (*pe).p[0].x = (*cp).x;
            (*pe).p[0].y = (*cp).y
...
/* no arg */ {
    ...
    i = (*pa).num_paths.wrapping_sub(1i32 as u32) as i32; /* No path or no start point(!) */
    while i >= 0i32 {
        pe = &mut *(*pa).path.offset(i as isize) as *mut pa_elem;
        if (*pe).type_0 == 0i32 {
            break;
...
unsafe extern "C" fn pdf_dev__flushpath(
    ...
    if (*pa).num_paths <= 0i32 as u32 && path_added == 0i32 {
        return 0i32;
    }
    path_added = 0i32;
    graphics_mode();
...
unsafe extern "C" fn m_stack_init(mut stack: *mut m_stack) {
    if !stack.is_null() {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            850i32 as u32,
            (*::std::mem::transmute::<&[u8; 29], &[i8; 29]>(b"void m_stack_init(m_stack *)\x00"))
                .as_ptr(),
        );
    }
    (*stack).size = 0i32;
...
unsafe extern "C" fn m_stack_push(mut stack: *mut m_stack, mut data: *mut libc::c_void) {
    ...
    elem =
        new((1i32 as u32 as u64).wrapping_mul(::std::mem::size_of::<m_stack_elem>() as u64) as u32)
            as *mut m_stack_elem;
    (*elem).prev = (*stack).top;
    (*elem).data = data;
    (*stack).top = elem;
...
unsafe extern "C" fn m_stack_pop(mut stack: *mut m_stack) -> *mut libc::c_void {
    ...
    if !stack.is_null() {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            885i32 as u32,
            (*::std::mem::transmute::<&[u8; 29], &[i8; 29]>(b"void *m_stack_pop(m_stack *)\x00"))
                .as_ptr(),
        );
    }
    if (*stack).size == 0i32 {
...
unsafe extern "C" fn m_stack_top(mut stack: *mut m_stack) -> *mut libc::c_void {
    ...
    if !stack.is_null() {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            907i32 as u32,
            (*::std::mem::transmute::<&[u8; 29], &[i8; 29]>(b"void *m_stack_top(m_stack *)\x00"))
                .as_ptr(),
        );
    }
    if (*stack).size == 0i32 {
...
pub unsafe extern "C" fn pdf_dev_init_gstates() {
    ...
    gs = new((1i32 as u32 as u64).wrapping_mul(::std::mem::size_of::<pdf_gstate>() as u64) as u32)
        as *mut pdf_gstate;
    init_a_gstate(gs);
...
pub unsafe extern "C" fn pdf_dev_gsave() -> i32 {
    ...
    gs1 = new((1i32 as u32 as u64).wrapping_mul(::std::mem::size_of::<pdf_gstate>() as u64) as u32)
        as *mut pdf_gstate;
    init_a_gstate(gs1);
...
pub unsafe extern "C" fn pdf_dev_push_gstate() -> i32 {
    ...
    let mut gs0: *mut pdf_gstate = 0 as *mut pdf_gstate;
    gs0 = new((1i32 as u32 as u64).wrapping_mul(::std::mem::size_of::<pdf_gstate>() as u64) as u32)
        as *mut pdf_gstate;
    init_a_gstate(gs0);
...
pub unsafe extern "C" fn pdf_dev_grestore_to(mut depth: i32) {
    ...
    if depth >= 0i32 {
        ...
        __assert_fail(
            ...
            b"dpx-pdfdraw.c\x00" as *const u8 as *const i8,
            1113i32 as u32,
            (*::std::mem::transmute::<&[u8; 30], &[i8; 30]>(b"void pdf_dev_grestore_to(int)\x00"))
                .as_ptr(),
        );
    }
...
pub unsafe extern "C" fn pdf_dev_setdash(
    ...
    pdf_doc_add_page_content(buf, len as u32);
    pdf_doc_add_page_content(b" d\x00" as *const u8 as *const i8, 2i32 as u32);
    return 0i32;
}
/* ZSYUEDVEDEOF */
#[no_mangle]
...
pub unsafe extern "C" fn pdf_dev_newpath() -> i32 {
    ...
    if (*p).num_paths > 0i32 as u32 {
        pdf_path__clearpath(p);
    }
    /* The following is required for "newpath" operator in mpost.c. */
    pdf_doc_add_page_content(b" n\x00" as *const u8 as *const i8, 2i32 as u32); /* op: n */
    return 0i32;
}
#[no_mangle]
