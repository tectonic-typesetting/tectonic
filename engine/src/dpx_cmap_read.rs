...
extern "C" {
    ...
    fn CMap_add_codespacerange(cmap: *mut CMap, codelo: *const libc::c_uchar,
     ...
     -> libc::c_int;
    #[no_mangle]
    fn CMap_add_notdefrange(cmap: *mut CMap, srclo: *const libc::c_uchar,
                            srchi: *const libc::c_uchar, srcdim: size_t,
                            dst: CID) -> libc::c_int;
    #[no_mangle]
    fn CMap_add_notdefchar(cmap: *mut CMap, src: *const libc::c_uchar,
                           srcdim: size_t, dst: CID) -> libc::c_int;
    #[no_mangle]
    fn CMap_add_cidrange(cmap: *mut CMap, srclo: *const libc::c_uchar,
                         hi: *const libc::c_uchar, srcdim: size_t, base: CID)
     -> libc::c_int;
    #[no_mangle]
    fn CMap_add_bfrange(cmap: *mut CMap, srclo: *const libc::c_uchar,
                        srchi: *const libc::c_uchar, srcdim: size_t,
                        dest: *const libc::c_uchar, destdim: size_t)
     -> libc::c_int;
    #[no_mangle]
...
unsafe extern "C" fn ifreader_create(mut handle: rust_input_handle_t,
                                     mut size: size_t, mut bufsize: size_t)
 -> *mut ifreader {
    let mut reader: *mut ifreader = 0 as *mut ifreader;
    reader =
        new((1i32 as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<ifreader>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut ifreader;
    (*reader).buf =
        new((bufsize.wrapping_add(1i32 as libc::c_ulong) as uint32_t as
...
unsafe extern "C" fn ifreader_destroy(mut reader: *mut ifreader) {
    if !reader.is_null() {
        ...
        __assert_fail(b"reader\x00" as *const u8 as *const libc::c_char,
                      b"dpx-cmap_read.c\x00" as *const u8 as
                          *const libc::c_char, 77i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 34],
                                                &[libc::c_char; 34]>(b"void ifreader_destroy(ifreader *)\x00")).as_ptr());
    }
    free((*reader).buf as *mut libc::c_void);
    free(reader as *mut libc::c_void);
}
unsafe extern "C" fn ifreader_read(mut reader: *mut ifreader,
    ...
    if !reader.is_null() {
        ...
        __assert_fail(b"reader\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 41],
                                                &[libc::c_char; 41]>(b"size_t ifreader_read(ifreader *, size_t)\x00")).as_ptr());
    }
    bytesrem =
        ((*reader).endptr as size_t).wrapping_sub((*reader).cursor as size_t);
    if size > (*reader).max {
        if __verbose != 0 {
        ...
        (*reader).buf =
            renew((*reader).buf as *mut libc::c_void,
                  (size.wrapping_add(1i32 as libc::c_ulong) as uint32_t as
                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                       as libc::c_ulong) as
                      uint32_t) as *mut libc::c_uchar;
        (*reader).max = size
    ...
    if (*reader).unread > 0i32 as libc::c_ulong && bytesrem < size {
        bytesread =
            if (*reader).max.wrapping_sub(bytesrem) < (*reader).unread {
                (*reader).max.wrapping_sub(bytesrem)
            } else { (*reader).unread };
        memmove((*reader).buf as *mut libc::c_void,
                (*reader).cursor as *const libc::c_void, bytesrem);
        (*reader).cursor = (*reader).buf;
        (*reader).endptr = (*reader).buf.offset(bytesrem as isize);
        if ttstub_input_read((*reader).handle,
                             (*reader).endptr as *mut libc::c_char, bytesread)
               as libc::c_ulong != bytesread {
            _tt_abort(b"Reading file failed.\x00" as *const u8 as
                          *const libc::c_char);
        }
        (*reader).endptr = (*reader).endptr.offset(bytesread as isize);
        (*reader).unread =
            ((*reader).unread as libc::c_ulong).wrapping_sub(bytesread) as
                size_t as size_t;
        if __verbose != 0 {
            dpx_message(b"Reading more %zu bytes (%zu bytes remains in buffer)...\n\x00"
                            as *const u8 as *const libc::c_char, bytesread,
                        bytesrem);
        }
    }
    *(*reader).endptr = 0i32 as libc::c_uchar;
...
unsafe extern "C" fn check_next_token(mut input: *mut ifreader,
...
unsafe extern "C" fn get_coderange(mut input: *mut ifreader,
    ...
    if tok1.is_null() { return -1i32 }
    tok2 = pst_get_token(&mut (*input).cursor, (*input).endptr);
    if tok2.is_null() { pst_release_obj(tok1); return -1i32 }
    ...
    memcpy(codeLo as *mut libc::c_void, pst_data_ptr(tok1),
           dim1 as libc::c_ulong);
    memcpy(codeHi as *mut libc::c_void, pst_data_ptr(tok2),
...
unsafe extern "C" fn do_codespacerange(mut cmap: *mut CMap,
 ...
 -> libc::c_int {
    ...
    loop  {
        ...
        if get_coderange(input, codeLo.as_mut_ptr(), codeHi.as_mut_ptr(),
                         &mut dim, 127i32) < 0i32 {
            return -1i32
        }
        CMap_add_codespacerange(cmap, codeLo.as_mut_ptr(),
                                codeHi.as_mut_ptr(), dim as size_t);
    }
    return check_next_token(input,
                            b"endcodespacerange\x00" as *const u8 as
                                *const libc::c_char);
}
/*
 * bfrange
 ...
 */
unsafe extern "C" fn handle_codearray(mut cmap: *mut CMap,
                                      mut input: *mut ifreader,
                                      mut codeLo: *mut libc::c_uchar,
                                      mut dim: libc::c_int,
    ...
    if dim < 1i32 {
        _tt_abort(b"Invalid code range.\x00" as *const u8 as
                      *const libc::c_char);
    }
    loop  {
        let fresh1 = count;
        ...
        if tok.is_null() {
            ...
            if pst_type_of(tok) == 5i32 {
                CMap_add_bfchar(cmap, codeLo, dim as size_t,
                                pst_data_ptr(tok) as *mut libc::c_uchar,
                                pst_length_of(tok) as size_t);
            } else if pst_type_of(tok) == 7i32 || !(pst_type_of(tok) == 6i32)
             {
                _tt_abort(b"%s: Invalid CMap mapping record.\x00" as *const u8
                              as *const libc::c_char,
                          b"CMap_parse:\x00" as *const u8 as
                              *const libc::c_char);
            } else {
                _tt_abort(b"%s: Mapping to charName not supported.\x00" as
...
unsafe extern "C" fn do_notdefrange(mut cmap: *mut CMap,
                                    mut input: *mut ifreader,
                                    mut count: libc::c_int) -> libc::c_int {
    let mut tok: *mut pst_obj = 0 as *mut pst_obj;
    let mut codeLo: [libc::c_uchar; 127] = [0; 127];
    ...
    loop  {
        ...
        if ifreader_read(input, (127i32 * 3i32) as size_t) ==
        ...
        if pst_type_of(tok) == 2i32 {
            ...
            if dstCID >= 0i32 && dstCID <= 65535i32 {
                CMap_add_notdefrange(cmap, codeLo.as_mut_ptr(),
                                     codeHi.as_mut_ptr(), dim as size_t,
                                     dstCID as CID);
            }
        } else {
            dpx_warning(b"%s: Invalid CMap mapping record. (ignored)\x00" as
                            *const u8 as *const libc::c_char,
                        b"CMap_parse:\x00" as *const u8 as
                            *const libc::c_char);
        }
    ...
    return check_next_token(input,
                            b"endnotdefrange\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_bfrange(mut cmap: *mut CMap, mut input: *mut ifreader,
                                mut count: libc::c_int) -> libc::c_int {
    let mut tok: *mut pst_obj = 0 as *mut pst_obj;
    let mut codeLo: [libc::c_uchar; 127] = [0; 127];
    let mut codeHi: [libc::c_uchar; 127] = [0; 127];
    ...
    loop  {
        ...
        if pst_type_of(tok) == 5i32 {
            CMap_add_bfrange(cmap, codeLo.as_mut_ptr(), codeHi.as_mut_ptr(),
                             srcdim as size_t,
                             pst_data_ptr(tok) as *mut libc::c_uchar,
                             pst_length_of(tok) as size_t);
        } else if pst_type_of(tok) == 7i32 {
            if handle_codearray(cmap, input, codeLo.as_mut_ptr(), srcdim,
                                codeHi[(srcdim - 1i32) as usize] as
                                    libc::c_int -
                                    codeLo[(srcdim - 1i32) as usize] as
    ...
    return check_next_token(input,
                            b"endbfrange\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_cidrange(mut cmap: *mut CMap,
                                 mut input: *mut ifreader,
                                 mut count: libc::c_int) -> libc::c_int {
    let mut tok: *mut pst_obj = 0 as *mut pst_obj;
    let mut codeLo: [libc::c_uchar; 127] = [0; 127];
    ...
    loop  {
        ...
        if pst_type_of(tok) == 2i32 {
            dstCID = pst_getIV(tok);
            if dstCID >= 0i32 && dstCID <= 65535i32 {
                CMap_add_cidrange(cmap, codeLo.as_mut_ptr(),
                                  codeHi.as_mut_ptr(), dim as size_t,
                                  dstCID as CID);
            }
        } else {
    ...
    return check_next_token(input,
                            b"endcidrange\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_notdefchar(mut cmap: *mut CMap,
                                   mut input: *mut ifreader,
                                   mut count: libc::c_int) -> libc::c_int {
    let mut tok1: *mut pst_obj = 0 as *mut pst_obj;
    let mut tok2: *mut pst_obj = 0 as *mut pst_obj;
    ...
    loop  {
        ...
        if ifreader_read(input, (127i32 * 2i32) as size_t) ==
               0i32 as libc::c_ulong {
            return -1i32
        }
        tok1 = pst_get_token(&mut (*input).cursor, (*input).endptr);
        ...
        if pst_type_of(tok1) == 5i32 && pst_type_of(tok2) == 2i32 {
            ...
            if dstCID >= 0i32 && dstCID <= 65535i32 {
                CMap_add_notdefchar(cmap,
                                    pst_data_ptr(tok1) as
                                        *const libc::c_uchar,
    ...
    return check_next_token(input,
                            b"endnotdefchar\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_bfchar(mut cmap: *mut CMap, mut input: *mut ifreader,
                               mut count: libc::c_int) -> libc::c_int {
    let mut tok1: *mut pst_obj = 0 as *mut pst_obj;
    let mut tok2: *mut pst_obj = 0 as *mut pst_obj;
    loop  {
        ...
        if pst_type_of(tok1) == 5i32 && pst_type_of(tok2) == 5i32 {
            CMap_add_bfchar(cmap, pst_data_ptr(tok1) as *mut libc::c_uchar,
                            pst_length_of(tok1) as size_t,
    ...
    return check_next_token(input,
                            b"endbfchar\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_cidchar(mut cmap: *mut CMap, mut input: *mut ifreader,
                                mut count: libc::c_int) -> libc::c_int {
    let mut tok1: *mut pst_obj = 0 as *mut pst_obj;
    let mut tok2: *mut pst_obj = 0 as *mut pst_obj;
    let mut dstCID: libc::c_int = 0;
    ...
    return check_next_token(input,
                            b"endcidchar\x00" as *const u8 as
                                *const libc::c_char);
}
unsafe extern "C" fn do_cidsysteminfo(mut cmap: *mut CMap,
                                      mut input: *mut ifreader)
 -> libc::c_int {
    let mut tok1: *mut pst_obj = 0 as *mut pst_obj;
    let mut tok2: *mut pst_obj = 0 as *mut pst_obj;
    ...
    while error == 0 &&
        ...
        if pst_type_of(tok1) < 0i32 &&
            ...
            break ;
                      ...
                      memcmp(pst_data_ptr(tok1),
                             b"end\x00" as *const u8 as *const libc::c_char as
                                 *const libc::c_void,
                             strlen(b"end\x00" as *const u8 as
                                        *const libc::c_char)) == 0 &&
                      simpledict == 0 {
            pst_release_obj(tok1);
            break ;
            ...
            if pst_type_of(tok1) == 6i32 &&
                   memcmp(pst_data_ptr(tok1),
                          b"Registry\x00" as *const u8 as *const libc::c_char
                              as *const libc::c_void,
                          strlen(b"Registry\x00" as *const u8 as
                                     *const libc::c_char)) == 0 &&
                   {
                       tok2 =
                           pst_get_token(&mut (*input).cursor,
                                         (*input).endptr);
                       !tok2.is_null()
                   } {
                    ...
                    csi.registry = pst_getSV(tok2) as *mut libc::c_char
                          ...
                          memcmp(pst_data_ptr(tok1),
                                 b"Ordering\x00" as *const u8 as
                                     ...
                                     *const libc::c_void,
                                 strlen(b"Ordering\x00" as *const u8 as
                                            *const libc::c_char)) == 0 &&
                          {
                              tok2 =
                                  pst_get_token(&mut (*input).cursor,
                    ...
                    csi.ordering = pst_getSV(tok2) as *mut libc::c_char
                          ...
                          memcmp(pst_data_ptr(tok1),
                                 b"Supplement\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"Supplement\x00" as *const u8 as
                                            *const libc::c_char)) == 0 &&
                          {
                              tok2 =
                                  pst_get_token(&mut (*input).cursor,
                ...
                if error == 0 { csi.supplement = pst_getIV(tok2) }
            }
            if !tok2.is_null() { pst_release_obj(tok2); }
            if !tok1.is_null() { pst_release_obj(tok1); }
            tok2 = 0 as *mut pst_obj;
            tok1 = tok2
        }
    }
    ...
    if error == 0 && !csi.registry.is_null() && !csi.ordering.is_null() &&
           csi.supplement >= 0i32 {
...
#[no_mangle]
pub unsafe extern "C" fn CMap_parse_check_sig(mut handle: rust_input_handle_t)
 -> libc::c_int {
    ...
    if ttstub_input_read(handle, sig.as_mut_ptr(), 64i32 as size_t) !=
           64i32 as libc::c_long {
        result = -1i32
    } else {
        sig[64] = 0i32 as libc::c_char;
        if strstartswith(sig.as_mut_ptr(),
                         b"%!PS\x00" as *const u8 as
                             *const libc::c_char).is_null() {
            result = -1i32
        } else if !strstr(sig.as_mut_ptr().offset(4),
                          b"Resource-CMap\x00" as *const u8 as
                              *const libc::c_char).is_null() {
            result = 0i32
        }
    }
...
*/
#[no_mangle]
pub unsafe extern "C" fn CMap_parse(mut cmap: *mut CMap,
 ...
 -> libc::c_int {
    ...
    if !cmap.is_null() && !handle.is_null() {
        ...
        __assert_fail(b"cmap && handle\x00" as *const u8 as
                          *const libc::c_char,
                      b"dpx-cmap_read.c\x00" as *const u8 as
                          *const libc::c_char, 519i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 44],
                                                &[libc::c_char; 44]>(b"int CMap_parse(CMap *, rust_input_handle_t)\x00")).as_ptr());
    }
    input =
        ifreader_create(handle, ttstub_input_get_size(handle),
    ...
    while status >= 0i32 {
        ...
        if pst_type_of(tok1) == 6i32 &&
               memcmp(pst_data_ptr(tok1),
                      b"CMapName\x00" as *const u8 as *const libc::c_char as
                          *const libc::c_void,
                      strlen(b"CMapName\x00" as *const u8 as
                                 *const libc::c_char)) == 0 {
            tok2 = pst_get_token(&mut (*input).cursor, (*input).endptr);
            if tok2.is_null() ||
                ...
                CMap_set_name(cmap,
                              pst_data_ptr(tok2) as *const libc::c_char);
            }
        } else if pst_type_of(tok1) == 6i32 &&
                      memcmp(pst_data_ptr(tok1),
                             b"CMapType\x00" as *const u8 as
                                 *const libc::c_char as *const libc::c_void,
                             strlen(b"CMapType\x00" as *const u8 as
                                        *const libc::c_char)) == 0 {
            tok2 = pst_get_token(&mut (*input).cursor, (*input).endptr);
            if tok2.is_null() || !(pst_type_of(tok2) == 2i32) ||
                   check_next_token(input,
                                    b"def\x00" as *const u8 as
                                        *const libc::c_char) < 0i32 {
                status = -1i32
            } else { CMap_set_type(cmap, pst_getIV(tok2)); }
        } else if pst_type_of(tok1) == 6i32 &&
                      memcmp(pst_data_ptr(tok1),
                             b"WMode\x00" as *const u8 as *const libc::c_char
                                 as *const libc::c_void,
                             strlen(b"WMode\x00" as *const u8 as
                                        *const libc::c_char)) == 0 {
            tok2 = pst_get_token(&mut (*input).cursor, (*input).endptr);
            if tok2.is_null() || !(pst_type_of(tok2) == 2i32) ||
                   check_next_token(input,
                                    b"def\x00" as *const u8 as
                                        *const libc::c_char) < 0i32 {
                status = -1i32
            } else { CMap_set_wmode(cmap, pst_getIV(tok2)); }
        } else if pst_type_of(tok1) == 6i32 &&
                      memcmp(pst_data_ptr(tok1),
                             b"CIDSystemInfo\x00" as *const u8 as
                                 *const libc::c_char as *const libc::c_void,
                             strlen(b"CIDSystemInfo\x00" as *const u8 as
                                        *const libc::c_char)) == 0 {
            status = do_cidsysteminfo(cmap, input)
                        ...
                        memcmp(pst_data_ptr(tok1),
                               b"Version\x00" as *const u8 as
                                   *const libc::c_char as *const libc::c_void,
                               strlen(b"Version\x00" as *const u8 as
                                          *const libc::c_char)) == 0 ||
                        pst_type_of(tok1) == 6i32 &&
                            memcmp(pst_data_ptr(tok1),
                                   b"UIDOffset\x00" as *const u8 as
                                       *const libc::c_char as
                                       *const libc::c_void,
                                   strlen(b"UIDOffset\x00" as *const u8 as
                                              *const libc::c_char)) == 0 ||
                        pst_type_of(tok1) == 6i32 &&
                            memcmp(pst_data_ptr(tok1),
                                   b"XUID\x00" as *const u8 as
                                       *const libc::c_char as
                                       *const libc::c_void,
                                   strlen(b"XUID\x00" as *const u8 as
                                              *const libc::c_char)) == 0) {
            if pst_type_of(tok1) == 6i32 {
                ...
                if !tok2.is_null() &&
                       (pst_type_of(tok2) < 0i32 &&
                            memcmp(pst_data_ptr(tok2),
                                   b"usecmap\x00" as *const u8 as
                                       *const libc::c_char as
                                       *const libc::c_void,
                                   strlen(b"usecmap\x00" as *const u8 as
                                              *const libc::c_char)) == 0) {
                    let mut id: libc::c_int = 0;
                    let mut ucmap: *mut CMap = 0 as *mut CMap;
                    id =
                        CMap_cache_find(pst_data_ptr(tok1) as
                                            *const libc::c_char);
                    if id < 0i32 {
                        status = -1i32
                    } else {
                        ...
                        CMap_set_usecmap(cmap, ucmap);
                          ...
                          memcmp(pst_data_ptr(tok1),
                                 b"begincodespacerange\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"begincodespacerange\x00" as
                                            *const u8 as *const libc::c_char))
                              == 0 {
                status = do_codespacerange(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"beginnotdefrange\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"beginnotdefrange\x00" as *const u8
                                            as *const libc::c_char)) == 0 {
                status = do_notdefrange(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"beginnotdefchar\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"beginnotdefchar\x00" as *const u8 as
                                            *const libc::c_char)) == 0 {
                status = do_notdefchar(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"beginbfrange\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"beginbfrange\x00" as *const u8 as
                                            *const libc::c_char)) == 0 {
                status = do_bfrange(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"beginbfchar\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"beginbfchar\x00" as *const u8 as
                                            *const libc::c_char)) == 0 {
                status = do_bfchar(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"begincidrange\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"begincidrange\x00" as *const u8 as
                                            *const libc::c_char)) == 0 {
                status = do_cidrange(cmap, input, tmpint)
            } else if pst_type_of(tok1) < 0i32 &&
                          memcmp(pst_data_ptr(tok1),
                                 b"begincidchar\x00" as *const u8 as
                                     *const libc::c_char as
                                     *const libc::c_void,
                                 strlen(b"begincidchar\x00" as *const u8 as
                                            *const libc::c_char)) == 0 {
                status = do_cidchar(cmap, input, tmpint)
            } else if pst_type_of(tok1) == 2i32 { tmpint = pst_getIV(tok1) }
        }
        if !tok1.is_null() { pst_release_obj(tok1); }
        if !tok2.is_null() { pst_release_obj(tok2); }
    }
    ...
    return if status < 0i32 {
               -1i32
           } else { CMap_is_valid(cmap) as libc::c_int };
}
