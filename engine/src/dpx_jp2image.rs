...
/* Label */
unsafe extern "C" fn read_box_hdr(mut fp: *mut FILE,
    ...
    *tbox = get_unsigned_quad(fp);
    ...
    if *lbox == 1i32 as libc::c_uint {
        if get_unsigned_quad(fp) != 0i32 as libc::c_uint {
            _tt_abort(b"JPEG2000: LBox value in JP2 file >32 bits.\nI can\'t handle this!\x00"
                          as *const u8 as *const libc::c_char);
        }
        *lbox = get_unsigned_quad(fp);
        bytesread = bytesread.wrapping_add(8i32 as libc::c_uint)
    } else if *lbox > 1i32 as libc::c_uint && *lbox < 8i32 as libc::c_uint {
        dpx_warning(b"JPEG2000: Unknown LBox value %u in JP2 file!\x00" as
                        *const u8 as *const libc::c_char, *lbox);
    }
    return bytesread;
}
unsafe extern "C" fn check_jp___box(mut fp: *mut FILE) -> libc::c_int {
    if get_unsigned_quad(fp) != 0xci32 as libc::c_uint { return 0i32 }
    if get_unsigned_quad(fp) != 0x6a502020i32 as libc::c_uint { return 0i32 }
    /* Next 4 bytes shall be 0D 0A 87 0A */
    if get_unsigned_quad(fp) != 0xd0a870ai32 as libc::c_uint { return 0i32 }
    return 1i32;
}
unsafe extern "C" fn check_ftyp_data(mut fp: *mut FILE,
    ...
    match BR {
        ...
        1785755680 => {
            ...
            while size > 0i32 as libc::c_uint {
                ...
                if CLi == 0x6a707862i32 as libc::c_uint { supported = 1i32 }
                size = size.wrapping_sub(4i32 as libc::c_uint)
            }
        }
        _ => {
            dpx_warning(b"JPEG2000: Unknown JPEG 2000 File Type box Brand field value.\x00"
                            as *const u8 as *const libc::c_char);
            seek_relative(fp, size as int32_t);
            size = 0i32 as libc::c_uint;
            supported = 0i32
...
unsafe extern "C" fn read_res__data(mut info: *mut ximage_info,
                                    mut fp: *mut FILE,
    ...
    (*info).xdensity =
        72.0f64 /
            (HR_N as libc::c_double / HR_D as libc::c_double *
                 pow(10.0f64, HR_E as libc::c_double) * 0.0254f64);
    (*info).ydensity =
        72.0f64 /
            (VR_N as libc::c_double / VR_D as libc::c_double *
                 pow(10.0f64, VR_E as libc::c_double) * 0.0254f64);
}
unsafe extern "C" fn scan_res_(mut info: *mut ximage_info, mut fp: *mut FILE,
                               mut size: libc::c_uint) -> libc::c_int {
    ...
    while size > 0i32 as libc::c_uint {
        ...
        if lbox == 0i32 as libc::c_uint {
            dpx_warning(b"JPEG2000: Unexpected lbox value 0 in JP2 Resolution box.\x00"
                            as *const u8 as *const libc::c_char);
            break ;
        } else {
            match tbox {
                ...
                _ => {
                    dpx_warning(b"JPEG2000: Unknown JPEG 2000 box type in Resolution box.\x00"
                                    as *const u8 as *const libc::c_char);
                    seek_relative(fp, lbox.wrapping_sub(len) as int32_t);
                }
            }
    ...
    return if size == 0i32 as libc::c_uint { 0i32 } else { -1i32 };
}
 ...
 */
unsafe extern "C" fn scan_cdef(mut info: *mut ximage_info,
                               mut smask: *mut libc::c_int, mut fp: *mut FILE,
                               mut size: libc::c_uint) -> libc::c_int {
    ...
    *smask = 0i32;
    ...
    if size <
           N.wrapping_mul(6i32 as
                              libc::c_uint).wrapping_add(2i32 as libc::c_uint)
       {
        dpx_warning(b"JPEG2000: Inconsistent N value in Channel Definition box.\x00"
                        as *const u8 as *const libc::c_char);
        return -1i32
    }
    ...
    while i < N {
        ...
        if Cn > N {
            dpx_warning(b"JPEG2000: Invalid Cn value in Channel Definition box.\x00"
                            as *const u8 as *const libc::c_char);
        }
        if Typ == 1i32 as libc::c_uint {
            if Asoc == 0i32 as libc::c_uint { have_type0 = 1i32 }
            opacity_channels += 1
        } else if Typ == 2i32 as libc::c_uint { opacity_channels += 1 }
        i = i.wrapping_add(1)
    }
    if opacity_channels == 1i32 {
        *smask = if have_type0 != 0 { 1i32 } else { 0i32 }
    } else if opacity_channels > 1i32 {
        dpx_warning(b"JPEG2000: Unsupported transparency type. (ignored)\x00"
                        as *const u8 as *const libc::c_char);
    }
    return 0i32;
}
unsafe extern "C" fn scan_jp2h(mut info: *mut ximage_info,
                               mut smask: *mut libc::c_int, mut fp: *mut FILE,
                               mut size: libc::c_uint) -> libc::c_int {
    ...
    while size > 0i32 as libc::c_uint && error == 0 {
        len = read_box_hdr(fp, &mut lbox, &mut tbox);
        if lbox == 0i32 as libc::c_uint {
            dpx_warning(b"JPEG2000: Unexpected lbox value 0 in JP2 Header box...\x00"
                            as *const u8 as *const libc::c_char);
            error = -1i32;
            break ;
        } else {
            match tbox {
                1768449138 => {
                    ...
                    (*info).num_components =
                        get_unsigned_pair(fp) as libc::c_int;
                    /* c = */
                ...
                1919251232 => {
                    error = scan_res_(info, fp, lbox.wrapping_sub(len))
                }
                1667523942 => {
                    error = scan_cdef(info, smask, fp, lbox.wrapping_sub(len))
                }
                ...
                _ => {
                    dpx_warning(b"JPEG2000: Unknown JPEG 2000 box in JP2 Header box.\x00"
                                    as *const u8 as *const libc::c_char);
                    seek_relative(fp, lbox.wrapping_sub(len) as int32_t);
                    error = -1i32
                }
    ...
    if have_ihdr == 0 {
        dpx_warning(b"JPEG2000: Expecting JPEG 2000 Image Header box but could not find.\x00"
                        as *const u8 as *const libc::c_char);
    }
...
unsafe extern "C" fn scan_file(mut info: *mut ximage_info,
                               mut smask: *mut libc::c_int, mut fp: *mut FILE)
 -> libc::c_int {
    let mut error: libc::c_int = 0i32;
  ...
  /* JPEG 2000 Singature box */
    if check_jp___box(fp) == 0 { return -1i32 }
    size -= 12i32;
    /* File Type box shall immediately follow */
    len = read_box_hdr(fp, &mut lbox, &mut tbox);
    if tbox != 0x66747970i32 as libc::c_uint { return -1i32 }
    if check_ftyp_data(fp, lbox.wrapping_sub(len)) == 0 { return -1i32 }
    size =
        (size as libc::c_uint).wrapping_sub(lbox) as libc::c_int as
            libc::c_int;
    /* Search for JP2 Header box */
    while size > 0i32 && error == 0 {
        len = read_box_hdr(fp, &mut lbox, &mut tbox);
        if lbox == 0i32 as libc::c_uint { lbox = size as libc::c_uint }
        match tbox {
            1785737832 => {
            ...
            1785737827 => {
                ...
                if have_jp2h == 0 {
                    dpx_warning(b"JPEG2000: JPEG 2000 Codestream box found before JP2 Header box.\x00"
                                    as *const u8 as *const libc::c_char);
                }
                seek_relative(fp, lbox.wrapping_sub(len) as int32_t);
            }
            _ => { seek_relative(fp, lbox.wrapping_sub(len) as int32_t); }
        }
        size =
            (size as libc::c_uint).wrapping_sub(lbox) as libc::c_int as
                libc::c_int
   ...
   */
    if have_jp2h == 0 && error == 0 {
        dpx_warning(b"JPEG2000: No JP2 Header box found. Not a JP2/JPX baseline file?\x00"
                        as *const u8 as *const libc::c_char);
        error = -1i32
    }
    return error;
...
pub unsafe extern "C" fn check_for_jp2(mut fp: *mut FILE) -> libc::c_int {
    ...
    let mut lbox: libc::c_uint = 0;
    let mut tbox: libc::c_uint = 0;
    if fp.is_null() { return 0i32 }
    rewind(fp);
    /* JPEG 2000 Singature box */
    if check_jp___box(fp) == 0 { return 0i32 }
    /* File Type box shall immediately follow */
    len = read_box_hdr(fp, &mut lbox, &mut tbox);
    if tbox != 0x66747970i32 as libc::c_uint { return 0i32 }
    if check_ftyp_data(fp, lbox.wrapping_sub(len)) == 0 { return 0i32 }
    return 1i32;
}
#[no_mangle]
pub unsafe extern "C" fn jp2_include_image(mut ximage: *mut pdf_ximage,
    ...
    if pdf_version < 5i32 as libc::c_uint {
        dpx_warning(b"JPEG 2000 support requires PDF version >= 1.5 (Current setting 1.%d)\n\x00"
                        as *const u8 as *const libc::c_char, pdf_version);
        return -1i32
    }
    ...
    if scan_file(&mut info, &mut smask, fp) < 0i32 {
        dpx_warning(b"JPEG2000: Reading JPEG 2000 file failed.\x00" as
    ...
    pdf_add_dict(stream_dict,
                 pdf_new_name(b"Filter\x00" as *const u8 as
                                  *const libc::c_char),
                 pdf_new_name(b"JPXDecode\x00" as *const u8 as
                                  *const libc::c_char));
    if smask != 0 {
        pdf_add_dict(stream_dict,
                     pdf_new_name(b"SMaskInData\x00" as *const u8 as
                                      *const libc::c_char),
    ...
    loop  {
        nb_read =
            fread(work_buffer.as_mut_ptr() as *mut libc::c_void,
                  ::std::mem::size_of::<libc::c_char>() as libc::c_ulong,
                  1024i32 as libc::c_ulong, fp) as libc::c_int;
        if !(nb_read > 0i32) { break ; }
...
*/
#[no_mangle]
pub unsafe extern "C" fn jp2_get_bbox(mut fp: *mut FILE,
