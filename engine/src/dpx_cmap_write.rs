...
unsafe extern "C" fn block_count(mut mtab: *mut mapDef, mut c: libc::c_int)
 -> size_t {
    let mut count: size_t = 0i32 as size_t;
    let mut n: size_t = 0;
    n = (*mtab.offset(c as isize)).len.wrapping_sub(1i32 as libc::c_ulong);
    c += 1i32;
    while c < 256i32 {
        if (*mtab.offset(c as isize)).flag & 1i32 << 4i32 != 0 ||
               ...
               (*mtab.offset((c - 1i32) as isize)).len !=
                   (*mtab.offset(c as isize)).len {
            break ;
        }
        if !(memcmp((*mtab.offset((c - 1i32) as isize)).code as
                        *const libc::c_void,
                    (*mtab.offset(c as isize)).code as *const libc::c_void, n)
                 == 0 &&
                 (*(*mtab.offset((c - 1i32) as isize)).code.offset(n as isize)
                      as libc::c_int) < 255i32 &&
                 *(*mtab.offset((c - 1i32) as isize)).code.offset(n as isize)
                     as libc::c_int + 1i32 ==
                     *(*mtab.offset(c as isize)).code.offset(n as isize) as
                         libc::c_int) {
            break ;
        }
...
unsafe extern "C" fn sputx(mut c: libc::c_uchar,
                           mut s: *mut *mut libc::c_char,
                           mut end: *mut libc::c_char) -> libc::c_int {
    ...
    **s =
        (if (hi as libc::c_int) < 10i32 {
         ...
         } else { hi as libc::c_int + '7' as i32 }) as libc::c_char;
    *(*s).offset(1) =
        (if (lo as libc::c_int) < 10i32 {
             lo as libc::c_int + '0' as i32
         } else { lo as libc::c_int + '7' as i32 }) as libc::c_char;
    *s = (*s).offset(2);
    return 2i32;
}
unsafe extern "C" fn write_map(mut mtab: *mut mapDef, mut count: size_t,
                               mut codestr: *mut libc::c_uchar,
                               mut depth: size_t, mut wbuf: *mut sbuf,
                               mut stream: *mut pdf_obj) -> libc::c_int {
    let mut c: size_t = 0;
    let mut i: size_t = 0;
    let mut block_length: size_t = 0;
    let mut mtab1: *mut mapDef = 0 as *mut mapDef;
    /* Must be greater than 1 */
    let mut blocks: [C2RustUnnamed_1; 129] =
        [C2RustUnnamed_1{start: 0, count: 0,}; 129];
    let mut num_blocks: size_t = 0i32 as size_t;
    c = 0i32 as size_t;
    while c < 256i32 as libc::c_ulong {
        *codestr.offset(depth as isize) =
            (c & 0xffi32 as libc::c_ulong) as libc::c_uchar;
        if (*mtab.offset(c as isize)).flag & 1i32 << 4i32 != 0 {
            mtab1 = (*mtab.offset(c as isize)).next;
            count =
                write_map(mtab1, count, codestr,
                          depth.wrapping_add(1i32 as libc::c_ulong), wbuf,
                          stream) as size_t
            ...
            match (*mtab.offset(c as isize)).flag & 0xfi32 {
                1 | 4 => {
                    ...
                    if block_length >= 2i32 as libc::c_ulong {
                        ...
                        blocks[num_blocks as usize].count =
                            block_length as libc::c_int;
                        num_blocks = num_blocks.wrapping_add(1);
                        c =
                            (c as libc::c_ulong).wrapping_add(block_length) as
                                size_t as size_t
                    } else {
                        ...
                        *fresh0 = '<' as i32 as libc::c_char;
                        i = 0i32 as size_t;
                        while i <= depth {
                            sputx(*codestr.offset(i as isize),
                                  &mut (*wbuf).curptr, (*wbuf).limptr);
                            i = i.wrapping_add(1)
                        }
                        ...
                        *fresh3 = '<' as i32 as libc::c_char;
                        ...
                        while i < (*mtab.offset(c as isize)).len {
                            sputx(*(*mtab.offset(c as
                                                     isize)).code.offset(i as
                                                                             isize),
                                  &mut (*wbuf).curptr, (*wbuf).limptr);
                            i = i.wrapping_add(1)
                        }
                ...
                8 => { }
                _ => {
                    _tt_abort(b"%s: Unknown mapping type: %d\x00" as *const u8
        ...
        if count >= 100i32 as libc::c_ulong ||
            ...
            if count > 100i32 as libc::c_ulong {
                _tt_abort(b"Unexpected error....: %zu\x00" as *const u8 as
            ...
            pdf_add_stream(stream, (*wbuf).buf as *const libc::c_void,
                           (*wbuf).curptr.wrapping_offset_from((*wbuf).buf) as
    ...
    if num_blocks > 0i32 as libc::c_ulong {
        ...
        sprintf(fmt_buf_0.as_mut_ptr(),
                ...
                num_blocks);
        pdf_add_stream(stream, fmt_buf_0.as_mut_ptr() as *const libc::c_void,
                       strlen(fmt_buf_0.as_mut_ptr()) as libc::c_int);
        i = 0i32 as size_t;
        while i < num_blocks {
            let mut j: size_t = 0;
            ...
            *fresh6 = '<' as i32 as libc::c_char;
            j = 0i32 as size_t;
            while j < depth {
                sputx(*codestr.offset(j as isize), &mut (*wbuf).curptr,
                      (*wbuf).limptr);
                j = j.wrapping_add(1)
            }
            ...
            *fresh9 = '<' as i32 as libc::c_char;
            ...
            sputx(c.wrapping_add(blocks[i as usize].count as libc::c_ulong) as
                      libc::c_uchar, &mut (*wbuf).curptr, (*wbuf).limptr);
            ...
            *fresh12 = '<' as i32 as libc::c_char;
            ...
            while j < (*mtab.offset(c as isize)).len {
                sputx(*(*mtab.offset(c as isize)).code.offset(j as isize),
                      &mut (*wbuf).curptr, (*wbuf).limptr);
                j = j.wrapping_add(1)
            }
...
*/
#[no_mangle]
pub unsafe extern "C" fn CMap_create_stream(mut cmap: *mut CMap)
 -> *mut pdf_obj {
    ...
    if (*cmap).type_0 == 0i32 { return 0 as *mut pdf_obj }
    ...
    if (*cmap).type_0 != 2i32 {
        ...
        pdf_add_dict(stream_dict,
                     pdf_new_name(b"Type\x00" as *const u8 as
                                      *const libc::c_char),
                     pdf_new_name(b"CMap\x00" as *const u8 as
                                      *const libc::c_char));
        pdf_add_dict(stream_dict,
                     pdf_new_name(b"CMapName\x00" as *const u8 as
                                      *const libc::c_char),
        ...
        if (*cmap).wmode != 0i32 {
            pdf_add_dict(stream_dict,
                         pdf_new_name(b"WMode\x00" as *const u8 as
                                          *const libc::c_char),
   ...
   */
    if !(*cmap).useCMap.is_null() {
        _tt_abort(b"UseCMap found (not supported yet)...\x00" as *const u8 as
                      *const libc::c_char);
    }
    wbuf.buf =
    ...
    codestr =
        new(((*cmap).profile.maxBytesIn as uint32_t as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_uchar>()
                                                 as libc::c_ulong) as
                uint32_t) as *mut libc::c_uchar;
    memset(codestr as *mut libc::c_void, 0i32, (*cmap).profile.maxBytesIn);
    wbuf.curptr = wbuf.buf;
    wbuf.limptr =
        wbuf.buf.offset(4096).offset(-((2i32 as
                                            libc::c_ulong).wrapping_mul((*cmap).profile.maxBytesIn.wrapping_add((*cmap).profile.maxBytesOut))
                                           as isize)).offset(16);
    /* Start CMap */
    pdf_add_stream(stream,
                   ...
                   strlen(b"/CIDInit /ProcSet findresource begin\n12 dict begin\nbegincmap\n\x00"
                       ...
                       libc::c_int);
    wbuf.curptr =
        wbuf.curptr.offset(sprintf(wbuf.curptr,
                                   b"/CMapName /%s def\n\x00" as *const u8 as
                                       *const libc::c_char, (*cmap).name) as
                               isize);
    wbuf.curptr =
        wbuf.curptr.offset(sprintf(wbuf.curptr,
                                   b"/CMapType %d def\n\x00" as *const u8 as
                                       *const libc::c_char, (*cmap).type_0) as
                               isize);
    if (*cmap).wmode != 0i32 && (*cmap).type_0 != 2i32 {
        wbuf.curptr =
            wbuf.curptr.offset(sprintf(wbuf.curptr,
                                       b"/WMode %d def\n\x00" as *const u8 as
                                           *const libc::c_char, (*cmap).wmode)
                                   as isize)
    }
    wbuf.curptr =
        wbuf.curptr.offset(sprintf(wbuf.curptr,
                                   ...
                                   (*csi).supplement) as isize);
    pdf_add_stream(stream, wbuf.buf as *const libc::c_void,
                   wbuf.curptr.wrapping_offset_from(wbuf.buf) as libc::c_long
                       as libc::c_int);
    wbuf.curptr = wbuf.buf;
    /* codespacerange */
    ...
    while i < (*cmap).codespace.num as libc::c_ulong {
        ...
        *fresh15 = '<' as i32 as libc::c_char;
        ...
        while j < (*ranges.offset(i as isize)).dim {
            sputx(*(*ranges.offset(i as isize)).codeLo.offset(j as isize),
                  &mut wbuf.curptr, wbuf.limptr);
            j = j.wrapping_add(1)
        }
        ...
        *fresh18 = '<' as i32 as libc::c_char;
        j = 0i32 as size_t;
        while j < (*ranges.offset(i as isize)).dim {
            sputx(*(*ranges.offset(i as isize)).codeHi.offset(j as isize),
                  &mut wbuf.curptr, wbuf.limptr);
            j = j.wrapping_add(1)
        }
    ...
    if !(*cmap).mapTbl.is_null() {
        count =
            write_map((*cmap).mapTbl, 0i32 as size_t, codestr, 0i32 as size_t,
                      &mut wbuf, stream) as size_t; /* Top node */
        if count > 0i32 as libc::c_ulong {
            /* Flush */
            let mut fmt_buf: [libc::c_char; 32] = [0; 32];
            if count > 100i32 as libc::c_ulong {
